...
test("atlas LOD remains registered to the same world coordinates", () => {
  ...
  const close = manifest.levels.find((level) => level.id === "atlas-24000-close-tiles");
  assert.ok(close);
  assert.equal(close.status, "active");
  assert.equal(close.columns, 12);
  assert.equal(close.rows, 8);
  assert.equal(close.tileWorldWidth, 10);
  assert.equal(close.tileWorldHeight, 10);
  assert.equal(close.tilePixelWidth, 2000);
  assert.equal(close.tilePixelHeight, 2000);
  assert.equal(close.columns * close.tileWorldWidth, 120);
  assert.equal(close.rows * close.tileWorldHeight, 80);
});
...
test("current Tideworn navigation architecture is present and atlas runtime only observes it", async () => {
  ...
  const manifestText = await readFile(new URL("../public/alpha/atlas-lod-manifest.json", import.meta.url), "utf8");
  ...
  assert.doesNotMatch(runtime, /atlas-map-label-size/);
  assert.doesNotMatch(runtime, /atlas-map-label-stroke/);
  assert.match(runtime, /currentLevelId/);
  assert.match(runtime, /this\.currentLevelId!==level\.id/);
  assert.match(runtime, /this\.clearTiles\(\)/);
  assert.match(runtime, /`\$\{level\.id\}:\$\{col\},\$\{row\}`/);
  assert.doesNotMatch(runtime, /addEventListener\("(?:pointerdown|pointermove|pointerup|wheel|click)"/);
  assert.match(css, /pointer-events:\s*none/);
  assert.doesNotMatch(css, /port-glyph text/);
  assert.doesNotMatch(css, /poi-glyph text/);
  assert.match(manifestText, /atlas-24000-close-tiles/);
});
