...
import { NAV_CAMERA, cameraForPoint, cameraForPoints, cameraLod, cameraViewBox, clampCameraCenter, clampViewWidth, legacyZoomBand, panCameraTarget, regionalLayerOpacity, viewHeightForWidth, zoomAroundAnchor, type NavigationCameraState } from "../game/navigationCamera.js";
import { hasLocalSave, loadLocal, saveLocal } from "../services/localSave.js";
import { audio, type AudioCue, type AudioScene } from "./audio.js";
...
import { learningSourcesAtCurrentContext, participateInReligiousLife, religiousParticipationProfile, studyLearningSource, type LearningSourceQuote } from "../game/characterConsequences.js";
...
import { affiliationMarkForCharacter, governmentIdentityMarksForPort, homelandMarkForRegion, primaryIdentityMark, religionIdentityMarksForPort, resolveIdentityPresentation, shipIdentityMarkForContext, type IdentityMarkDefinition, type IdentityPresentation } from "../data/seed/identitySymbols.js";
import { rankCuratedPortraits } from "../game/portraits.js";
import { CONTEXT_LOCATION_SCENE_ART, LOCATION_PRESENTATION_ART, PRESENTATION_BASE_ART, type ContextLocationSceneKind } from "../data/seed/presentationArt.js";
import { CANON_WORLD_LOCATION_BY_ID } from "../data/seed/settlementCanon.js";
import { POLITICAL_POWER_BY_FACTION, politicalPowerForRegion } from "../data/seed/politicalPowers.js";
...
import { artCalibrationEnabled, initializeArtDirectedCanvases, installArtCalibrationShortcut, renderArtDirectedCanvas } from "../artLayouts/ArtDirectedCanvas.js";
...
import type { ArtDirectedLayer, ArtLayoutManifest } from "../artLayouts/types.js";
...
type TabId = "town" | "ship" | "market" | "inventory" | "crew" | "people" | "tavern" | "government" | "religion" | "journal" | "chart" | "poi";
type CharacterPanelTab = "sheet" | "gear" | "outfitter";
type InventoryFilter = "all" | "weapons" | "clothing" | "tools" | "goods" | "charts" | "documents" | "quest";
type EquipmentTemplateVariant = "male" | "female";
type ShipPanelTab = "overview" | "work";
type JournalTab = "knowledge" | "contacts" | "obligations" | "law" | "history";
type LauncherMode = "new" | "continue";
interface LauncherRequestDetail { requestId: string; mode: LauncherMode; }
interface LauncherResultDetail { requestId: string; mode: LauncherMode; ok: boolean; hasLocalSave: boolean; message?: string; }
let state: GameState | undefined;
let tab: TabId = "ship";
let selectedMapTarget: NavigationTarget | undefined;
let selectedInventoryOwnerId: InventoryOwnerId = "player";
let selectedInventoryItemId: string | undefined;
let captainPanelTab: CharacterPanelTab = "sheet";
let crewPanelTab: CharacterPanelTab = "sheet";
let inventoryFilters: Record<string, InventoryFilter> = { player: "all" };
let shipPanelTab: ShipPanelTab = "overview";
let marketPage = 0;
let marketTradeQuantities: Record<string, number> = {};
let lastRenderedGameViewKey: string | undefined;
let crewPage = 0;
let journalTab: JournalTab = "knowledge";
let journalPage = 0;
let creatorStep = 0;
let inventoryPages: Record<string, number> = { player: 0 };
let dialogueLines: Array<{ speaker: string; text: string }> = [];
let activeDialogueNpcId: string | undefined;
let inspectedCrewNpcId: string | undefined;
let mapCameraState: NavigationCameraState | undefined;
let mapCameraFrame: number | undefined;
let mapDragState: { pointerId:number; startClientX:number; startClientY:number; startTargetX:number; startTargetY:number; dragging:boolean } | undefined;
...
function titleize(input: string): string { return input.split("_").map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join(" "); }
function pct(value: number): string { return `${Math.max(0, Math.min(100, Math.round(value)))}%`; }
function tip(text:string):string { return `data-tooltip="${esc(text)}" tabindex="0"`; }
...
const IMPLEMENTED_CHARACTER_CULTURE_PACKS=new Set<CharacterCreationChoices["culture"]>(["skeldran"]);
const IMPLEMENTED_CHARACTER_RELIGION_PACKS=new Set<CharacterCreationChoices["religion"]>(["old_gods","covenant"]);

function characterThemeAttributes(culture:CharacterCreationChoices["culture"],religion:CharacterCreationChoices["religion"]):string {
...
function syncCreatorCharacterTheme(form:HTMLFormElement):void {
  const root=app.querySelector<HTMLElement>(".creator-production-screen");
  if(!root)return;
  const culture=String((form.elements.namedItem("culture") as HTMLSelectElement|null)?.value??"skeldran") as CharacterCreationChoices["culture"];
  const religion=String((form.elements.namedItem("religion") as HTMLSelectElement|null)?.value??"unaffiliated") as CharacterCreationChoices["religion"];
...
function effectiveTradeCredentials(s:GameState):GameState["player"]["tradeCredentials"] {
  return s.player.tradeCredentials.filter(row=>{
    if(row.status!=="active"||(row.expiresAtHour!==undefined&&s.absoluteHour>=row.expiresAtHour))return false;
    if(row.kind==="customs_permit")return true;
    return activeLetterOfMarque(s,row.factionId)?.id===row.id;
  });
}

function tradeCredentialSummary(row:GameState["player"]["tradeCredentials"][number]):{title:string;detail:string} {
  ...
  if(row.kind==="customs_permit")return {title:`Customs Permit · ${issuer}`,detail:`Current trade papers${expiry}`};
...
interface CrewReactionSnapshot { morale:number; loyalty:number; }
interface CrewReactionDelta { morale:number; loyalty:number; }

function crewReactionSnapshot(s:GameState):CrewReactionSnapshot {
...
function crewReactionDelta(before:CrewReactionSnapshot,s:GameState):CrewReactionDelta {
...
function showCrewReaction(delta:CrewReactionDelta,context:"generic"|"shore_leave"|"prize_share"|"underprovisioned"|"battle"="generic"):void {
  if(Math.abs(delta.morale)<1 && Math.abs(delta.loyalty)<1)return;
...
function handleCrewResult(s:GameState,before:CrewReactionSnapshot,result:{ok:boolean;message:string},context:"generic"|"shore_leave"|"prize_share"|"underprovisioned"|"battle"="generic",sound?:AudioCue):void {
  const delta=result.ok?crewReactionDelta(before,s):{morale:0,loyalty:0};
  if(sound&&result.ok)cue(sound);
  toast(result.message);
  renderGame();
  if(result.ok)showCrewReaction(delta,context);
}

function revealWithinLocalPane(selector: string, align: "center" | "end" = "center"): void {
  const target = document.querySelector<HTMLElement>(selector);
  if (!target) return;
  let pane: HTMLElement | null = target.parentElement;
  while (pane && pane !== app) {
    const overflowY = getComputedStyle(pane).overflowY;
    if ((overflowY === "auto" || overflowY === "scroll") && pane.scrollHeight > pane.clientHeight) break;
    pane = pane.parentElement;
  }
  if (!pane || pane === app) return;
...
function itemSummaryLine(defId: string): string {
  const def = ITEM_BY_ID[defId];
  if (!def) return "Unknown item";
  const parts = [titleize(def.category)];
  if (def.attack) parts.push(`ATK ${def.attack}`);
  if (def.defense) parts.push(`DEF ${def.defense}`);
  return parts.join(" · ");
}

function ownerDisplayName(s: GameState, ownerId: InventoryOwnerId): string {
  return ownerId === "player" ? s.player.character.name : s.npcs[ownerId]?.name ?? "Companion";
}

function playerIdentityPresentation(s: GameState): IdentityPresentation {
...
function npcIdentityPresentation(npc: NpcCharacter): IdentityPresentation {
  ...
  if (affiliationMark) presentation.affiliationMark = affiliationMark;
  return presentation;
}

function identityMarkHtml(mark: IdentityMarkDefinition, className = ""): string {
...
function identityMarkImage(mark: IdentityMarkDefinition, className = ""): string {
  return `<img class="context-symbol-image ${esc(className)}" src="${esc(mark.path)}" alt="${esc(mark.label)}" title="${esc(mark.label)}">`;
}

function uniqueMarks(marks: Array<IdentityMarkDefinition | undefined>): IdentityMarkDefinition[] {
  const seen = new Set<string>();
  return marks.filter((mark): mark is IdentityMarkDefinition => {
    if (!mark || seen.has(mark.id)) return false;
    seen.add(mark.id);
    return true;
  });
}

function markStripHtml(marks: Array<IdentityMarkDefinition | undefined>, className = ""): string {
...
function npcPublicMarks(npc: NpcCharacter): IdentityMarkDefinition[] {
...
function npcDetailMarks(npc: NpcCharacter): IdentityMarkDefinition[] {
  const identity = npcIdentityPresentation(npc);
  return uniqueMarks([primaryIdentityMark(identity), identity.religionMark]);
}

function portPrimaryMark(portId: string): IdentityMarkDefinition | undefined {
  const port = PORT_BY_ID[portId];
  return port ? homelandMarkForRegion(port.region) : undefined;
}

function shipIdentityMark(s: GameState, shipId: string): IdentityMarkDefinition | undefined {
...
function identityStyle(presentation: IdentityPresentation): string {
...
function displaySlotName(slot: EquipmentSlot): string {
...
function itemVisual(definitionId: string | undefined) {
  const def = definitionId ? ITEM_BY_ID[definitionId] : undefined;
  const art = def?.artAssetId ? ASSET_BY_ID[def.artAssetId] : undefined;
  return { def, art };
}

function inventoryFilterFor(ownerId: InventoryOwnerId): InventoryFilter {
...
function itemMatchesFilter(definitionId: string, filter: InventoryFilter): boolean {
  if (filter === "all") return true;
  const def = ITEM_BY_ID[definitionId];
  if (!def) return false;
  if (filter === "weapons") return def.category === "weapon" || def.category === "firearm";
  if (filter === "clothing") return def.category === "armor" || def.category === "clothing" || def.slot === "head" || def.slot === "feet" || def.slot === "hands" || def.slot === "legs";
  if (filter === "tools") return def.category === "tool";
  if (filter === "charts") return def.category === "document" && (def.tags.includes("map") || def.tags.includes("chart") || def.name.toLowerCase().includes("chart"));
  if (filter === "documents") return def.category === "document" || def.category === "book";
  if (filter === "quest") return def.tags.includes("quest") || def.tags.includes("quest_item") || def.tags.includes("key_item");
  return def.category === "consumable" || def.category === "misc" || def.category === "trinket" || def.category === "relic";
}

function filteredInventory(s: GameState, ownerId: InventoryOwnerId): ReturnType<typeof inventoryForOwner> {
...
function ensureSelectedInventoryItem(s: GameState, ownerId: InventoryOwnerId): string | undefined {
  const items = filteredInventory(s, ownerId);
  if (!items.length) {
    if (selectedInventoryOwnerId === ownerId) selectedInventoryItemId = undefined;
    return undefined;
  }
  if (selectedInventoryOwnerId === ownerId && selectedInventoryItemId && items.some((item) => item.id === selectedInventoryItemId)) return selectedInventoryItemId;
  selectedInventoryOwnerId = ownerId;
  selectedInventoryItemId = items[0]?.id;
  return selectedInventoryItemId;
}

function inventoryTooltip(s: GameState, ownerId: InventoryOwnerId, instanceId: string): string {
  const inst = inventoryForOwner(s, ownerId).find((item) => item.id === instanceId);
  const def = inst ? ITEM_BY_ID[inst.definitionId] : undefined;
  if (!inst || !def) return "Unavailable item data.";
  const parts = [def.name, itemSummaryLine(def.id), `${titleize(inst.quality)} quality`, `${titleize(inst.condition)} condition`];
  if (def.description) parts.push(def.description);
  return parts.join(" · ");
}

function equipmentTemplateVariantForOwner(s: GameState, ownerId: InventoryOwnerId): EquipmentTemplateVariant {
  const sex = ownerId === "player" ? s.player.character.sex : s.npcs[ownerId]?.sex;
  return sex === "female" ? "female" : "male";
}

function itemAllowedSlots(definitionId: string | undefined, slots: EquipmentSlot[]): EquipmentSlot[] {
  const def = definitionId ? ITEM_BY_ID[definitionId] : undefined;
  if (!def) return [];
  return (def.allowedSlots ?? (def.slot ? [def.slot] : [])).filter((slot) => slots.includes(slot));
}

function ownerStatsForGearScreen(s: GameState, ownerId: InventoryOwnerId) {
  if (ownerId === "player") {
    return {
      title: "Captain",
  ...
  return {
    ...
    carried: inventoryForOwner(s, ownerId).length,
    coin: undefined as number | undefined,
    motto: npc.speakingStyle
  };
}

function equipmentManifestForOwner(s: GameState, ownerId: InventoryOwnerId): ArtLayoutManifest {
  ...
  if (!manifest) throw new Error(`Missing art layout manifest: ${layoutId}`);
  return manifest;
}

function manifestFor(layoutId: string): ArtLayoutManifest {
  const manifest = artLayoutById(layoutId);
  if (!manifest) throw new Error(`Missing art layout manifest: ${layoutId}`);
  return manifest;
}

function runtimeArtPathFor(manifest: ArtLayoutManifest): string {
  const assetId = manifest.runtimeBaseAssetId ?? manifest.assetId;
  const asset = ASSET_BY_ID[assetId];
  if (!asset?.path) throw new Error(`Missing runtime art asset for ${manifest.layoutId}: ${assetId}`);
  if (asset.artUsage === "reference") throw new Error(`Reference-only art cannot be used as runtime base: ${assetId}`);
  return asset.path;
}

function renderMappedArtScreen(layoutId: string, layers: ArtDirectedLayer[], options: { className?: string; hostClassName?: string; ariaLabel?: string; screenId?: string } = {}): string {
...
function regionIdForEquipmentSlot(slot: EquipmentSlot): string {
...
function renderEquipmentIdentityLayers(s: GameState, ownerId: InventoryOwnerId): ArtDirectedLayer[] {
...
function renderEquipmentSummaryLayers(s: GameState, ownerId: InventoryOwnerId): ArtDirectedLayer[] {
...
function renderEquipmentPrimaryLayers(ownerId: InventoryOwnerId): ArtDirectedLayer[] {
  const current = inventoryFilterFor(ownerId);
  const modes: Array<[string, InventoryFilter, string]> = [
...
function renderEquipmentFilterLayers(ownerId: InventoryOwnerId): ArtDirectedLayer[] {
  const current = inventoryFilterFor(ownerId);
...
function renderEquipmentSlotLayers(s: GameState, ownerId: InventoryOwnerId, slots: EquipmentSlot[], equipment: Record<string, string | undefined>): ArtDirectedLayer[] {
  ...
  return slots.map((slot) => {
    ...
    } satisfies ArtDirectedLayer;
  });
}

function renderInventoryGridContent(s: GameState, ownerId: InventoryOwnerId, slots: EquipmentSlot[]): string {
  const items = filteredInventory(s, ownerId);
  const cellCount = 30;
  ...
  return `<div class="equipment-grid-inside-art">${Array.from({ length: cellCount }, (_, index) => {
    ...
    if (!inst) return `<div class="equipment-grid-cell empty"></div>`;
    const { def, art } = itemVisual(inst.definitionId);
...
function renderSelectedItemPanelContent(s: GameState, ownerId: InventoryOwnerId, slots: EquipmentSlot[]): string {
  const selectedId = ensureSelectedInventoryItem(s, ownerId);
  if (!selectedId) return `<div class="equipment-detail-inside-art empty"><div class="equipment-detail-title">No item selected</div><p>Select an item in the grid to inspect it or equip it.</p></div>`;
  const inst = inventoryForOwner(s, ownerId).find((item) => item.id === selectedId);
  const { def, art } = itemVisual(inst?.definitionId);
  if (!inst || !def) return `<div class="equipment-detail-inside-art empty"><div class="equipment-detail-title">Item unavailable</div></div>`;
...
function renderArtFirstEquipmentScreen(s: GameState, ownerId: InventoryOwnerId, slots: EquipmentSlot[]): string {
  const manifest = equipmentManifestForOwner(s, ownerId);
  const equipment = equipmentForOwner(s, ownerId);
  if (!equipment) return `<div class="panel"><p class="muted">No equipment state available for this character.</p></div>`;
  const artPath = runtimeArtPathFor(manifest);
  const layers: ArtDirectedLayer[] = [
    ...
    ...renderEquipmentSlotLayers(s, ownerId, slots, equipment as Record<string, string | undefined>)
  ];
  const referenceArtPath = manifest.referenceAssetId ? ASSET_BY_ID[manifest.referenceAssetId]?.path : undefined;
...
function shipPortraitCard(s: GameState): string {
  const ship = getPlayerShip(s)!;
...
function setCreatorStepInDom(nextStep:number): void {
  creatorStep=Math.max(0,Math.min(5,nextStep));
  document.querySelectorAll<HTMLElement>("[data-creator-pane]").forEach(node=>{node.hidden=Number(node.dataset.creatorPane)!==creatorStep;});
  document.querySelectorAll<HTMLElement>("[data-creator-step-button]").forEach(node=>node.classList.toggle("active",Number(node.dataset.creatorStepButton)===creatorStep));
  const prev=document.querySelector<HTMLButtonElement>("[data-creator-prev]");
  const next=document.querySelector<HTMLButtonElement>("[data-creator-next]");
  const begin=document.querySelector<HTMLButtonElement>("[data-creator-begin]");
  if(prev) prev.disabled=creatorStep===0;
  if(next) next.hidden=creatorStep===5;
  if(begin) begin.hidden=creatorStep!==5;
  const current=document.querySelector<HTMLElement>("[data-creator-current]");
  if(current) current.textContent=`${creatorStep+1} / 6`;
}

function creatorReligionLabel(religion:string):string {
  if(religion==="covenant")return "Covenant of the One";
  if(religion==="pantheon")return "Central Pantheon";
  if(religion==="turning_wheel")return "Turning Wheel";
  if(religion==="unaffiliated")return "Unaffiliated / Skeptic";
  return titleize(religion);
}

function creatorBonusText(bonuses:Partial<Record<SkillId,number>>):string {
  return ALL_SKILLS.filter(skill=>(bonuses[skill]??0)!==0).map(skill=>`+${bonuses[skill]} ${SKILL_LABELS[skill]}`).join(" · ");
}

function creatorChoiceEffects(choices:CharacterCreationChoices):Array<{label:string;value:string;kind?:"direct"|"situational"}> {
  ...
  const rows:Array<{label:string;value:string;kind?:"direct"|"situational"}>=[];
  const socialExtras:string[]=[];
  if(choices.socialOrigin==="merchant_family")socialExtras.push("starts with 320 crowns");
  else if(choices.socialOrigin==="naval_family")socialExtras.push("starts with 270 crowns", "+8 Skeldran standing", "Heavy Weather specialization", "easier Admiralty instruction");
  if(creatorBonusText(social)||socialExtras.length)rows.push({label:`Social origin · ${titleize(choices.socialOrigin)}`,value:[creatorBonusText(social),...socialExtras].filter(Boolean).join(" · ")});

  const backgroundExtras:string[]=[];
  if(choices.background==="former_naval_midshipman")backgroundExtras.push("Naval Discipline specialization", "Precision Bore Sighting", "easier Admiralty instruction");
  if(["foundry_child","engineers_apprentice"].includes(choices.background))backgroundExtras.push("Naval Machinery specialization", "Emergency Hull Shoring", "faster/cheaper industrial instruction");
  if(choices.background==="raised_among_smugglers")backgroundExtras.push("Smuggling specialization");
  if(choices.background==="temple_educated")backgroundExtras.push("Read Wind", "Sense Resonance");
  if(creatorBonusText(background)||backgroundExtras.length)rows.push({label:`Background · ${titleize(choices.background)}`,value:[creatorBonusText(background),...backgroundExtras].filter(Boolean).join(" · ")});

  const professionExtras:string[]=[];
  if(choices.recentProfession==="navigator")professionExtras.push("Coastal Navigation specialization", "Calibrated Sextant Method");
  if(choices.recentProfession==="gunner")professionExtras.push("Long-Range Gunnery specialization", "Precision Bore Sighting");
  if(choices.recentProfession==="apprentice_engineer")professionExtras.push("Naval Machinery specialization", "Emergency Hull Shoring", "faster/cheaper industrial instruction");
  if(choices.recentProfession==="shipwright")professionExtras.push("faster/cheaper industrial instruction");
  if(choices.recentProfession==="smuggler")professionExtras.push("Smuggling specialization");
  if(choices.recentProfession==="sailor")professionExtras.push("Heavy Weather specialization");
  if(["scholar","priest"].includes(choices.recentProfession))professionExtras.push("Read Wind", "Sense Resonance");
  if(creatorBonusText(profession)||professionExtras.length)rows.push({label:`Profession · ${titleize(choices.recentProfession)}`,value:[creatorBonusText(profession),...professionExtras].filter(Boolean).join(" · ")});

  const traitExtras:string[]=[];
  if(choices.trait==="bookworm")traitExtras.push("written study takes 2 fewer hours");
  if(choices.trait==="superstitious")traitExtras.push("no stat bonus; notices omen/sailor interpretations in relevant conversations");
  if(creatorBonusText(trait)||traitExtras.length)rows.push({label:`Trait · ${titleize(choices.trait)}`,value:[creatorBonusText(trait),...traitExtras].filter(Boolean).join(" · ")});
  ...
  const shipExtras:string[]=[];
  if(shipImpact.hullBonus)shipExtras.push(`Tideworn +${shipImpact.hullBonus} current/max hull`);
  if(shipImpact.crownAdjustment)shipExtras.push(`${shipImpact.crownAdjustment} starting crowns`);
  if(shipImpact.cargo.length)shipExtras.push(shipImpact.cargo.map(row=>`${row.quantity} ${COMMODITY_BY_ID[row.commodityId]?.name??titleize(row.commodityId)}`).join(", ")+" starting cargo");
  if(choices.shipOrigin==="naval_surplus")shipExtras.push("Former Naval Surplus recognition", "easier Admiralty instruction");
  else if(choices.shipOrigin==="prize_share")shipExtras.push("prize-service history can matter with veteran sailors");
  else if(choices.shipOrigin==="inherited")shipExtras.push("family/local ship history can matter in the home port");
  else if(choices.shipOrigin==="purchased_on_debt")shipExtras.push("freight and idle-time pressure affect how opportunities are read; no separate debt meter yet");
  ...
  if(choices.religion!=="unaffiliated"){
    ...
    if(choices.religion==="old_gods"&&choices.devotion==="moderate")religionEffects.push("matching religious instruction: −1h / −4 crowns");
    if(choices.religion==="old_gods"&&choices.devotion==="devout")religionEffects.push("matching religious instruction: −2h / −8 crowns");
  ...
  } else rows.push({label:"Faith · Unaffiliated / Skeptic",value:"No own-faith starting knowledge; religious institutions are approached as an observer.",kind:"situational"});
...
function creatorToolboxHtml(choices:CharacterCreationChoices):string {
  const capability=buildCapabilityState(choices);
  const attributes=(Object.entries(capability.attributes) as Array<[keyof Attributes,number]>).map(([id,value])=>`<span><small>${esc(titleize(id))}</small><b>${value}</b></span>`).join("");
...
function creatorEffectRowsHtml(choices:CharacterCreationChoices,kind:"direct"|"situational"):string {
...
function creatorImpactSummary(choices:CharacterCreationChoices):string {
  const capability=buildCapabilityState(choices);
...
function relationshipImportance(npc:NpcCharacter): number {
...
function knownRelationshipCharacters(s:GameState): NpcCharacter[] {
  const crewIds=new Set(s.player.crew.map(member=>member.npcId).filter((id):id is string=>Boolean(id)));
...
function knownRelationshipRows(s:GameState,limit?:number): string {
  const all=knownRelationshipCharacters(s);
  const known=typeof limit==="number"?all.slice(0,Math.max(0,limit)):all;
  if(!known.length)return `<p class="manuscript-empty">No significant personal relationships have entered the captain's record yet.</p>`;
...
function companyFeelingTowardCaptain(s:GameState): string {
  const ship=getPlayerShip(s);
  if(!ship)return "Unknown";
  const community=ensureCrewCommunity(ship);
  const unrest=crewUnrestProfile(s);
  if(unrest.level==="defiant")return "Resentful";
  if(unrest.level==="discontented")return "Uneasy";
  ...
  if(score>=62)return "Content";
  if(score>=48)return "Reserved";
  if(score>=34)return "Uneasy";
  return "Resentful";
}

const STRUCTURAL_NPC_PORTRAIT_PATHS:Record<string,string>={
...
function structuralNpcPortrait(npc:NpcCharacter|undefined,variant:"crew"|"context"|"dialogue"="crew"): string {
  if(!npc)return `<span class="crew-portrait-fallback npc-portrait-fallback" aria-hidden="true">?</span>`;
...
function structuralCrewPortrait(npc:NpcCharacter|undefined): string { return structuralNpcPortrait(npc,"crew"); }

function npcPresentAtPort(s:GameState,npc:NpcCharacter,portId:string):boolean {
  if(npc.locationPortId===portId)return true;
  const ship=npc.shipId?s.ships[npc.shipId]:undefined;
  return ship?.dockedAtPortId===portId;
}

function playerKnowsNpc(s:GameState,npc:NpcCharacter):boolean {
  if(s.player.crew.some(member=>member.npcId===npc.id))return true;
  if(relationshipStatus(npc)!=="Neutral")return true;
  if(s.player.knowledge.some(row=>row.subjectId===npc.id))return true;
  if(npc.shipId&&s.player.shipIntel[npc.shipId]?.identified)return true;
  return s.worldEvents.some(event=>event.type==="character_conversation"&&event.participants?.includes(npc.id));
}

function npcInstitutionMatch(npc:NpcCharacter,kind:"government"|"religion"):boolean {
  const profession=npc.profession.toLowerCase();
  const role=npc.role.toLowerCase();
  if(kind==="religion")return profession==="priest"||/priest|preacher|cleric|temple|monk|religious|chaplain/.test(role);
  return profession==="naval_captain"||/governor|official|admiral|royal|court|magistrate|minister|council|harbormaster/.test(role);
}

function compactNpcContextCard(npc:NpcCharacter,buttonLabel="Speak"):string {
...
function characterManuscriptHousing(extraClass=""): string {
...
function renderCreation(): void {
  ...
  const attrs=["might","agility","perception","intellect","will","presence"] as const;
  ...
  const defaultPortrait=PORTRAIT_BY_ID[c.portraitId]; const defaultPortraitArt=defaultPortrait?ASSET_BY_ID[defaultPortrait.assetId]:undefined;
  ...
  const updateCounters=()=>{const fd=new FormData(form);const attrTotal=attrs.reduce((sum,a)=>sum+Number(fd.get(`attr_${a}`)),0);const selected=fd.getAll("coreSkill").length;const attrNode=form.querySelector("#attr-total");const skillNode=form.querySelector("#skill-total");if(attrNode)attrNode.textContent=`Attributes ${attrTotal} / 36`;if(skillNode)skillNode.textContent=`Skills ${selected} / 5`;};
  const syncHomeSettlement=()=>{const homeland=String((form.elements.namedItem("homelandRegion") as HTMLSelectElement).value);const select=form.elements.namedItem("homeSettlementId") as HTMLSelectElement;let firstVisible:HTMLOptionElement|undefined;let selectedVisible=false;for(const option of Array.from(select.options)){const visible=option.dataset.region===homeland;option.hidden=!visible;option.disabled=!visible;if(visible&&!firstVisible)firstVisible=option;if(visible&&option.selected)selectedVisible=true;}if(!selectedVisible&&firstVisible)select.value=firstVisible.value;};
  const syncPortraitPreview=()=>{const selected=form.querySelector<HTMLInputElement>('input[name="portraitId"]:checked');const node=document.querySelector<HTMLElement>("#creator-portrait-preview");if(!node)return;const caption=node.querySelector<HTMLElement>("#creator-identity-caption");const captionHtml=caption?.outerHTML??`<div class="creator-identity-caption" id="creator-identity-caption" aria-label="Selected homeland culture and faith"></div>`;if(!selected||selected.value==="portrait.pending.regional"){node.innerHTML=`<div class="portrait-placeholder">Regional portrait unavailable</div>${captionHtml}`;return;}const portrait=PORTRAIT_BY_ID[selected.value];const art=portrait?ASSET_BY_ID[portrait.assetId]:undefined;node.innerHTML=(art?.path?`<img src="${art.path}" alt="${esc(art.displayName)}">`:`<div class="portrait-placeholder">Portrait unavailable</div>`)+captionHtml;};
  const syncIdentityPreview=()=>{
    const choices=creationFromForm(form,true);
    const caption=document.querySelector<HTMLElement>("#creator-identity-caption");
    ...
    if(caption)caption.innerHTML=`<span><small>Homeland</small><b>${esc(homeland)}</b></span><span><small>Culture</small><b>${esc(culture)}</b></span><span><small>Faith</small><b>${esc(faith)}</b></span>`;
    const effects=creatorChoiceEffects(choices);
    const renderMini=(labels:string[])=>effects.filter(row=>labels.some(label=>row.label.startsWith(label))).map(row=>`<div><b>${esc(row.label)}</b><span>${esc(row.value)}</span></div>`).join("");
    const identityImpact=form.querySelector<HTMLElement>("#creator-identity-effects");
    const originImpact=form.querySelector<HTMLElement>("#creator-origin-effects");
    const historyImpact=form.querySelector<HTMLElement>("#creator-history-effects");
    if(identityImpact)identityImpact.innerHTML=renderMini(["Age ·","Ancestry ·"]);
    if(originImpact)originImpact.innerHTML=renderMini(["Faith ·","Home ·","Culture ·"]);
    if(historyImpact)historyImpact.innerHTML=renderMini(["Social origin ·","Background ·","Profession ·","Trait ·","Ship origin ·","Birth omen ·"]);
  };
  const updateReview=()=>{
    const node=form.querySelector<HTMLElement>("#creator-review");
    if(!node)return;
    const choices=creationFromForm(form,true);
  ...
  const rankPortraits=()=>{const fd=new FormData(form);const context={ancestry:String(fd.get("ancestry")) as CharacterCreationChoices["ancestry"],sex:String(fd.get("sex")) as CharacterCreationChoices["sex"],age:Number(fd.get("age")),homelandRegion:String(fd.get("homelandRegion")) as CharacterCreationChoices["homelandRegion"],culture:String(fd.get("culture")) as CharacterCreationChoices["culture"],religion:String(fd.get("religion")) as CharacterCreationChoices["religion"],background:String(fd.get("background")) as CharacterCreationChoices["background"],profession:String(fd.get("recentProfession")) as CharacterCreationChoices["recentProfession"]};const ranked=rankCuratedPortraits(context);const rankById=new Map(ranked.map((row,index)=>[row.portrait.portraitId,{index,row}]));const ancestryMatches=ranked.filter(row=>row.ancestryMatch).length;form.querySelectorAll<HTMLElement>("[data-portrait-id]").forEach(card=>{const row=rankById.get(card.dataset.portraitId??"");if(!row){card.style.display="none";return;}card.style.display="";card.style.order=String(row.index+1);});const pendingCard=form.querySelector<HTMLElement>("[data-pending-portrait]");if(pendingCard){pendingCard.style.display=ancestryMatches?"none":"";pendingCard.style.order="0";}const note=form.querySelector<HTMLElement>("#portrait-library-note");if(note){note.hidden=ancestryMatches>0;note.textContent=ancestryMatches?"":`No curated ${titleize(context.ancestry)} portrait library is shipped yet. The canonical Visual-DNA placeholder is recommended.`;}const selected=form.querySelector<HTMLInputElement>('input[name="portraitId"]:checked');const selectedCard=selected?.closest<HTMLElement>("[data-portrait-id]");const selectedStillVisible=Boolean(selectedCard&&selectedCard.style.display!=="none");if(!selectedStillVisible||(!ancestryMatches&&selected?.value!=="portrait.pending.regional")){const fallback=form.querySelector<HTMLInputElement>('input[name="portraitId"][value="portrait.pending.regional"]');const best=ranked[0]?form.querySelector<HTMLInputElement>(`input[name="portraitId"][value="${ranked[0].portrait.portraitId}"]`):undefined;if(!ancestryMatches&&fallback)fallback.checked=true;else if(best)best.checked=true;}syncPortraitPreview();};
  form.addEventListener("input",()=>{updateCounters();updateReview();});
  form.addEventListener("input",()=>{syncIdentityPreview();});
  const updateTrainingHelp=(choice:HTMLElement)=>{const help=form.querySelector<HTMLElement>("#creator-training-help");if(!help)return;const label=choice.dataset.trainingLabel??"Training";const description=choice.dataset.trainingDescription??"";help.innerHTML=`<span>Training notes</span><b>${esc(label)}</b><p>${esc(description)}</p>`;};
  const trainingChoiceFromEvent=(event:Event):HTMLElement|undefined=>event.target instanceof Element ? event.target.closest<HTMLElement>(".creator-training-choice")??undefined : undefined;
  form.addEventListener("pointerover",event=>{const choice=trainingChoiceFromEvent(event);if(choice)updateTrainingHelp(choice);});
  form.addEventListener("focusin",event=>{const choice=trainingChoiceFromEvent(event);if(choice)updateTrainingHelp(choice);});
  form.addEventListener("change",event=>{const target=event.target as HTMLInputElement|HTMLSelectElement|null;if(!target)return;if(target.name==="homelandRegion")syncHomeSettlement();if(["sex","ancestry","age","homelandRegion","culture","religion","background","recentProfession"].includes(target.name))rankPortraits();if(target.name==="portraitId")syncPortraitPreview();if(target.name==="culture"||target.name==="religion")syncCreatorCharacterTheme(form);updateReview();syncIdentityPreview();});
  updateCounters();syncHomeSettlement();rankPortraits();updateReview();syncIdentityPreview();syncCreatorCharacterTheme(form);setCreatorStepInDom(creatorStep);
  const generateButton=form.querySelector<HTMLButtonElement>("#generate-custom-portrait");
  generateButton?.addEventListener("click",async()=>{const status=form.querySelector<HTMLElement>("#custom-generation-status");const preview=form.querySelector<HTMLElement>("#custom-portrait-preview");const enabled=form.elements.namedItem("customPortraitEnabled") as HTMLInputElement|null;if(enabled)enabled.checked=true;const choices=creationFromForm(form,true);if(!choices.customPortrait){toast("Enable and define the custom portrait first.");return;}generateButton.disabled=true;if(status)status.textContent="Generating a lore-constrained portrait…";try{const response=await fetch("/api/portrait/generate",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({request:choices.customPortrait,character:{name:choices.name,age:choices.age,homelandRegion:choices.homelandRegion,homeSettlementId:choices.homeSettlementId,startingLocationId:choices.startingLocationId,culture:choices.culture,religion:choices.religion,profession:choices.recentProfession,socialOrigin:choices.socialOrigin}})});const data=await response.json() as {portraitId?:string;path?:string;error?:string};if(!response.ok||!data.portraitId||!data.path)throw new Error(data.error??"Portrait generation failed.");(form.elements.namedItem("customPortraitId") as HTMLInputElement).value=data.portraitId;(form.elements.namedItem("customPortraitAssetPath") as HTMLInputElement).value=data.path;if(preview){preview.hidden=false;preview.innerHTML=`<img src="${esc(data.path)}" alt="Generated custom portrait"><div><b>Generated custom portrait</b><small>${esc(data.portraitId)}</small></div>`;}const big=document.querySelector<HTMLElement>("#creator-portrait-preview");if(big){const caption=big.querySelector<HTMLElement>("#creator-identity-caption");big.innerHTML=`<img src="${esc(data.path)}" alt="Generated custom portrait">${caption?.outerHTML??`<div class="creator-identity-caption" id="creator-identity-caption" aria-label="Selected homeland culture and faith"></div>`}`;syncIdentityPreview();}if(status)status.textContent="Generated and cached locally.";}catch(error){if(status)status.textContent=error instanceof Error?error.message:"Portrait generation failed.";}finally{generateButton.disabled=false;}});
}

function creationFromForm(form: HTMLFormElement, allowUngeneratedCustom=false): CharacterCreationChoices {
  const fd = new FormData(form);
  const attributes: Attributes = { might:Number(fd.get("attr_might")), agility:Number(fd.get("attr_agility")), perception:Number(fd.get("attr_perception")), intellect:Number(fd.get("attr_intellect")), will:Number(fd.get("attr_will")), presence:Number(fd.get("attr_presence")) };
  const ancestry = String(fd.get("ancestry")) as CharacterCreationChoices["ancestry"];
  const secondary = String(fd.get("secondaryAncestry") ?? "") || undefined;
  const customEnabled = fd.get("customPortraitEnabled") === "on";
  const customPortrait: CustomPortraitRequest | undefined = customEnabled ? {
    enabled:true, ancestryPrimary:(ancestry === "mixed" ? "skeldran" : ancestry), ...(secondary ? { ancestrySecondary: secondary as NonNullable<CustomPortraitRequest["ancestrySecondary"]> } : {}),
    sex:String(fd.get("sex")) as CustomPortraitRequest["sex"], ageBand:Number(fd.get("age")) >= 38 ? "mature" : Number(fd.get("age")) <= 23 ? "young_adult" : "adult",
    homelandRegion:String(fd.get("homelandRegion")) as CharacterCreationChoices["homelandRegion"], homeSettlementId:String(fd.get("homeSettlementId")), startingLocationId:String(fd.get("startingLocationId")), culture:String(fd.get("culture")) as CharacterCreationChoices["culture"], religion:String(fd.get("religion")) as CharacterCreationChoices["religion"], profession:String(fd.get("recentProfession")), socialOrigin:String(fd.get("socialOrigin")),
  ...
  const generatedPortraitId=String(fd.get("customPortraitId")??""); const generatedPortraitPath=String(fd.get("customPortraitAssetPath")??"");
  if(customEnabled && !allowUngeneratedCustom && (!generatedPortraitId||!generatedPortraitPath)) throw new Error("Generate the custom portrait before entering the world, or turn off the custom portrait option.");
  return {
    name:String(fd.get("name") ?? "").trim(), age:Number(fd.get("age")), sex:String(fd.get("sex")) as CharacterCreationChoices["sex"], ancestry, ...(secondary ? { secondaryAncestry:secondary as NonNullable<CharacterCreationChoices["secondaryAncestry"]> } : {}), homelandRegion:String(fd.get("homelandRegion")) as CharacterCreationChoices["homelandRegion"], homeSettlementId:String(fd.get("homeSettlementId")), startingLocationId:String(fd.get("startingLocationId")), culture:String(fd.get("culture")) as CharacterCreationChoices["culture"], socialOrigin:String(fd.get("socialOrigin")) as CharacterCreationChoices["socialOrigin"], background:String(fd.get("background")) as CharacterCreationChoices["background"], religion:String(fd.get("religion")) as CharacterCreationChoices["religion"], devotion:String(fd.get("devotion")) as CharacterCreationChoices["devotion"], attributes, coreSkills:fd.getAll("coreSkill").map(String) as SkillId[], trait:String(fd.get("trait")) as CharacterCreationChoices["trait"], birthOmen:String(fd.get("birthOmen")) as CharacterCreationChoices["birthOmen"], recentProfession:String(fd.get("recentProfession")) as CharacterCreationChoices["recentProfession"], shipOrigin:String(fd.get("shipOrigin")) as CharacterCreationChoices["shipOrigin"], startingAttunement:0, portraitId:customEnabled&&generatedPortraitId?generatedPortraitId:String(fd.get("portraitId") ?? "portrait.pending.regional"), ...(customPortrait ? { customPortrait } : {}), ...(customEnabled&&generatedPortraitPath?{customPortraitAssetPath:generatedPortraitPath}:{})
  };
}
...
function renderNav(): string {
  const items: Array<[TabId,string]> = [];
  if (state?.player.currentPortId) items.push(["town", PORT_BY_ID[state.player.currentPortId]?.name ?? "Port"]);
  if (state?.player.currentPoiId) items.push(["poi", POI_BY_ID[state.player.currentPoiId]?.name ?? "Point of Interest"]);
...
function portArtPath(portId: string): string | undefined {
...
function poiArtPath(poiId: string): string | undefined {
...
function statusIcon(path: string, label: string): string { return `<img class="mini-status-icon" src="${path}" alt="${esc(label)}">`; }
function structuralCharacterIconSlot(kind: "attribute" | "skill", key: string): string {
  return `<span class="character-icon-slot ${kind}-icon-slot" data-${kind}-icon-slot="${esc(key)}" aria-hidden="true"></span>`;
}
function shipStatusIconSlot(path: string | undefined, label: string): string {
...
const SHIP_REFIT_ICON_PATHS: Record<string,string> = {
...
function shipRefitIcon(id: string, name: string): string {
  const path=SHIP_REFIT_ICON_PATHS[id];
  if(path)return `<img src="${esc(path)}" alt="${esc(name)} fitting icon">`;
  const initials=name.split(/\s+/).slice(0,2).map(word=>word.charAt(0)).join("");
  return `<span aria-hidden="true">${esc(initials)}</span>`;
}
function knowledgeStatusIcon(truthStatus: "unknown"|"confirmed"|"disproved", hardRumor: boolean): string {
  if (truthStatus === "confirmed") return "/art/ui/ability-library/Knowledge_Legality/01_Known.png";
  if (truthStatus === "disproved") return "/art/ui/ability-library/Knowledge_Legality/03_Uncertain.png";
...
function meter(label: string, value: number, max: number): string { return `<div class="meter"><div class="row between"><span>${esc(label)}</span><span>${value}/${max}</span></div><div class="meter-track"><i style="width:${pct(max ? value / max * 100 : 0)}"></i></div></div>`; }


function portReturnBar(portId: string, currentLabel: string): string {
  const port = PORT_BY_ID[portId];
...
function settlementHubTitle(portId: string): string {
  const port = PORT_BY_ID[portId];
  if (!port) return "Port";
  const canon = CANON_WORLD_LOCATION_BY_ID[portId];
  if (canon?.category === "major_capital_great_port") return `Capital of ${port.name}`;
  if (canon?.category === "sacred_city_fortress") return `City of ${port.name}`;
  if (canon?.category === "city_regional_port") return `City of ${port.name}`;
  return `Port of ${port.name}`;
}

function renderTown(s: GameState): string {
...
function learningSourceKindLabel(kind:LearningSourceQuote["source"]["kind"]):string {
...
function renderLearningSourcePanel(s:GameState,kinds:Array<LearningSourceQuote["source"]["kind"]>,title="Instruction & Study",venue?:LearningSourceQuote["source"]["venue"]):string {
...
function renderWorldCauseNotices(s:GameState,portId:string,kind:"government"|"religion"):string {
  const notices=activeWorldCauses(s).filter(cause=>{
    if(!cause.publicInformation||!worldCauseAppliesToPort(cause,portId))return false;
    if(kind==="religion"&&cause.kind!=="religious_policy")return false;
...
function renderTradeAuthorityPanel(s:GameState,portId:string):string {
  const port=PORT_BY_ID[portId]; if(!port)return "";
  const power=politicalPowerForRegion(port.region);
...
function renderInstitution(s: GameState, kind: "government" | "religion"): string {
  const portId = s.player.currentPortId!;
  const port = PORT_BY_ID[portId]!;
  const title = kind === "government" ? port.governmentName : port.religionName;
  const sceneKind: ContextLocationSceneKind = kind === "government" ? "royal_palace" : "temple";
...
function renderVoyageReport(report: VoyageReport | undefined): string {
  if(!report)return "";
...
function renderArrival(s: GameState): string {
...
function renderShip(s: GameState): string {
  ...
  if (shipPanelTab === "work" && portId) {
    const yardRefits=refits.slice(0,4).map(([id,refit])=>{
      ...
      const quote=quoteRefitAtPort(s,id as keyof typeof SHIP_REFITS);
  ...
  type ShipStatusRow=[string,string,string,string?];
  const repairQuick=(label:string)=>portId&&repairQuote?.ok?`<button class="ship-status-quick-action" data-action="repair-ship" title="Repair eligible ship damage · ${repairQuote.cost} cr" aria-label="Repair ${esc(label)} and other eligible ship damage for ${repairQuote.cost} crowns">+</button>`:"";
  ...
  const vesselStatus:ShipStatusRow[]=[
  ...
  if(ship.systems.flooding>0)vesselStatus.push(["/art/ui/ability-library/Ship_Damage/04_Flooding.png","Flooding",String(ship.systems.flooding),repairQuick("flooding")]);
  if(ship.systems.fire>0)vesselStatus.push(["/art/ui/ability-library/Ship_Damage/05_Fire.png","Fire",String(ship.systems.fire),repairQuick("fire damage")]);
  const companyStatus:ShipStatusRow[]=[
  ...
  const storesStatus:ShipStatusRow[]=[
  ...
  const refitCards=refits.slice(0,8).map(([id,refit])=>{
    const installed=ship.refits.includes(id);
    const quote=portId?quoteRefitAtPort(s,id as keyof typeof SHIP_REFITS):undefined;
...
function renderSpecializationStatus(s:GameState):string {
  ...
  if(band==="Neutral" && strain<20 && interference.severity==="none") return "";
  const notes:string[]=[];
  if(strain>=20) notes.push(`Arcane strain: ${titleize(strainBand(strain))}`);
  if(interference.severity!=="none") notes.push(`Current ship compatibility: ${titleize(interference.severity)}`);
...
function renderAdvancementPanel(s:GameState):string {
  const c=s.player.character; const a=c.advancement; const currentFloor=experienceThresholdForLevel(a.level); const next=experienceThresholdForLevel(a.level+1); const span=Math.max(1,next-currentFloor); const progress=Math.max(0,Math.min(100,((a.lifeExperience-currentFloor)/span)*100));
...
function characterPanelTabs(owner: "captain" | "crew", active: CharacterPanelTab): string {
...
function renderCaptainSummary(s: GameState): string {
  const captain=s.player.character;
...
function renderCaptainOverview(s: GameState): string {
  const captain=s.player.character;
  ...
  const attributeRows=(Object.entries(captain.attributes) as Array<[keyof Attributes,number]>).map(([id,value])=>`<div class="attribute-stat has-tooltip" ${tip(ATTRIBUTE_DESCRIPTIONS[id])}><span class="attribute-name">${structuralCharacterIconSlot("attribute",id)}<span>${titleize(id)}</span></span><b class="attribute-value">${value}</b></div>`).join("");
  ...
  const legalPriority:Record<string,number>={clear:0,watched:1,wanted:2,outlawed:3};
  const knownFactionIds=[...new Set([...reputationEntries.map(([key])=>key),...legalFactionIds])].sort((a,b)=>{
    const aLegal=legalStateForFaction(s,a); const bLegal=legalStateForFaction(s,b);
    const legalDelta=(legalPriority[bLegal.status]??0)-(legalPriority[aLegal.status]??0);
    if(legalDelta)return legalDelta;
  ...
  const knownPorts=s.player.knownPortIds.map(id=>PORT_BY_ID[id]).filter((port):port is NonNullable<typeof port>=>Boolean(port)).sort((a,b)=>{const current=Number(b.id===s.player.currentPortId)-Number(a.id===s.player.currentPortId);return current||Math.abs(portStanding(s,b.id))-Math.abs(portStanding(s,a.id))||a.name.localeCompare(b.name);});
  ...
  const accordion=(label:string,title:string,primary:string,secondary:string,body:string,open:boolean,extraClass="")=>`<details class="record-accordion ${extraClass}" ${open?"open":""}><summary class="record-accordion-summary"><span class="record-accordion-heading"><small>${esc(label)}</small><strong>${esc(title)}</strong></span><span class="record-accordion-meta"><b>${esc(primary)}</b><small>${esc(secondary)}</small></span></summary><div class="record-accordion-body">${body}</div></details>`;
...
function renderCaptainGear(s: GameState): string {
...
function renderInventory(s: GameState): string {
  const portId=s.player.currentPortId;
  ...
  if (captainPanelTab === "gear") return `<section class="fixed-screen art-fixed-screen captain-gear-screen">${toolbar}${renderCaptainGear(s)}</section>`;
  if(captainPanelTab === "outfitter" && portId){const stock=availableItemDefinitionsAtSettlement(portId);return `<section class="fixed-screen utility-screen outfitter-structural-screen">${toolbar}<div class="utility-scroll-host"><div class="outfitter-structural-shell"><div class="outfitter-heading"><div><div class="eyebrow">${esc(PORT_BY_ID[portId]?.name??"Port")} outfitter</div><h2 class="section-title">Equipment Available</h2></div><p class="small muted">Ordinary weapons, protection, and working gear currently offered in port.</p></div><div class="outfitter-stock-grid">${stock.map(def=>{const price=itemPurchasePriceAtSettlement(def.id,portId)??def.baseValue;const art=ASSET_BY_ID[def.artAssetId??def.id];return `<article class="outfitter-stock-item"><div class="outfitter-item-icon">${art?.path?`<img src="${esc(art.path)}" alt="${esc(def.name)}">`:`<span aria-hidden="true">${esc(def.name.charAt(0))}</span>`}</div><div class="outfitter-item-copy"><div class="outfitter-item-title"><div><small>${esc(titleize(def.category))}</small><h3>${esc(def.name)}</h3></div><b>${price} cr</b></div><p>${esc(def.description)}</p><button class="btn small" data-action="buy-item" data-id="${esc(def.id)}">Purchase</button></div></article>`;}).join("")||`<p class="muted">No ordinary outfitter stock is available here.</p>`}</div></div></div></section>`;}
...
function renderCrewInspector(s:GameState,npcId:string):string {
  const npc=s.npcs[npcId];if(!npc)return"";
  ...
  const attributes=(Object.entries(npc.attributes) as Array<[keyof Attributes,number]>).map(([id,value])=>`<div class="stat-row has-tooltip character-stat-row" ${tip(ATTRIBUTE_DESCRIPTIONS[id])}><span>${structuralCharacterIconSlot("attribute",id)}<span>${titleize(id)}</span></span><b>${value}</b></div>`).join("");
  ...
  if(crewPanelTab==="gear")return equipment?`<div class="fixed-art-content">${renderArtFirstEquipmentScreen(s,npcId,COMPANION_EQUIPMENT_SLOTS)}</div>`:`<div class="panel"><p class="muted">No personal equipment state is available for this crew member.</p></div>`;
...
function renderCrew(s: GameState): string {
  const ship=getPlayerShip(s)!;
...
function renderTavern(s: GameState): string {
  const portId = s.player.currentPortId!;
  const port = PORT_BY_ID[portId]!;
  const ship=getPlayerShip(s)!;
  const community=ensureCrewCommunity(ship);
...
function renderSimulationInspector(s: GameState): string {
...
function devInterfaceEnabled(): boolean {
...
function renderJournal(s: GameState): string {
  ...
  const learnedEventIds=new Set(s.player.knowledge.map(record=>record.sourceEventId).filter((id):id is string=>Boolean(id)));
  const history=s.worldEvents.filter(event=>!event.type.startsWith("world_cause_")||learnedEventIds.has(event.id)).reverse();
  const tabs:JournalTab[]=["knowledge","contacts","obligations","law","history"];
  const labels:Record<JournalTab,string>={knowledge:"What I Know",contacts:"Contacts",obligations:"Promises & Work",law:"Standing & Law",history:"What Happened"};
  ...
  const asEntries=()=>{
    if(journalTab==="knowledge") return knowledge.map(k=>`<article class="journal-production-entry"><img src="${knowledgeStatusIcon(k.truthStatus,k.hardRumor)}" alt=""><div><b>${esc(k.text)}</b><small>${esc(knowledgeConfidenceBand(k,s.absoluteHour))} · ${esc(k.source)}</small></div></article>`);
    if(journalTab==="contacts") return contacts.map(intel=>{const contactMark=intel.identified?shipIdentityMark(s,intel.shipId):undefined;return `<article class="journal-production-entry">${contactMark?identityMarkImage(contactMark,"journal-contact-symbol"):""}<div><b>${esc(intel.identified?intel.name??intel.shipId:"Unidentified contact")}</b><small>${titleize(intelFreshnessLabel(s,intel.shipId))} · ${esc(intel.source)} · last seen ${esc(formatClock(clockFromAbsoluteHour(intel.lastKnownAtHour)))}</small></div></article>`;});
    if(journalTab==="obligations") return contracts.map(c=>`<article class="journal-production-entry"><div><b>${titleize(c.status)} · ${c.quantity} ${esc(COMMODITY_BY_ID[c.commodityId]?.name)}</b><small>${esc(PORT_BY_ID[c.sourcePortId]?.name)} → ${esc(PORT_BY_ID[c.destinationPortId]?.name)} · ${c.reward} cr · due ${esc(formatClock(clockFromAbsoluteHour(c.deadlineHour)))}</small></div></article>`);
    if(journalTab==="law") {
      ...
      const ports=s.player.knownPortIds.map(portId=>PORT_BY_ID[portId]).filter((port):port is NonNullable<typeof port>=>Boolean(port)).map(port=>`<article class="journal-production-entry"><div><b>Port · ${esc(port.name)}</b><small>${esc(standingLabel(portStanding(s,port.id)))} local standing</small></div></article>`);
...
function pathPoints(points: GridPoint[]): string { return points.map((point) => `${point.x + 0.5},${point.y + 0.5}`).join(" "); }
function formatVoyageHours(hours:number):string {
  const rounded=Math.max(1,Math.ceil(hours)); const days=Math.floor(rounded/24); const rest=rounded%24;
  return days ? `${days}d${rest?` ${rest}h`:""}` : `${rest}h`;
}
function initialCameraWidth(s:GameState):number {
...
function ensureMapCamera(s:GameState):NavigationCameraState {
  const ship=getPlayerShip(s)!;
  if(!mapCameraState){
...
function applyMapCameraToDom():void {
  if(!mapCameraState)return;
  const svg=document.querySelector<SVGSVGElement>("svg.chart-v04"); if(!svg)return;
  const box=cameraViewBox(mapCameraState);
  svg.setAttribute("viewBox",`${box.x} ${box.y} ${box.width} ${box.height}`);
  const lod=cameraLod(box.width); svg.dataset.mapLod=lod;
  document.querySelectorAll<SVGImageElement>("[data-regional-map-layer]").forEach(node=>node.style.opacity=String(regionalLayerOpacity(box.width)));
  const readout=document.querySelector<HTMLElement>("[data-camera-readout]");
  if(readout)readout.textContent=`${GLOBAL_ATLAS.cellScaleNm} nm / cell · ${Math.round(box.width)}×${Math.round(box.height)} · ${lod==="far"?"Strategic":lod==="medium"?"Regional":lod==="navigation"?"Navigation":"Close"}`;
}
function scheduleMapCameraFrame():void {
  if(mapCameraFrame!==undefined)return;
  const tick=()=>{
    mapCameraFrame=undefined; if(!mapCameraState)return;
    const c=mapCameraState; const damping=NAV_CAMERA.damping;
    c.x+=(c.targetX-c.x)*damping; c.y+=(c.targetY-c.y)*damping; c.viewWidth+=(c.targetViewWidth-c.viewWidth)*damping;
    const center=clampCameraCenter({x:c.x,y:c.y},c.viewWidth); c.x=center.x;c.y=center.y;
    applyMapCameraToDom();
    if(Math.abs(c.targetX-c.x)>.002||Math.abs(c.targetY-c.y)>.002||Math.abs(c.targetViewWidth-c.viewWidth)>.002) mapCameraFrame=requestAnimationFrame(tick);
    else { c.x=c.targetX;c.y=c.targetY;c.viewWidth=c.targetViewWidth;applyMapCameraToDom(); }
  };
  mapCameraFrame=requestAnimationFrame(tick);
}
function centerMapOnPlayer(s:GameState):void {
  const ship=getPlayerShip(s)!; const c=ensureMapCamera(s); const center=clampCameraCenter(ship.position,c.targetViewWidth); c.targetX=center.x;c.targetY=center.y; scheduleMapCameraFrame();
}
function setMapZoom(direction:"in"|"out"):void {
  if(!state)return; const c=ensureMapCamera(state); const factor=direction==="in" ? .78 : 1.28; const width=clampViewWidth(c.targetViewWidth*factor); const center=clampCameraCenter({x:c.targetX,y:c.targetY},width); c.targetX=center.x;c.targetY=center.y;c.targetViewWidth=width; state.settings.navigationZoom=legacyZoomBand(width); scheduleMapCameraFrame();
}
function routeHazardSummary(path:GridPoint[]):string {
  const hazards=new Set(path.flatMap(point=>getWorldCell(point).hazards));
  if(hazards.has("grounding"))return "Reef / grounding risk along the route";
  if(hazards.has("shoal_water"))return "Shoal and coastal water along the route";
  return "No known route hazards";
}
function searchResultPanel(result:SearchWatersResult|undefined):string {
  if(!result)return "";
...
function renderChart(s: GameState): string {
  ...
  const reefCells:string[]=[];for(let y=WORLD_DEVELOPED_BOUNDS.y;y<WORLD_DEVELOPED_BOUNDS.y+WORLD_DEVELOPED_BOUNDS.height;y+=1){for(let x=WORLD_DEVELOPED_BOUNDS.x;x<WORLD_DEVELOPED_BOUNDS.x+WORLD_DEVELOPED_BOUNDS.width;x+=1){if(getWorldCell({x,y}).terrain==="reef")reefCells.push(`<circle cx="${x+.25}" cy="${y+.32}" r=".07" class="reef-dot"></circle><circle cx="${x+.62}" cy="${y+.7}" r=".06" class="reef-dot"></circle>`);}}
  ...
  const targetCell=target?getWorldCell(target.point):undefined; const targetPort=target?.type==="port"?PORT_BY_ID[target.id]:undefined; const targetPortMark=targetPort?portPrimaryMark(targetPort.id):undefined; const targetPoi=target?.type==="poi"?POI_BY_ID[target.id]:undefined;
...
function combatConditionCell(label:string,value:number,max:number,concealed=false):string {
  if(concealed)return `<div class="combat-condition-cell concealed"><span>${esc(label)}</span><b>?</b><small>Unknown</small></div>`;
...
function combatShipStatus(ship:ShipEntity,concealed=false):string {
...
function captureGameScrollPositions():Map<string,ScrollPosition>{
  const snapshot=new Map<string,ScrollPosition>();
  for(const selector of PRESERVED_SCROLL_SELECTORS){
    app.querySelectorAll<HTMLElement>(selector).forEach((node,index)=>snapshot.set(`${selector}:${index}`,{top:node.scrollTop,left:node.scrollLeft}));
  }
  return snapshot;
}
function restoreGameScrollPositions(snapshot:Map<string,ScrollPosition>,viewKey:string):void{
  const apply=()=>{
    if(lastRenderedGameViewKey!==viewKey)return;
    for(const selector of PRESERVED_SCROLL_SELECTORS){
      app.querySelectorAll<HTMLElement>(selector).forEach((node,index)=>{
        const position=snapshot.get(`${selector}:${index}`); if(!position)return;
        node.scrollTop=position.top; node.scrollLeft=position.left;
      });
    }
  };
  apply();
...
function gameViewKey(s:GameState):string{
  ...
  if(tab==="inventory")return `${place}:captain:${captainPanelTab}`;
  if(tab==="crew")return `${place}:crew:${inspectedCrewNpcId??"roster"}:${crewPanelTab}`;
  if(tab==="journal")return `${place}:journal:${journalTab}`;
  return `${place}:${tab}`;
}

function renderGame(): void {
  if (!state) { renderCreation(); syncAudio(); return; }
  ...
  if (state.personalCombat) app.innerHTML=`<div class="shell">${renderTopBar(state)}${renderPersonalCombat(state)}</div>`;
    ...
    if (state.player.currentPoiId && tab === "poi") content=renderPoi(state);
    else if(state.voyage||tab==="chart"||(!state.player.currentPortId && portOnlyTab)) content=renderChart(state);
    else if(tab==="town") content=renderTown(state);
    else if(tab==="market")content=renderMarket(state);
    else if(tab==="inventory")content=renderInventory(state);
    else if(tab==="crew")content=renderCrew(state);
    else if(tab==="people")content=renderPeople(state);
    else if(tab==="tavern")content=renderTavern(state);
    else if(tab==="government")content=renderInstitution(state,"government");
    else if(tab==="religion")content=renderInstitution(state,"religion");
    else if(tab==="journal")content=renderJournal(state);
    else content=renderShip(state);
  ...
  if(scrollSnapshot)restoreGameScrollPositions(scrollSnapshot,viewKey);
}
...
function resetRuntimeForNewVoyage(): void {
  state=undefined;
  dialogueLines=[];
...
function publishLauncherReadiness(): void {
...
function respondToLauncher(detail: LauncherRequestDetail, ok:boolean, message?:string): void {
  const hasSave=hasLocalSave();
  document.documentElement.dataset.etHasLocalSave=hasSave?"true":"false";
  const result:LauncherResultDetail={requestId:detail.requestId,mode:detail.mode,ok,hasLocalSave:hasSave,...(message?{message}:{})};
  window.dispatchEvent(new CustomEvent<LauncherResultDetail>(LAUNCHER_RESULT_EVENT,{detail:result}));
}

window.addEventListener(LAUNCHER_REQUEST_EVENT,(event)=>{
  const detail=(event as CustomEvent<LauncherRequestDetail>).detail;
  if(!detail || typeof detail.requestId!=="string" || (detail.mode!=="new" && detail.mode!=="continue"))return;
  if(detail.mode==="new"){
...
function handleClick(target: Element):void {
  const actionEl=target.closest<Element>("[data-action]"); const tabEl=target.closest<Element>("[data-tab]"); const combatEl=target.closest<Element>("[data-combat]"); const personalEl=target.closest<Element>("[data-personal]");
  ...
  if(action==="continue-save"){ const result=continueLocalCampaign(); toast(result.message); return; }
  if(!state)return; const s=state;
  switch(action){
    case "save": saveLocal(s); cue("page"); toast("Campaign saved with global-grid position, plotted navigation state, world history, crew, injuries, equipment and intelligence."); return;
    case "new-game": if(confirm("Start a new campaign? Your current local save remains until you save over it."))resetRuntimeForNewVoyage(); return;
    case "toggle-audio": s.settings.audioEnabled=!s.settings.audioEnabled; audio.configure(s.settings.audioEnabled,s.settings.masterVolume); if(s.settings.audioEnabled){void audio.unlock();cue("bell");} renderGame(); return;
    case "volume-down": s.settings.masterVolume=Math.max(0,Math.round((s.settings.masterVolume-0.08)*100)/100); audio.configure(s.settings.audioEnabled,s.settings.masterVolume); cue("ui"); renderGame(); return;
    case "volume-up": s.settings.masterVolume=Math.min(1,Math.round((s.settings.masterVolume+0.08)*100)/100); audio.configure(s.settings.audioEnabled,s.settings.masterVolume); cue("ui"); renderGame(); return;
    case "trade-quantity": { const id=d(actionEl,"id"); const delta=Number(d(actionEl,"delta"))||0; const input=app.querySelector<HTMLInputElement>(`[data-market-quantity="${CSS.escape(id)}"]`); const current=Math.max(1,Math.floor(Number(input?.value)||marketTradeQuantities[id]||1)); const next=Math.max(1,Math.min(999,current+delta)); marketTradeQuantities[id]=next; if(input)input.value=String(next); cue("ui"); return; }
    case "trade": { const id=d(actionEl,"id"); const input=actionEl.closest("td")?.querySelector<HTMLInputElement>("[data-market-quantity]"); const quantity=Math.max(1,Math.min(999,Math.floor(Number(input?.value)||marketTradeQuantities[id]||1))); marketTradeQuantities[id]=quantity; handleResult(transact(s,id,quantity,d(actionEl,"dir")==="sell"?"sell":"buy"),"coin"); return; }
    case "smuggle-trade": { const id=d(actionEl,"id"); const input=actionEl.closest("td")?.querySelector<HTMLInputElement>("[data-market-quantity]"); const quantity=Math.max(1,Math.min(999,Math.floor(Number(input?.value)||marketTradeQuantities[id]||1))); marketTradeQuantities[id]=quantity; handleResult(attemptSmuggledTransaction(s,id,quantity,d(actionEl,"dir")==="sell"?"sell":"buy"),"coin"); return; }
    case "obtain-customs-permit": handleResult(obtainCustomsPermit(s),"page"); return;
    case "obtain-letter-marque": handleResult(obtainLetterOfMarque(s),"page"); return;
    case "customs-present": handleResult(presentCustomsPapers(s),"page"); return;
    case "customs-conceal": handleResult(concealFromCustoms(s),"page"); return;
    case "accept-contract": handleResult(acceptContract(s,d(actionEl,"id")),"page"); return;
    case "fulfill-contract": handleResult(fulfillContract(s,d(actionEl,"id")),"coin"); return;
    case "buy-supplies": handleResult(buySupplies(s),"coin"); return;
    case "repair-ship": handleResult(repairShip(s),"repair"); return;
    case "install-refit": handleResult(installRefit(s,d(actionEl,"id") as keyof typeof SHIP_REFITS),"repair"); return;
    case "set-character-subtab": { const owner=d(actionEl,"owner"); const view=(d(actionEl,"view") || "sheet") as CharacterPanelTab; if(owner === "crew") crewPanelTab=view; else captainPanelTab=view; cue("ui"); renderGame(); return; }
    case "set-inventory-filter": { const owner=(d(actionEl,"owner") || "player") as InventoryOwnerId; const filter=(d(actionEl,"filter") || "all") as InventoryFilter; inventoryFilters[owner]=filter; inventoryPages[owner]=0; selectedInventoryOwnerId=owner; selectedInventoryItemId=undefined; cue("ui"); renderGame(); return; }
    case "inventory-page": { const owner=(d(actionEl,"owner")||"player") as InventoryOwnerId; inventoryPages[owner]=Math.max(0,(inventoryPages[owner]??0)+(Number(d(actionEl,"dir"))||0)); selectedInventoryOwnerId=owner; selectedInventoryItemId=undefined; cue("page"); renderGame(); return; }
    case "inspect-item": selectedInventoryOwnerId=(d(actionEl,"owner") || "player") as InventoryOwnerId; selectedInventoryItemId=d(actionEl,"id") || undefined; cue("ui"); renderGame(); return;
    case "inspect-slot": cue("ui"); toast("Select an item in the inventory grid, then click a highlighted compatible slot or use the Equip button."); return;
    case "equip-item": { selectedInventoryOwnerId=(d(actionEl,"owner") || "player") as InventoryOwnerId; selectedInventoryItemId=d(actionEl,"id") || undefined; handleResult(equipItem(s,d(actionEl,"id"),selectedInventoryOwnerId, (d(actionEl,"slot") || undefined) as EquipmentSlot | undefined),"ui"); return; }
    case "quick-equip-slot": { selectedInventoryOwnerId=(d(actionEl,"owner") || "player") as InventoryOwnerId; const itemId=d(actionEl,"id") || selectedInventoryItemId || ""; if(!itemId){toast("Select an item first."); return;} selectedInventoryItemId=itemId; handleResult(equipItem(s,itemId,selectedInventoryOwnerId,(d(actionEl,"slot") || undefined) as EquipmentSlot | undefined),"ui"); return; }
    case "unequip-slot": { selectedInventoryOwnerId=(d(actionEl,"owner") || "player") as InventoryOwnerId; handleResult(unequipItem(s,(d(actionEl,"slot") || "mainHand") as EquipmentSlot, selectedInventoryOwnerId),"ui"); return; }
    case "buy-item": handleResult(purchaseItem(s,d(actionEl,"id")),"coin"); return;
    case "use-ability": handleResult(usePlayerAbility(s,d(actionEl,"id")),"ui"); return;
    case "focus-skill": handleResult(spendDevelopmentPointOnSkill(s,d(actionEl,"id") as SkillId),"page"); return;
    case "take-perk": handleResult(takeGeneralPerk(s,d(actionEl,"id")),"page"); return;
    case "ship-subtab": shipPanelTab=(d(actionEl,"view")==="work"?"work":"overview"); cue("ui"); renderGame(); return;
    case "market-page": marketPage=Math.max(0,marketPage+(Number(d(actionEl,"dir"))||0)); cue("page"); renderGame(); return;
    case "crew-page": crewPage=Math.max(0,crewPage+(Number(d(actionEl,"dir"))||0)); cue("page"); renderGame(); return;
    case "crew-back": inspectedCrewNpcId=undefined; activeDialogueNpcId=undefined; crewPanelTab="sheet"; cue("page"); renderGame(); return;
    case "journal-tab": journalTab=(d(actionEl,"tab-id")||"knowledge") as JournalTab; journalPage=0; cue("page"); renderGame(); return;
    case "journal-page": journalPage=Math.max(0,journalPage+(Number(d(actionEl,"dir"))||0)); cue("page"); renderGame(); return;
    case "inspect-crew": inspectedCrewNpcId=d(actionEl,"id"); crewPanelTab="sheet"; selectedInventoryOwnerId=inspectedCrewNpcId; selectedInventoryItemId=undefined; inventoryFilters[inspectedCrewNpcId] ??= "all"; inventoryPages[inspectedCrewNpcId]??=0; tab="crew"; cue("page"); renderGame(); return;
    case "dev-advance-world": { const hours=Math.max(1,Number(d(actionEl,"hours"))||24); if(s.voyage||s.encounter||s.personalCombat){toast("Resolve the active player voyage/encounter before accelerated simulation.");return;} advanceWorld(s,hours); cue("page"); toast(`Advanced the persistent world ${hours} hours.`); renderGame(); return; }
    case "treat-injuries": handleResult(treatInjuries(s),"ui"); return;
    case "settle-local-warrant": handleResult(satisfyActiveWarrant(s,d(actionEl,"jurisdiction-id")),"coin"); return;
    case "recruit": { tab="tavern"; const desertion=checkTavernDesertion(s); cue("bell"); renderGame(); if(desertion.left>0)toast(desertion.message); return; }
    case "hire-crew": handleResult(recruitCrewOffer(s,d(actionEl,"id")),"coin"); return;
    case "share-prize": { const before=crewReactionSnapshot(s); handleCrewResult(s,before,sharePrizeWithCrew(s),"prize_share","coin"); return; }
    case "rest-crew": { const before=crewReactionSnapshot(s); handleCrewResult(s,before,restCrew(s),"shore_leave","bell"); return; }
    case "gather-rumor": handleResult(gatherRumor(s),"page"); return;
    case "assess-port": handleResult(assessPort(s),"page"); return;
    case "study-learning-source": handleResult(studyLearningSource(s,d(actionEl,"id")),"page"); return;
    case "participate-religion": handleResult(participateInReligiousLife(s),"bell"); return;
    case "deck-drill": handleResult(beginDeckDrill(s),"blade"); return;
    case "select-port": selectedMapTarget=navigationTargetForPort(d(actionEl,"id")); lastSearchWatersResult=undefined; cue("page"); renderGame(); return;
    case "select-poi": selectedMapTarget=navigationTargetForPoi(d(actionEl,"id")); lastSearchWatersResult=undefined; cue("page"); renderGame(); return;
    case "map-center": centerMapOnPlayer(s); cue("ui"); return;
    ...
    case "begin-navigation": {
      ...
      if(!result.ok){toast(result.message);renderGame();return;}
    ...
    case "continue-voyage": { if(!s.voyage){toast("No voyage is active.");return;} runVoyageUntilAttention(s); return; }
    case "cancel-voyage": { const result=cancelVoyage(s); if(result.ok){mapCameraState=undefined;lastSearchWatersResult=undefined;tab="chart";} handleResult(result,"sail"); return; }
    case "search-waters": { const result=searchWaters(s); lastSearchWatersResult=result; if(result.ok)cue(result.kind==="ship"?"bell":"page"); toast(result.message); renderGame(); return; }
    case "port-action": { const id=d(actionEl,"id") as PortActionId; delete s.arrival; if(id==="harbor"){shipPanelTab="work";tab="ship";} else tab=id === "town" ? "town" : id as TabId; const desertion=id==="tavern"?checkTavernDesertion(s):undefined; cue(id==="tavern"?"bell":id==="market"?"coin":"ui"); renderGame(); if(desertion&&desertion.left>0)toast(desertion.message); return; }
    case "poi-action": { const id=d(actionEl,"id") as PoiActionId; const poi=s.player.currentPoiId ? POI_BY_ID[s.player.currentPoiId] : undefined; delete s.arrival; poiFocusAction=id; tab="poi"; if(poi){s.worldEvents.push({id:`event.poi_action.${poi.id}.${s.absoluteHour}.${s.worldEvents.length}`,type:"poi_action",atHour:s.absoluteHour,locationId:poi.id,participants:[s.player.character.id,s.player.shipId],summary:`${s.player.character.name}: ${poi.arrivalActions.find((a)=>a.id===id)?.label ?? titleize(id)} at ${poi.name}.`,canonicalData:{poiId:poi.id,action:id},importance:0});} cue(id==="salvage"?"repair":"ui"); renderGame(); return; }
    case "leave-poi": delete s.arrival; tab="chart"; mapCameraState=undefined; cue("sail"); renderGame(); return;
    case "leave-port": delete s.arrival; tab="chart"; cue("sail"); renderGame(); return;
    case "observe": handleResult(observeEncounter(s),"ui"); return;
...
app.addEventListener("click",(event)=>{
  ...
  if(suppressNextMapClick&&target.closest("svg.chart-v04")){event.preventDefault();event.stopPropagation();suppressNextMapClick=false;return;}
  handleClick(target);
});

app.addEventListener("pointerdown",(event)=>{
  const target=event.target; if(!(target instanceof Element)||event.button!==0||!state)return;
  const chart=target.closest<SVGSVGElement>("svg.chart-v04"); if(!chart)return;
...
app.addEventListener("pointermove",(event)=>{
  if(!mapDragState||event.pointerId!==mapDragState.pointerId||!state)return;
  const chart=document.querySelector<SVGSVGElement>("svg.chart-v04"); if(!chart)return;
  const dx=event.clientX-mapDragState.startClientX; const dy=event.clientY-mapDragState.startClientY;
  if(!mapDragState.dragging&&Math.hypot(dx,dy)<NAV_CAMERA.dragThresholdPx)return;
  ...
  const rect=chart.getBoundingClientRect(); if(rect.width<=0||rect.height<=0)return;
  const c=ensureMapCamera(state);
  const width=c.targetViewWidth; const height=viewHeightForWidth(width);
  ...
  c.targetX=center.x;c.targetY=center.y; scheduleMapCameraFrame();
});
function finishMapDrag(event:PointerEvent):void {
  if(!mapDragState||event.pointerId!==mapDragState.pointerId)return;
  const chart=document.querySelector<SVGSVGElement>("svg.chart-v04");
  if(mapDragState.dragging){suppressNextMapClick=true;window.setTimeout(()=>{suppressNextMapClick=false;},0);}
  chart?.classList.remove("is-dragging"); try{chart?.releasePointerCapture?.(event.pointerId);}catch{}
  mapDragState=undefined;
}
app.addEventListener("pointerup",finishMapDrag);
app.addEventListener("pointercancel",finishMapDrag);
app.addEventListener("wheel",(event)=>{
  const target=event.target; if(!(target instanceof Element)||!state)return;
  const chart=target.closest<SVGSVGElement>("svg.chart-v04"); if(!chart)return;
  event.preventDefault();
  const rect=chart.getBoundingClientRect(); if(rect.width<=0||rect.height<=0)return;
  const fx=Math.max(0,Math.min(1,(event.clientX-rect.left)/rect.width)); const fy=Math.max(0,Math.min(1,(event.clientY-rect.top)/rect.height));
  const c=ensureMapCamera(state); const box=cameraViewBox({x:c.targetX,y:c.targetY,viewWidth:c.targetViewWidth});
...
window.addEventListener("keydown",(event)=>{
  ...
  const active=document.activeElement; if(active instanceof HTMLInputElement||active instanceof HTMLTextAreaElement||active instanceof HTMLSelectElement||(active instanceof HTMLElement&&active.isContentEditable))return;
  const key=event.key.toLowerCase();
  if(key==="+"||key==="="){event.preventDefault();setMapZoom("in");return;}
  if(key==="-"||key==="_"){event.preventDefault();setMapZoom("out");return;}
  if(key==="0"){event.preventDefault();centerMapOnPlayer(state);return;}
  const c=ensureMapCamera(state); const step=c.targetViewWidth*.12; let dx=0,dy=0;
  if(key==="arrowleft"||key==="a")dx=-step; else if(key==="arrowright"||key==="d")dx=step; else if(key==="arrowup"||key==="w")dy=-step; else if(key==="arrowdown"||key==="s")dy=step; else return;
  event.preventDefault(); const center=panCameraTarget(c,dx,dy);c.targetX=center.x;c.targetY=center.y;scheduleMapCameraFrame();
});
window.addEventListener("pointerdown",()=>{void audio.unlock();},{once:true});
installArtCalibrationShortcut((enabled)=>{ if(state) renderGame(); else renderCreation(); toast(`Art calibration mode ${enabled?"enabled":"disabled"}. ${enabled?"Drag regions to move, shift-drag or use the corner handle to resize, then copy normalized values.":"Production presentation restored."}`); });
installReferenceGhostShortcut((enabled)=>{ if(state) renderGame(); else renderCreation(); toast(`Reference Ghost Mode ${enabled?"enabled":"disabled"}. ${enabled?"Use opacity, Live / Both / Reference, or Blink to compare the approved design against the runtime screen.":"Live presentation restored."}`); });
renderCreation();
