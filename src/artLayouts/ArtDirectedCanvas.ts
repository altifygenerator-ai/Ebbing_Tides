import type { ArtDirectedCanvasOptions, ArtDirectedLayer, ArtLayoutManifest, ArtLayoutRegion } from "./types.js";
...
export function normalizedRegionStyle(region: ArtLayoutRegion): string {
  return `left:${region.x * 100}%;top:${region.y * 100}%;width:${region.width * 100}%;height:${region.height * 100}%;`;
}

export function artCalibrationEnabled(): boolean {
  if (typeof window === "undefined") return false;
  const params = new URLSearchParams(window.location.search);
  if (params.get("artCalibration") === "1") return true;
  if (params.get("artCalibration") === "0") return false;
  return window.sessionStorage.getItem(CALIBRATION_SESSION_KEY) === "1";
}

export function setArtCalibrationEnabled(enabled: boolean): void {
  if (typeof window === "undefined") return;
  window.sessionStorage.setItem(CALIBRATION_SESSION_KEY, enabled ? "1" : "0");
}

export function installArtCalibrationShortcut(onToggle: (enabled: boolean) => void): () => void {
...
function debugRegionMarkup(region: ArtLayoutRegion): string {
...
function layerMarkup(manifest: ArtLayoutManifest, layer: ArtDirectedLayer): string {
...
export function renderArtDirectedCanvas(options: ArtDirectedCanvasOptions): string {
...
function readWorkingRegion(node: HTMLElement): WorkingRegion {
  return {
...
function applyWorkingRegion(node: HTMLElement, region: WorkingRegion): void {
  ...
  const label = node.querySelector<HTMLElement>(".art-calibration-region-label");
  if (label) {
...
function initializeCalibrationCanvas(canvas: HTMLElement): () => void {
  ...
  const renderedNode = canvas.querySelector<HTMLElement>("[data-art-rendered]");
  const scaleNode = canvas.querySelector<HTMLElement>("[data-art-scale]");
  const cursorNode = canvas.querySelector<HTMLElement>("[data-art-cursor]");
  const selectedNode = canvas.querySelector<HTMLElement>("[data-art-selected]");
  const copyButton = canvas.querySelector<HTMLButtonElement>("[data-art-copy-region]");
  const dimensionStatus = canvas.querySelector<HTMLElement>("[data-art-dimension-status]");
  const baseImage = canvas.querySelector<HTMLImageElement>("[data-art-base-image]");
  let selected: HTMLElement | undefined;

  const updateSize = () => {
    const rect = canvas.getBoundingClientRect();
    if (renderedNode) renderedNode.textContent = `${Math.round(rect.width)} × ${Math.round(rect.height)}`;
    if (scaleNode) scaleNode.textContent = `${(rect.width / logicalWidth).toFixed(4)}x`;
  };
  updateSize();
  ...
  const moveCursor = (event: PointerEvent) => {
    ...
    if (cursorNode) cursorNode.textContent = `x ${x.toFixed(4)} · y ${y.toFixed(4)}`;
  };
  canvas.addEventListener("pointermove", moveCursor);

  const regionNodes = Array.from(canvas.querySelectorAll<HTMLElement>(".art-calibration-region"));
  ...
  for (const node of regionNodes) {
    const down = (event: PointerEvent) => {
      ...
      if (selectedNode) selectedNode.textContent = node.dataset.artRegionId ?? "region";
  ...
  const copy = async () => {
    ...
    try { await navigator.clipboard.writeText(text); } catch { /* Clipboard access can be unavailable in local dev contexts. */ }
  };
  ...
  const validateDimensions = () => {
    ...
    if (!matches) console.error(`[Ebbing Tides] Art layout ${canvas.dataset.artLayoutId ?? "unknown"} expected ${nativeWidth}×${nativeHeight} but loaded ${baseImage.naturalWidth}×${baseImage.naturalHeight}.`);
  };
  if (baseImage?.complete) validateDimensions();
...
function validateProductionCanvas(canvas: HTMLElement): () => void {
  const nativeWidth = Number(canvas.dataset.artNativeWidth ?? 1);
  const nativeHeight = Number(canvas.dataset.artNativeHeight ?? 1);
  const baseImage = canvas.querySelector<HTMLImageElement>("[data-art-base-image]");
  const validate = () => {
  ...
  if (baseImage?.complete) validate();
  baseImage?.addEventListener("load", validate);
  return () => baseImage?.removeEventListener("load", validate);
}

let calibrationCleanups: Array<() => void> = [];

export function initializeArtDirectedCanvases(root: ParentNode = document): void {
  calibrationCleanups.forEach((cleanup) => cleanup());
  calibrationCleanups = [];
  for (const canvas of Array.from(root.querySelectorAll<HTMLElement>(".art-directed-canvas"))) {
