...
import type { Attributes, CharacterCreationChoices, EquipmentSlot, GameState, SkillId } from "../game/types.js";
...
function normalizeEquipment(raw: unknown, slots: EquipmentSlot[]): Record<EquipmentSlot, string | undefined> {
  const next = defaultEquipmentState(slots);
  if (!raw || typeof raw !== "object") return next;
  for (const [key, value] of Object.entries(raw as Record<string, unknown>)) {
    const slot = slots.includes(key as EquipmentSlot) ? key as EquipmentSlot : LEGACY_EQUIPMENT_SLOT_MAP[key];
...
function migrateV1ToV2(parsed: any): any {
  ...
  const find = (definitionId: string) => inventory.find((item) => item.definitionId === definitionId)?.id;
  ...
  for (const ship of Object.values(parsed.ships ?? {}) as any[]) ship.refits ??= [];
  parsed.settings.audioEnabled ??= true;
  parsed.settings.masterVolume ??= 0.32;
  return parsed;
}

function migrateV2ToV3(parsed: any): GameState {
  parsed.schemaVersion = 3;
  // Docked ships now occupy navigable harbor approach cells rather than painted land markers.
  for (const ship of Object.values(parsed.ships ?? {}) as any[]) {
    if (ship.dockedAtPortId) { const port = PORT_BY_ID[ship.dockedAtPortId]; if (port) ship.position = { ...port.approachPoint }; }
  }
...
function nearestNavigable(point: { x: number; y: number }): { x: number; y: number } {
  const base = { x: Math.round(point.x), y: Math.round(point.y) };
  if (getWorldCell(base).navigable) return base;
  for (let radius = 1; radius <= 8; radius += 1) {
    for (let dy = -radius; dy <= radius; dy += 1) {
      for (let dx = -radius; dx <= radius; dx += 1) {
        if (Math.max(Math.abs(dx), Math.abs(dy)) !== radius) continue;
        const candidate = { x: base.x + dx, y: base.y + dy };
        if (getWorldCell(candidate).navigable) return candidate;
      }
    }
  }
...
function remapV03Point(point: { x: number; y: number }): { x: number; y: number } {
...
function migrateV3ToV4(parsed: any): GameState {
  ...
  for (const ship of Object.values(parsed.ships ?? {}) as any[]) {
    if (ship.dockedAtPortId) {
      ...
      try { ship.position = routePoint(ship.route.fromPortId, ship.route.toPortId, ship.route.progress ?? 0); }
      catch { ship.position = remapV03Point(ship.position ?? { x: 24, y: 18 }); }
  ...
  if (parsed.voyage) {
    ...
    let target;
    if (parsed.voyage.destination?.type === "port") target = navigationTargetForPort(parsed.voyage.destination.id);
    else if (parsed.voyage.destination?.type === "sea") target = navigationTargetForSea(remapV03Point(parsed.voyage.destination.point));
    if (!target) { delete parsed.voyage; }
    else {
      const path = findSeaPath(start, target.point);
      if (path.length < 2) delete parsed.voyage;
      else {
        ...
        if (ship) ship.position = { ...path[idx] };
      }
    }
  }
...
function migrateV4ToV5(parsed: any): GameState {
  ...
  for (const [oldId,value] of Object.entries(old.skills ?? {})) { const mapped=skillMap[oldId]; if(!mapped)continue; const n=Number(value); capability.skills[mapped]=Math.max(capability.skills[mapped], n <= 10 ? Math.round(n*10) : Math.round(n)); }
  ...
  parsed.settings.audioEnabled ??= true; parsed.settings.masterVolume ??= .32; parsed.settings.firstUseTips ??= "minimal"; parsed.settings.journalMode ??= "advanced";
  for (const ship of Object.values(parsed.ships ?? {}) as any[]) { ship.systemTags ??= ["wooden_hull","sailing_rig","naval_battery","damage_control_kit"]; ship.attunementLoad ??= {arcane:0,industrial:ship.classId?.includes("industrial")?18:3,sensitivity:ship.classId?.includes("industrial")?.8:.25,mitigationTags:[]}; }
  const migratedNpcs:any = {};
  for (const npc of Object.values(parsed.npcs ?? {}) as any[]) {
    const seed = { id:npc.id, name:npc.name, age:Number(npc.age ?? 35), sex:npc.sex ?? "male", ancestry:npc.ancestry ?? "skeldran", homelandRegion:npc.homelandRegion ?? "skeldra", culture:npc.culture ?? "skeldran", religion:npc.religion ?? "unaffiliated", profession:npc.profession ?? npc.role, role:npc.role ?? npc.profession ?? "resident", socialTier:npc.socialTier ?? 2, ...(npc.locationPortId?{locationPortId:npc.locationPortId}:{}), ...(npc.shipId?{shipId:npc.shipId}:{}), personality:npc.personality ?? {}, values:npc.values ?? {}, goals:(npc.goals ?? []).map((g:any)=>typeof g === "string"?g:g.description).filter(Boolean), beliefs:npc.beliefs ?? [], knownFacts:npc.knownFacts ?? [], speakingStyle:npc.speakingStyle ?? "Plainspoken.", relationshipToPlayer:npc.relationshipToPlayer ?? {trust:0,respect:0,fear:0,affection:0,suspicion:0} };
    const next=createNpcFromSeed(seed); if(npc.relationshipToPlayer)next.relationshipToPlayer={...next.relationshipToPlayer,...npc.relationshipToPlayer}; migratedNpcs[next.id]=next;
  }
  parsed.npcs=migratedNpcs;
  const state=parsed as GameState;
  for(const npc of Object.values(state.npcs))ensureNpcPlan(state,npc);
  return state;
}


function migrateV5ToV6(parsed: any): GameState {
  ...
  for (const npc of Object.values(parsed.npcs ?? {}) as any[]) npc.advancement ??= startingAdvancement(Math.min(10, startingLevelFromAge(Number(npc.age ?? 35)) + (Number(npc.socialTier ?? 2) >= 3 ? 1 : 0)));
  return parsed as GameState;
}

function migrateV6ToV7(parsed: any): GameState {
  parsed.player ??= {};
  parsed.player.character ??= {};
...
function migrateV7ToV8(parsed: any): GameState {
...
function bestSavedSeamanship(parsed: any): number {
  let best = Number(parsed.player?.character?.skills?.seamanship ?? 50);
  for (const assignment of parsed.player?.crew ?? []) {
    if (!assignment?.npcId || !["first_mate","navigator"].includes(String(assignment.role))) continue;
    const rating = Number(parsed.npcs?.[assignment.npcId]?.skills?.seamanship ?? 0);
    if (Number.isFinite(rating)) best = Math.max(best, rating);
  }
  return Math.max(0, Math.min(100, best));
}

function migrateV8ToV9(parsed: any): GameState {
  // 0.6C Physical Distance Hotfix 1: geometry stays fixed while miles/knots and exact combat range become persistent truth.
  for (const ship of Object.values(parsed.ships ?? {}) as any[]) {
    if (typeof ship?.classId === "string") ship.classId = resolveShipClassId(ship.classId);
  ...
  for (const [portId,seedMarket] of Object.entries(registryMarkets)) {
    ...
    for (const [commodityId,row] of Object.entries(seedMarket.goods)) parsed.markets[portId].goods[commodityId] ??= structuredClone(row);
  }
  ...
  for (const npc of Object.values(parsed.npcs ?? {}) as any[]) {
    ...
    if (!plan || plan.type !== "travel" || !plan.path?.length || !ship) continue;
...
function migrateV9ToV10(parsed: any): GameState {
  parsed.player ??= {};
  parsed.player.inventory ??= [];
  parsed.player.equipment = normalizeEquipment(parsed.player.equipment, PLAYER_EQUIPMENT_SLOTS);
  for (const npc of Object.values(parsed.npcs ?? {}) as any[]) {
    if (npc?.inventory || npc?.equipment) npc.equipment = normalizeEquipment(npc.equipment, COMPANION_EQUIPMENT_SLOTS);
  }
...
function migrateV10ToV11(parsed: any): GameState {
  const playerShip = parsed.ships?.[parsed.player?.shipId];
  if (playerShip) {
  ...
  parsed.player ??= {};
  const legacyOfficerIds: Record<string,string> = {
  ...
  parsed.player.crew = (parsed.player.crew ?? []).filter((member:any)=>Boolean(member?.npcId));
  for (const member of parsed.player.crew) { member.health ??= 100; }
  parsed.schemaVersion = 11;
  return parsed as GameState;
}

function migrateV11ToV12(parsed: any): GameState {
  parsed.player ??= {};
  parsed.player.character ??= {};
  parsed.player.character.reputation ??= {};
  parsed.player.portStanding ??= Object.fromEntries((parsed.player.knownPortIds ?? []).map((id:string)=>[id,0]));
  for (const portId of parsed.player.knownPortIds ?? []) parsed.player.portStanding[portId] ??= 0;
  parsed.player.legal ??= {};
...
export function migrateSaveData(parsed: any): GameState {
  if (parsed.schemaVersion === 1) parsed = migrateV1ToV2(parsed);
  ...
  if (parsed.schemaVersion === 8) parsed = migrateV8ToV9(parsed);
  if (parsed.schemaVersion === 9) parsed = migrateV9ToV10(parsed);
  if (parsed.schemaVersion === 10) parsed = migrateV10ToV11(parsed);
  if (parsed.schemaVersion === 11) parsed = migrateV11ToV12(parsed);
  if (parsed.schemaVersion === 12) {
    ...
    for (const npc of Object.values(state.npcs ?? {})) { normalizeArcaneStrain(npc); normalizeProgressionCadence(npc,state.absoluteHour); }
    compactSimulationEvents(state);
    for (const npc of Object.values(parsed.npcs ?? {}) as any[]) ensureNpcPlan(state, npc);
    compactSimulationEvents(state);
    return state;
  }
