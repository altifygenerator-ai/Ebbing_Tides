...
import type {
  AvailabilityBand,
  CanonicalContentDefinition,
  MarketType,
  RegionalAvailability,
  SettlementAvailability,
  SettlementEconomicProfile,
  ShipClassDefinition,
  ShipyardCapabilityProfile
...
export const REGIONS: RegionId[] = ["skeldra", "asteria", "serath", "kaishin", "crossroads", "outer_isles"];

interface RegionDoctrine {
  regionId: RegionId;
  primaryIndustries: string[];
  exports: string[];
  imports: string[];
  dominantTags: string[];
  costlyImportTags: string[];
  marketTypes: MarketType[];
  shipbuildingLevel: 0 | 1 | 2 | 3 | 4;
  repairCapability: 0 | 1 | 2 | 3 | 4;
  medicalCapability: 0 | 1 | 2 | 3 | 4;
  militarySupplyLevel: 0 | 1 | 2 | 3 | 4;
  arcaneServices: 0 | 1 | 2 | 3 | 4;
  industrialServices: 0 | 1 | 2 | 3 | 4;
  luxuryAvailability: 0 | 1 | 2 | 3 | 4;
  smugglingAvailability: 0 | 1 | 2 | 3 | 4;
  religiousGoods: 0 | 1 | 2 | 3 | 4;
}

export const REGION_ECONOMIC_DOCTRINES: Record<RegionId, RegionDoctrine> = {
  skeldra: {
    regionId:"skeldra",
...
const REGION_SHIP_CLASSES: Record<RegionId, EntityId[]> = Object.fromEntries(
  REGIONS.map((regionId) => [regionId, SHIP_CLASS_DEFINITIONS.filter((row) => row.region === regionId || row.region === "common").map((row) => row.id)])
) as Record<RegionId, EntityId[]>;

const s = (name:string) => SHIP_CLASS_DEFINITIONS.find((row) => row.name === name)!.id;
const EMPTY_YARD: ShipyardCapabilityProfile = { builds:[], commonlySells:[], sometimesSells:[], imports:[], repairs:0, refits:0, specialistCapabilities:[] };

function genericYard(regionId: RegionId, shipbuildingLevel: 0|1|2|3|4, repairCapability:0|1|2|3|4): ShipyardCapabilityProfile {
  const regional = REGION_SHIP_CLASSES[regionId];
  const builds = regional.filter((id) => {
    const role = SHIP_CLASS_BY_ID[id]?.role;
    if (shipbuildingLevel <= 1) return role === "workboat" || role === "coastal" || role === "coastal_trader" || role === "coastal_merchant";
    if (shipbuildingLevel === 2) return !["capital_warship","capital_arcane","industrial_special","industrial_test"].includes(role ?? "");
    if (shipbuildingLevel === 3) return role !== "capital_warship" && role !== "capital_arcane";
    return true;
  });
  return { builds, commonlySells:builds.slice(0,4), sometimesSells:builds.slice(4), imports:[], repairs:repairCapability, refits:Math.min(4,repairCapability) as 0|1|2|3|4, specialistCapabilities:[] };
}

const SETTLEMENT_OVERRIDES: Partial<Record<EntityId, Partial<SettlementEconomicProfile>>> = {
  "port.veyrholm": {
...
function profileForSettlement(settlementId: EntityId): SettlementEconomicProfile {
  const settlement = CANON_WORLD_LOCATION_BY_ID[settlementId];
  if (!settlement || settlement.kind !== "settlement") throw new Error(`Unknown canonical settlement ${settlementId}`);
  ...
  const shipbuildingLevel = (isMajor ? Math.max(3,base.shipbuildingLevel) : isFortress ? Math.min(base.shipbuildingLevel,2) : Math.min(base.shipbuildingLevel,3)) as 0|1|2|3|4;
  const repairCapability = (isMajor ? Math.max(3,base.repairCapability) : Math.min(base.repairCapability,3)) as 0|1|2|3|4;
  const generic: SettlementEconomicProfile = {
    settlementId,
    regionId,
    ...
    militarySupplyLevel:(isMajor ? Math.max(3,base.militarySupplyLevel) : base.militarySupplyLevel) as 0|1|2|3|4,
    ...
    religiousGoods:(isFortress ? Math.max(3,base.religiousGoods) : base.religiousGoods) as 0|1|2|3|4,
...
export const SETTLEMENT_ECONOMIC_PROFILES: SettlementEconomicProfile[] = CANON_SETTLEMENTS.map((row) => profileForSettlement(row.id));
export const SETTLEMENT_ECONOMIC_PROFILE_BY_ID = Object.fromEntries(SETTLEMENT_ECONOMIC_PROFILES.map((row) => [row.settlementId,row])) as Record<EntityId,SettlementEconomicProfile>;

function tagsFor(def: CanonicalContentDefinition): string[] {
  return [def.category,def.subcategory ?? "",String(def.originRegion ?? ""),...def.availabilityTags,...def.gameplayRoles].map((v)=>v.toLowerCase());
}

const REGIONAL_COMMODITY_LOCAL_NAMES: Record<RegionId, string[]> = {
...
const REGIONAL_COMMODITY_IMPORT_NAMES: Partial<Record<RegionId, string[]>> = {
...
function commodityRegionalSignal(definition: CanonicalContentDefinition, regionId: RegionId): "local" | "import" | undefined {
  if (definition.category !== "commodity") return undefined;
  const name = definition.name.toLowerCase();
  if (REGIONAL_COMMODITY_LOCAL_NAMES[regionId].some((entry) => entry === name)) return "local";
  if ((REGIONAL_COMMODITY_IMPORT_NAMES[regionId] ?? []).some((entry) => entry === name)) return "import";
  return undefined;
}

export function regionalAvailabilityFor(definition: CanonicalContentDefinition, regionId: RegionId): RegionalAvailability {
  ...
  let commonality: AvailabilityBand = commoditySignal === "import" ? "rare_import" : local ? "local_specialty" : commonOrigin ? "common" : dominant ? "common" : costly ? "rare_import" : "occasional";
  ...
  let importDependency: RegionalAvailability["importDependency"] = commoditySignal === "import" ? "high" : local ? "none" : commonOrigin ? "low" : commonality === "rare_import" ? "high" : "moderate";
...
export const REGIONAL_AVAILABILITY: RegionalAvailability[] = REGIONS.flatMap((regionId) => CONTENT_DEFINITIONS.map((def) => regionalAvailabilityFor(def,regionId)));

function settlementSource(def: CanonicalContentDefinition, profile: SettlementEconomicProfile, regional: RegionalAvailability): SettlementAvailability["source"] {
  if (regional.manufacturedLocally) return "local";
  if (def.category === "religious_object" && profile.religiousGoods >= 3) return "religious";
  if (profile.regionId === "outer_isles" && def.originRegion !== "common") return def.legalStatus === "contraband" ? "smuggled" : "captured";
  if (def.legalStatus === "military_only" && profile.militarySupplyLevel >= 3) return "state";
  return regional.importDependency === "none" || regional.importDependency === "low" ? "regional" : "import";
}

export function settlementAvailabilityFor(definition: CanonicalContentDefinition, settlementId: EntityId): SettlementAvailability {
  const profile = SETTLEMENT_ECONOMIC_PROFILE_BY_ID[settlementId];
  if (!profile) throw new Error(`Unknown settlement profile ${settlementId}`);
  const regional = regionalAvailabilityFor(definition,profile.regionId);
  let availability = regional.commonality;
  const restrictions:string[] = [];
  if (definition.rarity === "unique" || definition.uniqueMarketStock) availability = "unavailable";
  if (definition.legalStatus === "military_only" && profile.militarySupplyLevel < 3) { availability = "restricted"; restrictions.push("military supply authorization required"); }
  if (definition.legalStatus === "restricted" || definition.legalStatus === "licensed") restrictions.push(`${definition.legalStatus} item`);
  if (definition.legalStatus === "contraband" && profile.smugglingAvailability < 2) { availability = "unavailable"; restrictions.push("not ordinary legal stock"); }
  if (definition.legalStatus === "sacred" && profile.religiousGoods < 2) { availability = "restricted"; restrictions.push("religious provenance or institutional access required"); }
  if (definition.category === "arcane_equipment" && profile.arcaneServices === 0) availability = "rare_import";
  if (definition.category === "industrial_equipment" && profile.industrialServices === 0) availability = "rare_import";
  ...
  if (definition.category === "commodity") {
    const name = definition.name.toLowerCase();
    const localByPort: Record<string,string[]> = {
    ...
    if ((localByPort[settlementId] ?? []).includes(name)) availability = "local_specialty";
  }
  const source = settlementSource(definition,profile,regional);
  const bandWeight:Record<AvailabilityBand,number>={unavailable:0,restricted:0.15,rare_import:0.25,occasional:0.55,common:0.8,local_specialty:1};
...
export const SETTLEMENT_AVAILABILITY: SettlementAvailability[] = SETTLEMENT_ECONOMIC_PROFILES.flatMap((profile) => CONTENT_DEFINITIONS.map((def) => settlementAvailabilityFor(def,profile.settlementId)));

export function marketInventoryForSettlement(settlementId: EntityId, marketType: MarketType, limit = 40): SettlementAvailability[] {
  const profile = SETTLEMENT_ECONOMIC_PROFILE_BY_ID[settlementId];
  if (!profile || !profile.marketTypes.includes(marketType)) return [];
...
export function blackMarketInventoryForSettlement(settlementId: EntityId, limit = 30): SettlementAvailability[] {
  const profile = SETTLEMENT_ECONOMIC_PROFILE_BY_ID[settlementId];
  if (!profile || profile.smugglingAvailability < 2) return [];
  return CONTENT_DEFINITIONS
    ...
    .map((def) => ({...settlementAvailabilityFor(def,settlementId), source:"smuggled" as const, availability:"occasional" as const}))
    .slice(0,limit);
}

export function shipyardClassesForSettlement(settlementId: EntityId): { builds: ShipClassDefinition[]; sells: ShipClassDefinition[]; imports: ShipClassDefinition[] } {
  const yard = SETTLEMENT_ECONOMIC_PROFILE_BY_ID[settlementId]?.shipyard ?? EMPTY_YARD;
  const rows = (ids:EntityId[]) => ids.map((id)=>SHIP_CLASS_BY_ID[id]).filter((row):row is ShipClassDefinition=>Boolean(row));
  return { builds:rows(yard.builds), sells:rows([...yard.commonlySells,...yard.sometimesSells]), imports:rows(yard.imports) };
}

export const REGIONAL_IDENTITY_SAMPLE_SETTLEMENTS: Record<RegionId,EntityId> = {
