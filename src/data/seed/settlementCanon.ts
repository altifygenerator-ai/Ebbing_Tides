...
export type CanonGeographyRegionId =
  | "skeldra"
...
export type CanonLocationCategory =
  | "major_capital_great_port"
  | "city_regional_port"
  | "sacred_city_fortress"
  | "poi_danger_geography";

export interface CanonLocationRecord {
  id: EntityId;
  name: string;
  aliases?: string[];
  canonicalRegion: CanonGeographyRegionId;
  runtimeRegion: RegionId;
  category: CanonLocationCategory;
  role: string;
  kind: "settlement" | "poi";
  playableInCurrentAlpha: boolean;
...
export function canonLocationsForRegion(region: CanonGeographyRegionId): CanonLocationRecord[] {
  return CANON_WORLD_LOCATIONS.filter((row) => row.canonicalRegion === region);
}

export function canonicalLocationName(id: EntityId): string {
