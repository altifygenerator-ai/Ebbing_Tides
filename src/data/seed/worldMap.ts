...
function inAtlas(point: GridPoint): boolean {
  return point.x >= 0 && point.y >= 0 && point.x < GLOBAL_ATLAS.width && point.y < GLOBAL_ATLAS.height;
}

function inBounds(point: GridPoint, bounds: { x: number; y: number; width: number; height: number }): boolean {
...
function developmentAt(point: GridPoint): "active" | "reserved" {
...
export function isNavigableCell(point: GridPoint): boolean {
  return getWorldCell(point).navigable;
}

export function isMappedWater(point: GridPoint): boolean {
