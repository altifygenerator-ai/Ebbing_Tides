import type { EntityId, ItemDefinition, ItemInstance } from "../../game/types.js";
...
export const ITEM_BY_ID = Object.fromEntries(ITEM_DEFINITIONS.map((item) => [item.id, item])) as Record<string, ItemDefinition>;

export function itemPurchasePriceAtSettlement(definitionId: EntityId, settlementId: EntityId): number | undefined {
  const def = ITEM_BY_ID[definitionId];
  if (!def) return undefined;
  const content = CONTENT_BY_ID[definitionId];
  if (!content) return def.baseValue;
  const availability = settlementAvailabilityFor(content,settlementId);
  if (availability.availability === "unavailable" || availability.availability === "restricted") return undefined;
  if (content.legalStatus === "contraband" || content.legalStatus === "stolen" || content.legalStatus === "military_only") return undefined;
  return Math.max(1,Math.round(def.baseValue*availability.priceModifier));
}

export function availableItemDefinitionsAtSettlement(settlementId: EntityId): ItemDefinition[] {
  ...
  return ITEM_DEFINITIONS.filter((def) => {
    ...
    if (def.id === "item.armor.naval_breastplate" && profile.militarySupplyLevel < 3) return false;
    // Licensed naval firearms need at least a working military/arms supply chain.
    if (def.category === "firearm" && profile.militarySupplyLevel < 2) return false;
    // Precision navigation instruments need a fitting, shipyard, or exchange economy.
    if (def.id === "item.tool.calibrated_sextant" && profile.shipbuildingLevel < 2 && !profile.marketTypes.includes("book_chart_seller")) return false;
    return true;
  });
}

export function startingInventory(background: string, profession: string): ItemInstance[] {
  const ids = [
  ...
  if (background === "foundry_child" || profession === "dockworker") ids.push("item.weapon.common_boarding_axe");
  if (profession === "navigator") ids.push("item.tool.calibrated_sextant");
