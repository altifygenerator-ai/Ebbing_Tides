...
export type BaselineTradeTreatment = "open" | "permit" | "prohibited";

export interface JurisdictionTradePolicyDefinition {
  jurisdictionId: EntityId;
  factionId: EntityId;
  label: string;
  authorityLabel: string;
  customsPermitCost: number;
  customsPermitHours: number;
  customsPermitDurationHours: number;
  commissionCost: number;
  commissionHours: number;
  commissionMinimumStanding: number;
  byLegalStatus: Record<string, BaselineTradeTreatment>;
}
 ...
 */
export const JURISDICTION_TRADE_POLICIES: JurisdictionTradePolicyDefinition[] = [
  {
...
export const JURISDICTION_TRADE_POLICY_BY_ID = Object.fromEntries(JURISDICTION_TRADE_POLICIES.map(row=>[row.jurisdictionId,row])) as Record<EntityId,JurisdictionTradePolicyDefinition>;
