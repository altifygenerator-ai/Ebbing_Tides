import type { Attributes, CharacterCreationChoices, SkillId, SkillSpecialization } from "./types.js";

export const ATTRIBUTE_IDS: Array<keyof Attributes> = ["might", "agility", "perception", "intellect", "will", "presence"];
...
export const SPECIALIZATION_LIBRARY: Partial<Record<SkillId, string[]>> = {
...
function add(skills: Record<SkillId, number>, id: SkillId, value: number): void {
  skills[id] = Math.max(0, Math.min(100, skills[id] + value));
}

export type StartingSkillContributionSource = "core_training" | "social_origin" | "background" | "profession" | "trait";
export interface StartingSkillContribution {
  source: StartingSkillContributionSource;
  choiceId: string;
  bonuses: Partial<Record<SkillId, number>>;
}

const SOCIAL_ORIGIN_SKILL_BONUSES: Record<CharacterCreationChoices["socialOrigin"], Partial<Record<SkillId, number>>> = {
...
const BACKGROUND_SKILL_BONUSES: Record<CharacterCreationChoices["background"], Partial<Record<SkillId, number>>> = {
...
const PROFESSION_SKILL_BONUSES: Record<CharacterCreationChoices["recentProfession"], Partial<Record<SkillId, number>>> = {
...
const TRAIT_SKILL_BONUSES: Record<CharacterCreationChoices["trait"], Partial<Record<SkillId, number>>> = {
...
export function startingSkillContributions(choices: CharacterCreationChoices): StartingSkillContribution[] {
  const coreBonuses = Object.fromEntries(choices.coreSkills.map((skillId)=>[skillId,15])) as Partial<Record<SkillId, number>>;
  return [
...
export function buildStartingSkills(choices: CharacterCreationChoices): Record<SkillId, number> {
  ...
  const skills = Object.fromEntries(ALL_SKILLS.map((skill) => [skill, 5])) as Record<SkillId, number>;
  for (const contribution of startingSkillContributions(choices)) {
    for (const [skillId, value] of Object.entries(contribution.bonuses)) add(skills, skillId as SkillId, value ?? 0);
  }
  return skills;
}

export function buildStartingSpecializations(choices: CharacterCreationChoices): SkillSpecialization[] {
  const out: SkillSpecialization[] = [];
  const push = (skillId: SkillId, name: string, rating: number, source: string) => out.push({
    id: `spec.${skillId}.${name.toLowerCase().replace(/[^a-z0-9]+/g,"_")}`,
    skillId, name, rating, source, learnedAtHour: 0
  });
  if (choices.recentProfession === "navigator") push("navigation", "Coastal", 8, "Recent profession: navigator");
  if (choices.recentProfession === "gunner") push("gunnery", "Long-Range", 8, "Recent profession: gunner");
  if (choices.recentProfession === "apprentice_engineer" || choices.background === "foundry_child" || choices.background === "engineers_apprentice") push("engineering", "Naval Machinery", 7, "Industrial upbringing/training");
  if (choices.background === "former_naval_midshipman") push("command", "Naval Discipline", 7, "Former naval service");
  if (choices.background === "raised_among_smugglers" || choices.recentProfession === "smuggler") push("streetwise", "Smuggling", 7, "Smuggling background");
  if (choices.recentProfession === "sailor" || choices.socialOrigin === "naval_family") push("seamanship", "Heavy Weather", 6, "Northern maritime experience");
  if (choices.coreSkills.includes("arcana")) push("arcana", choices.culture === "skeldran" ? "Warding" : "Divination", 5, "Creation focus");
  return out;
}

export function validateAttributes(attributes: Attributes): boolean {
  const values = Object.values(attributes);
  if (values.some((value) => !Number.isFinite(value) || value < 1 || value > 10)) return false;
  return values.reduce((sum, value) => sum + value, 0) === 36;
}

export function skillRatingLabel(value: number): string {
  if (value < 10) return "Untrained";
  if (value < 25) return "Novice";
  if (value < 40) return "Competent";
  if (value < 60) return "Professional";
  if (value < 75) return "Veteran / Expert";
  if (value < 90) return "Elite Specialist";
