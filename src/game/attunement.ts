...
import type { AttunementLoad, AttunementState, CharacterCapabilityState } from "./types.js";

export interface InterferenceResult {
  severity: "none" | "low" | "moderate" | "high" | "extreme";
  score: number;
  symptoms: string[];
  arcaneReliabilityModifier: number;
  industrialReliabilityModifier: number;
}
 ...
 */
export function attunementBand(value: number): string {
  if (value <= -70) return "Strongly Arcane";
  if (value <= -40) return "Moderately Arcane";
  if (value <= -20) return "Slightly Arcane";
  if (value < 20) return "Neutral";
  if (value < 40) return "Slightly Industrial";
  if (value < 70) return "Moderately Industrial";
 ...
 */
export function deriveAttunementValue(character: CharacterCapabilityState): number {
  ...
  for (const learned of character.abilities) {
    if (learned.status !== "known") continue;
    const definition = ABILITY_BY_ID[learned.abilityId];
    if (!definition) continue;
    const mastery = Math.max(1, learned.mastery || 1);
    if (definition.type === "arcane_practice") arcane += 9 * mastery;
    if (definition.type === "technical_technique") industrial += 8 * mastery;
  }
...
export function refreshDerivedAttunement(character: CharacterCapabilityState): number {
  const value = deriveAttunementValue(character);
  character.attunement.value = value;
  return value;
}

export function calculateInterference(character: AttunementState, loads: AttunementLoad[]): InterferenceResult {
  ...
  const severity: InterferenceResult["severity"] = score >= 70 ? "extreme" : score >= 45 ? "high" : score >= 25 ? "moderate" : score >= 10 ? "low" : "none";
  const symptoms: string[] = [];
  if (industrialLoad && (personalArcane >= 20 || arcaneLoad >= 20)) symptoms.push("precision drift, timing noise, or pressure irregularity in sensitive machinery");
  if (arcaneLoad && (personalIndustrial >= 20 || industrialLoad >= 20)) symptoms.push("ward flicker, damped resonance, or increased Arcane strain");
  if (severity === "none") symptoms.push("no meaningful high-order interference");
  return {
    severity,
    score,
    symptoms,
...
export function applyArcaneStrain(attunement: AttunementState, amount: number): void {
 ...
 */
export function arcaneStrainReliabilityModifier(value: number): number {
  const strain=Math.max(0,Math.min(100,value));
  if (strain < 20) return 0;
  if (strain < 45) return -2;
  if (strain < 75) return -5;
  return -9;
}
 ...
 */
export function advanceArcaneStrainRecovery(character: CharacterCapabilityState, fromHour: number, toHour: number): number {
...
export function normalizeArcaneStrain(character: CharacterCapabilityState): void {
...
export function strainBand(value: number): string {
