...
import type { CapabilityLevel } from "./content.js";
import { calculatePrice } from "./economy.js";
import type { EntityId, GameState, Injury, ShipEntity } from "./types.js";

export type PortServiceKind = "repair" | "refit" | "medical";
...
export interface ServiceResourceQuote {
  ok: boolean;
  requiredUnits: number;
  availableUnits: number;
  stockRatio: number;
  lines: ServiceResourceLine[];
  procurementCost: number;
}

export interface ShipRepairQuote {
  ok: boolean;
  portId: EntityId;
  ...
  targetHull: number;
  targetSails: number;
  targetRigging: number;
  targetFire: number;
  targetFlooding: number;
  fullRestoration: boolean;
  resources: ServiceResourceQuote;
  message: string;
}

export interface MedicalTreatmentQuote {
  ok: boolean;
  portId: EntityId;
  capability: CapabilityLevel;
  capabilityLabel: string;
  maxSeverity: 0 | 1 | 2 | 3;
  treatableInjuryIds: EntityId[];
  unsupportedInjuryIds: EntityId[];
  cost: number;
  hours: number;
  resources: ServiceResourceQuote;
  message: string;
...
export interface GenericRefitServiceQuote {
  ok: boolean;
  portId: EntityId;
  capability: CapabilityLevel;
  capabilityLabel: string;
...
const YARD_CAPABILITY_LABELS: Record<CapabilityLevel,string> = {
  0:"No local yard",
  1:"Basic boat repair",
...
const MEDICAL_CAPABILITY_LABELS: Record<CapabilityLevel,string> = {
...
const REPAIR_CONDITION_CEILING: Record<CapabilityLevel,number> = { 0:0, 1:0.65, 2:0.80, 3:0.92, 4:1 };
const REPAIR_FULL_DAMAGE_THRESHOLD: Record<CapabilityLevel,number> = { 0:0, 1:0.08, 2:0.15, 3:0.30, 4:1 };
const REPAIR_LABOR_MULTIPLIER: Record<CapabilityLevel,number> = { 0:2, 1:1.60, 2:1.30, 3:1.10, 4:1 };
const REPAIR_BASE_HOURS: Record<CapabilityLevel,number> = { 0:0, 1:18, 2:12, 3:9, 4:6 };
const REFIT_PRICE_MULTIPLIER: Record<CapabilityLevel,number> = { 0:2, 1:1.55, 2:1.30, 3:1.12, 4:1 };
const REFIT_TIME_MULTIPLIER: Record<CapabilityLevel,number> = { 0:2, 1:1.75, 2:1.45, 3:1.20, 4:1 };
const MEDICAL_PRICE_MULTIPLIER: Record<CapabilityLevel,number> = { 0:2, 1:1.35, 2:1.15, 3:1, 4:0.90 };
const MEDICAL_HOURS: Record<CapabilityLevel,number> = { 0:0, 1:8, 2:6, 3:4, 4:3 };

function clamp(value:number,min:number,max:number):number { return Math.max(min,Math.min(max,value)); }
function ceilToHour(value:number):number { return Math.max(1,Math.ceil(value)); }

function resourceRowsForCategories(state:GameState,portId:EntityId,categories:Set<string>) {
 ...
 */
export function quoteServiceResources(state:GameState,portId:EntityId,categories:ReadonlySet<string>,requiredUnits:number):ServiceResourceQuote {
  const market=state.markets[portId];
  const need=Math.max(0,Math.ceil(requiredUnits));
  if(!market || need===0)return {ok:need===0,requiredUnits:need,availableUnits:0,stockRatio:0,lines:[],procurementCost:0};
  ...
  const lines:ServiceResourceLine[]=[];
...
export function consumeServiceResources(state:GameState,portId:EntityId,quote:ServiceResourceQuote):void {
  if(!quote.ok)return;
  const market=state.markets[portId];
  if(!market)return;
  for(const line of quote.lines){
    const row=market.goods[line.commodityId];
    if(row)row.stock=Math.max(0,Number((row.stock-line.quantity).toFixed(3)));
  }
}

function targetForRepair(current:number,max:number,capability:CapabilityLevel):number {
  if(current>=max || capability===0)return current;
  const missingFraction=(max-current)/Math.max(1,max);
  if(missingFraction<=REPAIR_FULL_DAMAGE_THRESHOLD[capability])return max;
  return Math.max(current,Math.floor(max*REPAIR_CONDITION_CEILING[capability]));
}

export function quoteShipRepair(state:GameState,portId:EntityId=state.player.currentPortId ?? ""):ShipRepairQuote {
  ...
  if(!resources.ok)return {ok:false,portId,capability,capabilityLabel,cost:0,hours:0,repairPoints,targetHull,targetSails,targetRigging,targetFire,targetFlooding,fullRestoration:false,resources,message:"The yard has the skill, but current material stocks cannot support this repair."};
...
export function quoteGenericRefitService(state:GameState,portId:EntityId,baseCost:number,baseHours:number,requiredCapability:CapabilityLevel,resourceUnits:number,specialistTag?:string):GenericRefitServiceQuote {
  const profile=SETTLEMENT_ECONOMIC_PROFILE_BY_ID[portId];
  const capability=profile?.shipyard.refits ?? 0;
  const capabilityLabel=YARD_CAPABILITY_LABELS[capability];
  const emptyResources:ServiceResourceQuote={ok:false,requiredUnits:Math.max(1,resourceUnits),availableUnits:0,stockRatio:0,lines:[],procurementCost:0};
  if(!profile || capability<requiredCapability)return {ok:false,portId,capability,capabilityLabel,cost:0,hours:0,resources:emptyResources,specialistMatched:false,message:`This yard is not capable of that refit. ${requiredCapability>=4?"A major specialist facility is required.":"A better-equipped shipyard is required."}`};
  ...
  if(!resources.ok)return {ok:false,portId,capability,capabilityLabel,cost:0,hours:0,resources,specialistMatched,message:"Current yard material stocks are too thin for that refit."};
...
function medicalSeverityLimit(capability:CapabilityLevel):0|1|2|3 {
  if(capability<=0)return 0;
  if(capability===1)return 1;
  if(capability===2)return 2;
  return 3;
}

function medicalEligible(injuries:Injury[],maxSeverity:0|1|2|3):{treatable:Injury[];unsupported:Injury[]} {
...
export function quoteMedicalTreatment(state:GameState,portId:EntityId=state.player.currentPortId ?? ""):MedicalTreatmentQuote {
  const profile=SETTLEMENT_ECONOMIC_PROFILE_BY_ID[portId];
  ...
  const emptyResources:ServiceResourceQuote={ok:false,requiredUnits:0,availableUnits:0,stockRatio:0,lines:[],procurementCost:0};
  if(!portId || !profile)return {ok:false,portId,capability,capabilityLabel,maxSeverity,treatableInjuryIds:[],unsupportedInjuryIds:[],cost:0,hours:0,resources:emptyResources,message:"No local medical service is available here."};
  ...
  if(!resources.ok)return {ok:false,portId,capability,capabilityLabel,maxSeverity,treatableInjuryIds:treatable.map(injury=>injury.id),unsupportedInjuryIds:unsupported.map(injury=>injury.id),cost:0,hours:0,resources,message:"Practitioners are available, but the port does not have enough medical stock for treatment."};
  const scarcityMultiplier=resources.stockRatio<0.35?1.25:resources.stockRatio<0.60?1.12:1;
...
export function portServiceSummary(state:GameState,portId:EntityId):{yard:string;medical:string;provisions:string} {
  const profile=SETTLEMENT_ECONOMIC_PROFILE_BY_ID[portId];
  const market=state.markets[portId];
  if(!profile)return {yard:"No yard data",medical:"No medical data",provisions:"No provisioning data"};
...
export function capabilityLabel(level:CapabilityLevel):string { return YARD_CAPABILITY_LABELS[clamp(level,0,4) as CapabilityLevel]; }
