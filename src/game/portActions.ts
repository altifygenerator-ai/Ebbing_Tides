...
import type { GameState, KnowledgeRecord } from "./types.js";
...
interface RumorCandidate extends KnowledgeRecord {
  /** Seed/event reports are heard once; live market reports can be refreshed as conditions change. */
  refreshable: boolean;
}

const RUMORS: Record<string, Array<Omit<RumorCandidate, "id" | "learnedAtHour" | "observedAtHour" | "refreshedAtHour">>> = {
  "port.veyrholm": [
...
const PORT_READS: Record<string, Record<string, string>> = {
  "port.veyrholm": {
...
function latestPublicPortEvent(state: GameState, portId: string) {
...
function dynamicPortReadText(state: GameState, portId: string, lensId: string): string {
  ...
  if (lensId === "trade") {
    ...
    if(useful.length) text=`${text} Right now: ${useful.join(" ")}`;
  }
  if (lensId === "maritime") {
    const event = latestPublicPortEvent(state,portId);
    if (event) text=`${text} The freshest harbor talk you can place is this: ${event.summary}`;
  }
...
function marketRumorCandidates(state: GameState, portId: string): RumorCandidate[] {
...
function publicEventRumorCandidates(state: GameState, portId: string): RumorCandidate[] {
  return state.worldEvents.flatMap((event) => {
    const profile = publicEventInformationProfile(event);
    if (!profile || !eventInformationCanReachPort(state,event,portId)) return [];
    if (event.atHour < state.absoluteHour - 336) return [];
    ...
    return [{
      ...
      informationState:"current" as const,
      hardRumor:false,
...
function authoredRumorCandidates(state: GameState, portId: string): RumorCandidate[] {
...
export function rumorCandidatesForPort(state: GameState, portId: string): RumorCandidate[] {
  ...
  const byClaim = new Map<string,RumorCandidate>();
  for (const candidate of all) if (!byClaim.has(candidate.claimKey!)) byClaim.set(candidate.claimKey!,candidate);
  return [...byClaim.values()];
}

function candidateIsUsefulNow(state: GameState, candidate: RumorCandidate): boolean {
  const existing = currentKnowledgeForClaim(state,candidate.claimKey!);
  if (!existing) return true;
  if (!candidate.refreshable) return false;
  return knowledgeNeedsRefresh(state,candidate.claimKey!,candidate.text,12);
}

export function assessPort(state: GameState): { ok: boolean; message: string } {
  const portId = state.player.currentPortId;
  if (!portId) return { ok:false, message:"You need a port around you to take its measure." };
...
export function gatherRumor(state: GameState): { ok: boolean; message: string } {
  const portId = state.player.currentPortId;
  if (!portId) return { ok:false, message:"Rumors need people, and you are at sea." };
  const useful = rumorCandidatesForPort(state,portId).filter((candidate)=>candidateIsUsefulNow(state,candidate));
  if (!useful.length) return { ok:false, message:"You have already pulled the useful talk from this port for now. Give the harbor time to change or new news to arrive." };
...
export function restCrew(state: GameState): { ok: boolean; message: string } {
...
export function recruitDeckhand(state: GameState): { ok: boolean; message: string } {
  const offer=crewRecruitOffers(state)[0];
  if(!offer)return {ok:false,message:"No sailors are available to recruit here right now."};
