...
import type { GridPoint, ShipEntity, WorldMapCell } from "./types.js";
import { routeDistanceNm, straightLineDistanceNm } from "./physicalDistance.js";

export function legacyCruiseSpeedKnots(speedRating: number): number {
  return 2.25 + 0.75 * Math.max(0, speedRating);
}

export function baseCruiseSpeedKnots(ship: ShipEntity): number {
  if (Number.isFinite(ship.cruiseSpeedKnots) && (ship.cruiseSpeedKnots ?? 0) > 0) return ship.cruiseSpeedKnots!;
  const definition = shipClassDefinition(ship.classId);
  if (definition?.cruiseSpeedKnots && definition.cruiseSpeedKnots > 0) return definition.cruiseSpeedKnots;
  return legacyCruiseSpeedKnots(ship.speed);
}

export function terrainSpeedFactor(cell: WorldMapCell): number {
  switch (cell.terrain) {
...
export function shipConditionSpeedFactor(ship: ShipEntity): number {
...
export function crewSpeedFactor(seamanshipRating = 50): number {
...
export function effectiveSpeedKnots(ship: ShipEntity, cellOrPoint: WorldMapCell | GridPoint, seamanshipRating = 50): number {
...
export function estimateRouteTravelHours(path: GridPoint[], ship: ShipEntity, seamanshipRating = 50): number {
  let hours = 0;
...
export function plannedAverageSpeedKnots(path: GridPoint[], ship: ShipEntity, seamanshipRating = 50): number {
 ...
 */
export function advanceRouteDistanceByHours(path: GridPoint[], currentDistanceNm: number, hours: number, ship: ShipEntity, seamanshipRating = 50): { distanceTravelledNm: number; hoursUsed: number } {
  ...
  let hoursUsed = 0;
  if (hoursLeft <= 0 || alongNm >= totalNm || path.length < 2) return { distanceTravelledNm: alongNm, hoursUsed };
  ...
  for (let i = 1; i < path.length && hoursLeft > 0; i += 1) {
    ...
    if (alongNm >= segmentEnd - 1e-9) continue;
...
export function estimateRemainingRouteHours(path: GridPoint[], currentDistanceNm: number, ship: ShipEntity, seamanshipRating = 50): number {
  const totalNm = routeDistanceNm(path);
  if (currentDistanceNm >= totalNm) return 0;
