...
import type { CommodityDefinition, EntityId, MarketCommodityState, PortMarketState } from "./types.js";
...
function generatedMarketRow(portId: EntityId, good: CommodityDefinition): MarketCommodityState | undefined {
  if (!good.contentDefinitionId) return undefined;
  ...
  if (definition.legalStatus === "contraband" || definition.legalStatus === "stolen" || definition.legalStatus === "military_only") return undefined;
  ...
  if (targetStock <= 0) return undefined;
...
function canonicalCargoCandidates(portId: EntityId): Array<{ good: CommodityDefinition; row: MarketCommodityState; score: number }> {
  return COMMODITIES.flatMap((good) => {
    if (LEGACY_GOOD_IDS.has(good.id)) return [];
    const row = generatedMarketRow(portId, good);
    if (!row || !good.contentDefinitionId) return [];
    const definition = CONTENT_BY_ID[good.contentDefinitionId];
...
export function buildInitialPortMarkets(): Record<string, PortMarketState> {
  const markets: Record<string, PortMarketState> = {};
  for (const portId of Object.keys(PORT_BY_ID)) {
    const legacyProfile = PORT_MARKET_PROFILES[portId] ?? {};
    const goods: Record<string, MarketCommodityState> = Object.fromEntries(Object.entries(legacyProfile).map(([commodityId,row]) => [commodityId, {
      commodityId,
      stock:row.stock,
    ...
    for (const candidate of canonicalCargoCandidates(portId)) {
      if (Object.keys(goods).length >= MAX_LIVE_CARGO_ROWS) break;
