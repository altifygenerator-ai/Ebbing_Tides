...
import type { CharacterAbility, CharacterCapabilityState, CharacterCreationChoices, CharacterKnowledgeEntry, CharacterSchematic, VisualDNA } from "./types.js";
...
function startingAbilities(choices: CharacterCreationChoices): CharacterAbility[] {
  const ids: string[] = [];
  ...
  if (choices.recentProfession === "apprentice_engineer" || choices.background === "foundry_child" || choices.background === "engineers_apprentice") ids.push("tech.naval_engineering.emergency_hull_shoring");
  if (choices.recentProfession === "navigator") ids.push("tech.instruments.calibrated_sextant_method");
  if (choices.recentProfession === "gunner" || choices.background === "former_naval_midshipman") ids.push("tech.precision_weapons.precision_bore_sighting");
...
function startingSchematics(choices: CharacterCreationChoices): CharacterSchematic[] {
  const rows: CharacterSchematic[] = [];
  if (choices.recentProfession === "apprentice_engineer" || choices.background === "engineers_apprentice") rows.push({ schematicId: "schematic.naval.pump_bypass_basic", learnedAtHour: 0, source: "Apprenticeship", access: "known" });
  return rows;
}

function startingKnowledge(choices: CharacterCreationChoices): CharacterKnowledgeEntry[] {
  const rows: CharacterKnowledgeEntry[] = [
  ...
  if (choices.religion !== "unaffiliated") rows.push({ id: `knowledge.religion.${choices.religion}`, domain: "religious", claim: "Knows the ordinary rites, institutions, and taboos of their own faith as practiced in their community.", source: "Religious upbringing", confidence: 95, learnedAtHour: 0, status: "fact" });
  return rows;
}

export function visualDnaFromChoices(choices: CharacterCreationChoices): VisualDNA {
  ...
  if (portrait) {
    ...
    if (choices.ancestry === "mixed" && choices.secondaryAncestry) dna.ancestrySecondary = choices.secondaryAncestry;
...
export function buildCapabilityState(choices: CharacterCreationChoices): CharacterCapabilityState {
  const capability: CharacterCapabilityState = {
