...
function findEncounterShip(state: GameState): ShipEntity | undefined {
...
export function beginVoyage(state: GameState, destinationPortId: string): { ok: boolean; message: string } {
  ...
  if (!from) return { ok: false, message: "You are already at sea." };
  if (!state.player.knownPortIds.includes(destinationPortId)) return { ok: false, message: "Your character does not know a usable route to that destination." };
  const route = findRoute(from, destinationPortId);
  if (!route) return { ok: false, message: "No direct route is charted in this alpha slice." };
  const ship = state.ships[state.player.shipId];
  if (!ship) return { ok: false, message: "Player ship not found." };
  if (ship.supplies < 3) return { ok: false, message: "You need more ship supplies before leaving port." };
...
export function advanceVoyage(state: GameState, hours = 4): { ok: boolean; message: string; encounter?: ActiveEncounter; arrived?: string; weather?: string } {
  ...
  if (encounter) return { ok: true, message: "Sails sighted. Voyage interrupted.", encounter, ...(weather ? { weather } : {}) };
...
export function hailEncounter(state: GameState): { ok: boolean; message: string } {
  const encounter = state.encounter;
  if (!encounter || encounter.phase !== "sighting") return { ok: false, message: "No ship is in hailing range." };
  const other = state.ships[encounter.otherShipId];
