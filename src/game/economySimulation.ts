...
import type { GameState, MarketCommodityState, NpcCharacter, PortMarketState, ShipEntity } from "./types.js";

export interface CommodityFlowRates {
  dailyProduction: number;
  dailyExternalSupply: number;
...
const TURNOVER_BY_CATEGORY: Record<string, number> = {
  food: 0.024,
  medical: 0.014,
  raw: 0.013,
  textile: 0.011,
...
function availabilityProductionFactor(portId: string, row: MarketCommodityState): { factor:number; source:string } {
  const explicit = PORT_COMMODITY_PRODUCTION_FACTORS[portId]?.[row.commodityId];
  if (explicit !== undefined) return { factor:explicit, source:"authored settlement production" };
  ...
  if (definition) {
    ...
    const factorBySource: Record<string, number> = {
      local: 0.97,
      regional: 0.56,
      import: 0.06,
      state: 0.62,
    ...
    if (availability.availability === "local_specialty") factor += 0.02;
    if (availability.availability === "rare_import") factor = Math.min(factor, 0.18);
...
export function commodityFlowRates(portId: string, row: MarketCommodityState): CommodityFlowRates {
  const good = COMMODITY_BY_ID[row.commodityId];
  ...
  if (explicit) {
    ...
    const externalBySource:Record<string,number>={local:0.02,regional:0.42,import:0.82,state:0.34,religious:0.30,captured:0.70,surplus:0.06,smuggled:0.50};
    externalFactor = externalBySource[availability.source] ?? 0.35;
    if (availability.availability === "rare_import") externalFactor = Math.min(externalFactor,0.72);
  } else {
  ...
  if (good?.category === "food") externalFactor += 0.20;
  return {
 ...
 */
export function advanceSettlementEconomy(state: GameState, beforeHour: number): void {
  ...
  for (let day = beforeDay + 1; day <= afterDay; day += 1) {
    for (const market of Object.values(state.markets)) {
      for (const row of Object.values(market.goods)) {
        ...
        if (flow.dailyConsumption <= 0 && flow.dailyProduction <= 0) continue;
...
function cargoUnitsUsed(ship: ShipEntity): number {
  return ship.cargo.reduce((sum, stack) => sum + (COMMODITY_BY_ID[stack.commodityId]?.cargoUnits ?? 1) * stack.quantity, 0);
}

function marketableCargoRows(market: PortMarketState, ship: ShipEntity) {
...
export function settleNpcCargoAtPort(state: GameState, npc: NpcCharacter, portId: string): number {
  const ship = npc.shipId ? state.ships[npc.shipId] : undefined;
  const market = state.markets[portId];
  if (!ship || !market) return 0;
  let revenue = 0;
...
function foodRowsForPort(state: GameState, portId: string) {
  const market = state.markets[portId];
  if (!market) return [];
...
export function provisionNpcAtPort(state: GameState, npc: NpcCharacter, portId: string, targetDays = 18): { foodDaysAdded:number; cost:number } {
  const market = state.markets[portId];
  if (!market) return { foodDaysAdded:0, cost:0 };
  ...
  for (const row of foodRowsForPort(state,portId)) {
    ...
    if (selfFinanced) npc.brain.needs.moneyReserve -= lineCost;
    cost += lineCost;
  ...
  if (PORT_BY_ID[portId]) npc.brain.needs.waterDays = Math.max(npc.brain.needs.waterDays, Math.min(targetDays, npc.brain.needs.foodDays));
  return { foodDaysAdded, cost };
}

function exportableUnits(row: MarketCommodityState): number {
...
export function tradeOpportunityScore(state: GameState, fromPortId: string, toPortId: string): number {
  const from = state.markets[fromPortId];
  const to = state.markets[toPortId];
  if (!from || !to || fromPortId === toPortId) return -Infinity;
  let best = 0;
  for (const row of Object.values(from.goods)) {
    const destination = to.goods[row.commodityId];
    const good = COMMODITY_BY_ID[row.commodityId];
    if (!destination || !good || good.category === "illicit") continue;
    ...
    if (margin <= 0 && needGap < 0.20) continue;
...
export function loadNpcTradeCargo(state: GameState, npc: NpcCharacter, fromPortId: string, toPortId: string): { cost:number; units:number } {
  const ship = npc.shipId ? state.ships[npc.shipId] : undefined;
  const from = state.markets[fromPortId];
  const to = state.markets[toPortId];
  if (!ship || !from || !to || !["merchant","privateer"].includes(ship.disposition)) return {cost:0,units:0};

  const candidates = Object.values(from.goods).flatMap(row => {
    const destination = to.goods[row.commodityId];
    const good = COMMODITY_BY_ID[row.commodityId];
    if (!destination || !good || good.category === "illicit") return [];
    const available = exportableUnits(row);
  ...
  for (const candidate of candidates) {
    ...
    if (freeCargoUnits < candidate.good.cargoUnits) break;
    ...
    else ship.cargo.push({commodityId:candidate.good.id,quantity});
    if (cargoUnitsUsed(ship) >= ship.cargoCapacity * 0.82) break;
  }
  return {cost,units};
}
...
export interface SupplyQuote {
  ...
  cost: number;
  marketUnits: number;
  sourceRows: Array<{commodityId:string;quantity:number;unitPrice:number}>;
  message?: string;
}
...
export function quoteShipSupplies(state: GameState, portId: string, amount = 6): SupplyQuote {
  const market = state.markets[portId];
  const requested = Math.max(1,Math.floor(amount));
  if (!market) return {ok:false,amount:requested,cost:0,marketUnits:0,sourceRows:[],message:"No provisioning market is available here."};
  let marketUnits = Math.max(1,Math.ceil(requested/6));
  let cost = 0;
  const sourceRows: SupplyQuote["sourceRows"] = [];
  for (const row of foodRowsForPort(state,portId)) {
    if (marketUnits <= 0) break;
  ...
  if (marketUnits > 0) return {ok:false,amount:requested,cost,marketUnits:Math.max(1,Math.ceil(requested/6)),sourceRows:[],message:"The harbor does not have enough staple provisions to fill that order."};
  return {ok:true,amount:requested,cost:Math.max(1,cost),marketUnits:Math.max(1,Math.ceil(requested/6)),sourceRows};
}

export function consumeSupplyQuote(state: GameState, portId: string, quote: SupplyQuote): void {
  const market = state.markets[portId];
  if (!market || !quote.ok) return;
  for (const source of quote.sourceRows) {
    const row = market.goods[source.commodityId];
    if (row) row.stock = Math.max(0,row.stock-source.quantity);
  }
}
...
export function currentEconomicFlowSnapshot(state: GameState, portId: string) {
  const market = state.markets[portId];
  if (!market) return [];
