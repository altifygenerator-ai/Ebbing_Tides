...
export function rollInt(seed: string, key: string, min: number, max: number): number {
  return min + Math.floor(deterministicUnit(seed, key) * (max - min + 1));
}

export function roll2d10(seed: string, key: string): { dice: [number, number]; total: number } {
  const a = rollInt(seed, `${key}:a`, 1, 10);
  const b = rollInt(seed, `${key}:b`, 1, 10);
  return { dice: [a, b], total: a + b };
}

export function pickWeighted<T>(seed: string, key: string, options: Array<{ value: T; weight: number }>): T {
  const total = options.reduce((sum, option) => sum + Math.max(0, option.weight), 0);
  if (total <= 0 || options.length === 0) throw new Error("Weighted pick requires positive weights");
  let target = deterministicUnit(seed, key) * total;
  for (const option of options) {
    target -= Math.max(0, option.weight);
    if (target <= 0) return option.value;
  }
