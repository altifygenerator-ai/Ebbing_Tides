...
import type { CharacterKnowledgeEntry, GameState, KnowledgeRecord, WorldEvent } from "./types.js";

export type KnowledgeFreshness = "current" | "aging" | "stale" | "contradicted" | "superseded";

const CATEGORY_DEFAULT_STALE_HOURS: Partial<Record<KnowledgeRecord["category"], number>> = {
  trade: 72,
  danger: 120,
  maritime: 120,
  political: 336,
...
function categoryForLegacyDomain(domain: CharacterKnowledgeEntry["domain"]): KnowledgeRecord["category"] {
  switch (domain) {
...
function truthForLegacyStatus(status: CharacterKnowledgeEntry["status"]): KnowledgeRecord["truthStatus"] {
  if (status === "fact") return "confirmed";
  if (status === "contradiction") return "disproved";
  return "unknown";
}

function legacyKnowledgeRecord(entry: CharacterKnowledgeEntry): KnowledgeRecord {
...
function claimKeyForLegacyRuntimeRecord(record: KnowledgeRecord): string {
  if (record.id.startsWith("knowledge.port_read.")) return record.id.slice("knowledge.".length);
  if (record.source === "Direct observation" && record.truthStatus === "confirmed" && record.subjectId && record.category === "maritime") return `ship.identity.${record.subjectId}`;
  return record.id;
}

export function knowledgeClaimKey(record: KnowledgeRecord): string {
  return record.claimKey ?? record.id;
}

export function knowledgeReferenceHour(record: KnowledgeRecord): number {
  return record.refreshedAtHour ?? record.observedAtHour ?? record.learnedAtHour;
}

export function knowledgeFreshnessAtHour(record: KnowledgeRecord, absoluteHour: number): KnowledgeFreshness {
  if (record.informationState === "contradicted") return "contradicted";
  if (record.informationState === "superseded") return "superseded";
  if (record.staleAfterHours === undefined) return "current";
  const age = Math.max(0, absoluteHour - knowledgeReferenceHour(record));
  if (age >= record.staleAfterHours) return "stale";
  if (age >= Math.max(1, Math.floor(record.staleAfterHours * 0.55))) return "aging";
  return "current";
}

export function currentKnowledgeForClaim(state: GameState, claimKey: string): KnowledgeRecord | undefined {
...
export function knowledgeNeedsRefresh(state: GameState, claimKey: string, nextText?: string, minimumChangeRefreshHours = 12): boolean {
  ...
  if (freshness === "stale" || freshness === "contradicted" || freshness === "superseded") return true;
 ...
 */
export function upsertPlayerKnowledge(state: GameState, incoming: KnowledgeRecord): { record: KnowledgeRecord; created: boolean; changed: boolean } {
  normalizeKnowledgeOwnership(state);
  ...
  const refreshed: KnowledgeRecord = {
    ...previous,
    ...incoming,
    id: previous.id,
    claimKey,
...
export function contradictPlayerKnowledge(state: GameState, claimKey: string): boolean {
...
export function supersedePlayerKnowledge(state: GameState, claimKey: string): boolean {
  const record = currentKnowledgeForClaim(state, claimKey);
  if (!record) return false;
 ...
 */
export function normalizeKnowledgeOwnership(state: GameState): void {
  ...
  for (const record of state.player.knowledge) {
    ...
    if (record.staleAfterHours === undefined && record.truthStatus === "unknown") {
      ...
      if (fallback !== undefined) record.staleAfterHours = fallback;
    }
  }
  ...
  if (legacy.length) {
    for (const entry of legacy) {
      ...
      if (!existing) state.player.knowledge.push(converted);
        ...
        if (converted.staleAfterHours === undefined) delete existing.staleAfterHours;
      }
    }
  ...
  const compacted: KnowledgeRecord[] = [];
  ...
  if (compacted.length !== state.player.knowledge.length) state.player.knowledge = compacted;
}

/** A rumor or old report is not enough to identify a ship on sight forever. */
export function knowledgeSupportsRecognition(state: GameState, subjectId: string): boolean {
  normalizeKnowledgeOwnership(state);
  return state.player.knowledge.some((record) => {
    ...
    if (freshness === "contradicted" || freshness === "superseded") return false;
    if (record.truthStatus === "confirmed") return true;
 ...
 */
export function informationTravelHours(fromPortId: string, toPortId: string): number | undefined {
  if (fromPortId === toPortId) return 0;
...
export function eventInformationCanReachPort(state: GameState, event: WorldEvent, targetPortId: string): boolean {
  if (!event.locationId) return false;
  if (event.locationId === targetPortId) return true;
 ...
 */
export function publicEventInformationProfile(event: WorldEvent): { category: KnowledgeRecord["category"]; source: string; confidence: number; staleAfterHours?: number } | undefined {
  if (event.type === "npc_arrival") return { category:"maritime", source:"Harbor and sailor traffic", confidence:72, staleAfterHours:120 };
  if (event.canonicalData.publicKnowledge === true) {
    ...
    const allowed: KnowledgeRecord["category"][] = ["trade","local","danger","personal","political","religious","maritime"];
    return {
      category: allowed.includes(category as KnowledgeRecord["category"]) ? category as KnowledgeRecord["category"] : "local",
