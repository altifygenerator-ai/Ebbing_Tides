import type { CharacterCapabilityState, GameState, PreparedAbilityEffect, PreparedAbilityTrigger } from "./types.js";

export interface PreparedAbilityRule {
  trigger: PreparedAbilityTrigger;
  durationHours: number;
  bonus: number;
  context: "character" | "ship";
}
 ...
 */
export const PREPARED_ABILITY_RULES: Record<string, PreparedAbilityRule> = {
...
function contextForRule(state: GameState, rule: PreparedAbilityRule): string {
  return rule.context === "ship" ? state.player.shipId : state.player.character.id;
}

function cleanCharacterPreparedEffects(character: CharacterCapabilityState, currentHour: number): void {
  const raw = Array.isArray(character.preparedEffects) ? character.preparedEffects : [];
  const byAbility = new Map<string, PreparedAbilityEffect>();
  for (const row of raw) {
    ...
    if (remainingUses <= 0 || expiresAtHour <= currentHour) continue;
    const normalized: PreparedAbilityEffect = {
    ...
    if (!prior || normalized.preparedAtHour >= prior.preparedAtHour) byAbility.set(normalized.abilityId, normalized);
  }
  character.preparedEffects = [...byAbility.values()];
}

function latestLegacySuccessfulUse(state: GameState, abilityId: string, durationHours: number) {
  return [...state.worldEvents].reverse().find(event =>
    event.type === "character_ability_use" &&
 ...
 */
export function normalizePreparedAbilityEffects(state: GameState): void {
  ...
  for (const npc of Object.values(state.npcs ?? {})) cleanCharacterPreparedEffects(npc, state.absoluteHour);
}

export function prepareAbilityEffect(state: GameState, abilityId: string): PreparedAbilityEffect | undefined {
  const rule = PREPARED_ABILITY_RULES[abilityId];
  if (!rule) return undefined;
  cleanCharacterPreparedEffects(state.player.character, state.absoluteHour);
  const effect: PreparedAbilityEffect = {
    abilityId,
    trigger: rule.trigger,
...
export function preparedAbilityEffect(
  state: GameState,
  abilityId: string,
  trigger: PreparedAbilityTrigger,
  contextEntityId?: string
): PreparedAbilityEffect | undefined {
  cleanCharacterPreparedEffects(state.player.character, state.absoluteHour);
  return state.player.character.preparedEffects?.find(row =>
...
export function preparedAbilityBonus(
  state: GameState,
  abilityId: string,
  trigger: PreparedAbilityTrigger,
  contextEntityId?: string
...
export function consumePreparedAbilityEffect(
  state: GameState,
  abilityId: string,
  trigger: PreparedAbilityTrigger,
  contextEntityId?: string
