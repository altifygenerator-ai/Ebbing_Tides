...
import type { CharacterCreationChoices, GameState, PortMarketState, ShipEntity, SkillId } from "./types.js";
...
const DEFAULT_CORE_SKILLS: SkillId[] = ["sailing", "navigation", "trading", "blades", "ship_repair"];
...
function buildMarkets(): Record<string, PortMarketState> {
...
export function createGame(choices: CharacterCreationChoices, seed?: string): GameState {
  ...
  if (!validateAttributes(choices.attributes)) throw new Error("Attributes must remain 1-10 and total no more than 36 in Alpha 0.1.");
  if (choices.coreSkills.length !== 5) throw new Error("Choose exactly five core skills.");
  if (!PORT_BY_ID[choices.homePortId]) throw new Error("Unknown home port.");
