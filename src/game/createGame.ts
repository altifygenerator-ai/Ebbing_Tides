...
import type { CharacterCreationChoices, GameState, ShipEntity, SkillId } from "./types.js";
import { validateAttributes } from "./skills.js";
import { buildCapabilityState, visualDnaFromChoices } from "./characterSystem.js";
import { createNpcFromSeed, ensureNpcPlan, type NpcSeedDefinition } from "./npcBrain.js";
...
const DEFAULT_CORE_SKILLS: SkillId[] = ["seamanship", "navigation", "commerce", "blades", "survival"];

export interface StartingShipOriginImpact {
  hullBonus: number;
  crownAdjustment: number;
  cargo: Array<{ commodityId:string; quantity:number }>;
  fameTags: string[];
}

export function startingShipOriginImpact(choices: CharacterCreationChoices): StartingShipOriginImpact {
  if (choices.shipOrigin === "naval_surplus") return { hullBonus:8, crownAdjustment:0, cargo:[], fameTags:["Former Naval Surplus"] };
  if (choices.shipOrigin === "prize_share") return { hullBonus:5, crownAdjustment:0, cargo:[], fameTags:[] };
  if (choices.shipOrigin === "purchased_on_debt") return { hullBonus:0, crownAdjustment:-40, cargo:[{ commodityId:"good.grain", quantity:4 }], fameTags:[] };
  return { hullBonus:0, crownAdjustment:0, cargo:[], fameTags:[] };
}

export function startingCrownsForChoices(choices: CharacterCreationChoices): number {
...
export const DEFAULT_CHARACTER_CHOICES: CharacterCreationChoices = {
  name: "",
  age: 27,
  sex: "male",
  ancestry: "skeldran",
...
function playerShip(choices: CharacterCreationChoices): ShipEntity {
...
function npcShips(): Record<string, ShipEntity> {
  const ships: Record<string, ShipEntity> = {
    "ship.stormcrow": {
      id: "ship.stormcrow",
      name: "Stormcrow",
  ...
  for (const ship of Object.values(ships)) {
    ...
    if (ship.route) ship.position = routePoint(ship.route.fromPortId, ship.route.toPortId, ship.route.progress);
  }
  return ships;
}

export function createGame(choices: CharacterCreationChoices, seed?: string): GameState {
  const name = choices.name.trim();
  if (!name) throw new Error("Captain name is required.");
  if (choices.age < 18 || choices.age > 45) throw new Error("Starting age must be 18-45.");
  if (!validateAttributes(choices.attributes)) throw new Error("Attributes must remain 1-10 and total exactly 36.");
  if (choices.coreSkills.length !== 5) throw new Error("Choose exactly five core skills.");
  const homeSettlement = ORIGIN_SETTLEMENT_BY_ID[choices.homeSettlementId];
  if (!homeSettlement) throw new Error("Unknown home settlement.");
  if (homeSettlement.region !== choices.homelandRegion) throw new Error("Home settlement must belong to the selected homeland region.");
  const startingPort = PORT_BY_ID[choices.startingLocationId];
  if (!startingPort) throw new Error("Unknown starting location.");
  ...
  const extraSeeds: NpcSeedDefinition[] = [
  ...
  const equipNpcLoadout = (npcId: string, inventory: ReturnType<typeof startingInventory>, equip: Partial<typeof state.player.equipment>) => {
