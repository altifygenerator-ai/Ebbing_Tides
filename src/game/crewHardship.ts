import type { GameState, ShipCrewWelfareState, ShipEntity } from "./types.js";
import { hasGeneralPerk } from "./progression.js";
import { ensureCrewCommunity } from "./crewState.js";

function clamp(value:number,min:number,max:number):number { return Math.max(min,Math.min(max,value)); }
function average(values:number[],fallback:number):number { return values.length ? values.reduce((sum,value)=>sum+value,0)/values.length : fallback; }

export interface CrewLeadershipProfile {
  score: number;
  moraleLossFactor: number;
  healthLossFactor: number;
  repeatedNeglectFactor: number;
  sources: Array<{source:string;value:number}>;
}

export interface CrewHardshipResult {
  zeroSupplyHours: number;
  moraleLoss: number;
  healthLoss: number;
  stage: "none" | "strained" | "serious" | "severe";
  leadership: CrewLeadershipProfile;
}

export function ensureCrewWelfare(ship:ShipEntity):ShipCrewWelfareState {
  if(!ship.crewWelfare){
    ship.crewWelfare={
      averageHealth:100,
      zeroSupplyHours:0,
...
export function crewLeadershipProfile(state:GameState):CrewLeadershipProfile {
...
function cumulativeMoralePressure(hours:number):number {
...
function cumulativeHealthPressure(hours:number):number {
  const h=Math.max(0,hours);
  if(h<=72)return 0;
...
export function crewHardshipStage(hours:number):CrewHardshipResult["stage"] {
  if(hours<24)return "none";
  if(hours<72)return "strained";
  if(hours<120)return "serious";
  return "severe";
}

function rememberHardship(state:GameState,eventId:string):void {
...
function recordThreshold(state:GameState,ship:ShipEntity,hours:number,threshold:number,summary:string):void {
...
export function markSupplyExhausted(state:GameState):void {
  const ship=state.ships[state.player.shipId];
  if(!ship)return;
  const welfare=ensureCrewWelfare(ship);
  if(welfare.shortageActive)return;
...
export function resetSupplyHardshipIfProvisioned(state:GameState):void {
  const ship=state.ships[state.player.shipId];
  if(!ship || ship.supplies<=0)return;
  const welfare=ensureCrewWelfare(ship);
  if(!welfare.shortageActive && welfare.zeroSupplyHours<=0)return;
...
export function applyZeroSupplyHardship(state:GameState,hoursWithoutSupplies:number):CrewHardshipResult {
  const ship=state.ships[state.player.shipId];
  if(!ship || hoursWithoutSupplies<=0 || ship.supplies>0){
    if(ship?.supplies && ship.supplies>0)resetSupplyHardshipIfProvisioned(state);
  ...
  if(moraleLoss>0){
    ...
    for(const member of state.player.crew) member.morale=clamp(member.morale-moraleLoss,0,100);
  ...
  if(healthLoss>0){
    ...
    for(const member of state.player.crew){
      ...
      if(member.npcId){
        const npc=state.npcs[member.npcId];
        if(npc)npc.brain.fastState.health=clamp(npc.brain.fastState.health-healthLoss,0,100);
      }
    }
  ...
  for(const threshold of [24,72,120]){
    if(beforeHours<threshold && welfare.zeroSupplyHours>=threshold){
      const community=ensureCrewCommunity(ship);
      if(threshold===72) community.loyalty=clamp(community.loyalty-2,0,100);
      if(threshold===120) community.loyalty=clamp(community.loyalty-4,0,100);
