import type { GameState, RangeState, ShipEntity } from "./types.js";
import { roll2d10 } from "./rng.js";
import { attributeModifier } from "./skills.js";

const RANGE_ORDER: RangeState[] = ["distant", "long", "medium", "close", "grapple", "boarding"];

function clamp(value: number, min: number, max: number): number {
  return Math.max(min, Math.min(max, value));
}

function moveRange(range: RangeState, direction: "close" | "open"): RangeState {
  const index = RANGE_ORDER.indexOf(range);
  if (direction === "close") return RANGE_ORDER[Math.min(RANGE_ORDER.length - 1, index + 1)]!;
  return RANGE_ORDER[Math.max(0, index - 1)]!;
}

function rangeModifier(range: RangeState): number {
  switch (range) {
    case "long": return -2;
...
export type CombatAction = "close" | "open" | "fire_hull" | "fire_rigging" | "repair" | "flee";
...
export function combatAction(state: GameState, action: CombatAction): { ok: boolean; message: string } {
  ...
  if (action === "close" || action === "open") {
    ...
    if (encounter.range === "distant" || encounter.range === "boarding") return { ok: false, message: "Your guns cannot resolve that fire order at this range." };
