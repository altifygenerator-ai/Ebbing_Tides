import type { GameState, SimulationEvent } from "./types.js";
 ...
 */
export function upsertSimulationEvent(state: GameState, row: SimulationEvent): void {
  const index = state.simulationEvents.findIndex((event) => event.id === row.id);
  if (index >= 0) state.simulationEvents[index] = row;
  else state.simulationEvents.push(row);
}

export function removePlanCheckpoints(state: GameState, planId: string): void {
 ...
 */
export function compactSimulationEvents(state: GameState): void {
  const seen = new Set<string>();
  state.simulationEvents = (state.simulationEvents ?? []).filter((event) => {
    if (event.status !== "scheduled") return false;
    if (!Number.isFinite(event.scheduledAtHour)) return false;
    ...
    if (event.eventType === "npc_plan_checkpoint") {
      ...
      if (!plan || plan.status !== "active" || plan.id !== eventPlanId) return false;
    }
