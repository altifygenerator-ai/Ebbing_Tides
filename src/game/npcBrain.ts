...
import type { Attributes, CultureId, GameState, GridPoint, NpcBrainState, NpcCharacter, NpcGoal, NpcNeeds, NpcPlan, RegionId, ReligionId, SexId, SkillId, SimulationEvent, VisualDNA } from "./types.js";
...
export interface NpcSeedDefinition {
  id: string;
  name: string;
  age: number;
  sex?: SexId;
  ancestry?: "skeldran" | "asterian" | "serathi" | "kaishin" | "mixed";
...
function defaultAttributes(seed: NpcSeedDefinition): Attributes {
...
function defaultSkills(seed: NpcSeedDefinition): Record<SkillId, number> {
  const skills = Object.fromEntries(ALL_SKILLS.map((id) => [id, 5])) as Record<SkillId, number>;
  const role = `${seed.profession ?? ""} ${seed.role}`.toLowerCase();
  const set = (id: SkillId, value: number) => { skills[id] = Math.max(skills[id], value); };
  if (/captain|master|mate/.test(role)) { set("seamanship", 58); set("navigation", 48); set("command", 52); set("commerce", 30); }
  if (/navy|stormcrow/.test(role)) { set("gunnery", 60); set("command", 64); set("blades", 48); set("firearms", 44); }
  if (/gunner/.test(role)) { set("gunnery", 62); set("firearms", 48); set("craft", 36); }
  if (/navigator/.test(role)) { set("navigation", 64); set("seamanship", 54); set("survival", 42); }
  if (/shipwright|carpenter/.test(role)) { set("engineering", 58); set("craft", 62); set("seamanship", 38); }
  if (/merchant|privateer/.test(role)) { set("commerce", 58); set("persuasion", 46); set("streetwise", 34); }
  if (/pirate|raider/.test(role)) { set("seamanship", 62); set("command", 55); set("blades", 55); set("streetwise", 58); set("deception", 44); }
  if (/preacher|pastor|priest/.test(role)) { set("scholarship", 62); set("persuasion", 58); set("medicine", 40); }
  if (/engineer|foundry/.test(role)) { set("engineering", 60); set("craft", 50); }
  return skills;
}

function dnaForSeed(seed: NpcSeedDefinition): VisualDNA {
...
function defaultNeeds(seed: NpcSeedDefinition): NpcNeeds {
...
export function createNpcFromSeed(seed: NpcSeedDefinition): NpcCharacter {
  const skills = defaultSkills(seed);
  const goals: NpcGoal[] = seed.goals.map((description, index) => ({ id: `goal.${seed.id}.${index}`, type: "role_goal", description, priority: Math.max(30, 80-index*12), status: "active", createdAtHour: 0 }));
  const brain: NpcBrainState = {
...
function schedulePlanCheckpoint(state: GameState, npc: NpcCharacter, plan: NpcPlan): void {
  if (plan.nextDecisionAtHour === undefined) return;
  const eventId = `sim.plan_checkpoint.${plan.id}`;
  const row: SimulationEvent = {
    id: eventId,
...
function createPortDutiesPlan(state: GameState, npc: NpcCharacter, reason: string): NpcPlan {
  const plan: NpcPlan = {
...
function survivalReserveProblemsForPlan(state: GameState, npc: NpcCharacter, plan: NpcPlan | undefined): string[] {
  if (!plan || plan.type !== "travel") return [];
  const etaDays = plan.expectedCompletionHour ? Math.max(0, plan.expectedCompletionHour - state.absoluteHour) / 24 : 0;
  const reasons: string[] = [];
  if (npc.brain.needs.foodDays < etaDays + 2) reasons.push("food reserve forecast below destination ETA + safety reserve");
  if (npc.brain.needs.waterDays < etaDays + 2) reasons.push("water reserve forecast below destination ETA + safety reserve");
  return reasons;
}

function nearestNavigablePoint(point: GridPoint): GridPoint | undefined {
  const rounded = { x: Math.round(point.x), y: Math.round(point.y) };
  if (isNavigableCell(rounded)) return rounded;
  for (let radius = 1; radius <= 3; radius += 1) {
    for (let dy = -radius; dy <= radius; dy += 1) {
      for (let dx = -radius; dx <= radius; dx += 1) {
        if (Math.abs(dx) !== radius && Math.abs(dy) !== radius) continue;
        const candidate = { x: rounded.x + dx, y: rounded.y + dy };
        if (isNavigableCell(candidate)) return candidate;
      }
    }
  }
  return undefined;
...
function nearestReachablePort(point: GridPoint): { portId: string; path: GridPoint[] } | undefined {
  ...
  for (const { port } of candidates) {
    ...
    if (path.length >= 2) return { portId: port.id, path };
  }
  return undefined;
}

function travelPlanFromPath(state: GameState, npc: NpcCharacter, path: GridPoint[], destinationPortId: string, reason: string, fromPortId?: string, survivalRecovery = false): NpcPlan | undefined {
  const ship = npc.shipId ? state.ships[npc.shipId] : undefined;
  if (!ship || path.length < 2) return undefined;
...
export function planTravel(state: GameState, npc: NpcCharacter, fromPortId: string, destinationPortId: string, reason: string): NpcPlan | undefined {
  const ship = npc.shipId ? state.ships[npc.shipId] : undefined;
  const from = PORT_BY_ID[fromPortId]; const to = PORT_BY_ID[destinationPortId];
  if (!ship || !from || !to) return undefined;
  if (worldCauseInfluenceForRoute(state,fromPortId,destinationPortId).trafficMultiplier <= 0.01) return undefined;
...
function chooseRoutineDestination(state: GameState, npc: NpcCharacter, currentPortId: string): string | undefined {
  const ship = npc.shipId ? state.ships[npc.shipId] : undefined;
  const from = PORT_BY_ID[currentPortId];
  if (!ship || !from) return undefined;
  ...
  const candidates = Object.values(PORT_BY_ID).flatMap((port) => {
    ...
    if (routeCause.trafficMultiplier <= 0.01) return [];
...
export function ensureNpcPlan(state: GameState, npc: NpcCharacter): void {
  if (!npc.shipId) return;
  const ship = state.ships[npc.shipId]; if (!ship) return;
  if (!isShipOperational(ship)) {
    if (npc.brain.currentPlan?.status === "active") { npc.brain.currentPlan.status="invalid"; npc.brain.currentPlan.interruptReason="VESSEL_TERMINAL_STATE"; }
    return;
  }
  ...
  if (!currentPortId) {
    ...
    if (plan) {
      npc.brain.currentPlan = plan;
      if (plan.nextDecisionAtHour !== undefined) npc.brain.nextDecisionAtHour = plan.nextDecisionAtHour;
      delete ship.dockedAtPortId;
      delete npc.locationPortId;
    }
    return;
  ...
  if (plan) {
    ...
    if (reserveProblems.length) {
      ...
      if (servicePlan.nextDecisionAtHour !== undefined) npc.brain.nextDecisionAtHour = servicePlan.nextDecisionAtHour;
...
export function forecastNpcNeeds(state: GameState, npc: NpcCharacter): string[] {
  const needs = npc.brain.needs;
  const reasons: string[] = [];
  const plan = npc.brain.currentPlan;
  reasons.push(...survivalReserveProblemsForPlan(state, npc, plan));
  if (needs.moneyReserve < 30) reasons.push("money reserve below operating threshold");
  if (needs.hullSafety < 45) reasons.push("ship condition below safe voyage threshold");
  if (needs.moraleSafety < 30 || needs.wageArrearsDays > 20) reasons.push("crew morale/wage state threatens discipline");
  return reasons;
}

export function interruptNpcPlan(state: GameState, npc: NpcCharacter, reason: string): void {
  const plan = npc.brain.currentPlan;
  if (!plan || plan.status !== "active") return;
  plan.status = "interrupted"; plan.interruptReason = reason; npc.brain.nextDecisionAtHour = state.absoluteHour;
  removePlanCheckpoints(state, plan.id);
...
export function advanceNpcPlan(state: GameState, npc: NpcCharacter, hours: number): void {
  ensureNpcPlan(state, npc);
  const plan = npc.brain.currentPlan; const ship = npc.shipId ? state.ships[npc.shipId] : undefined;
  if (!plan || plan.status !== "active" || !ship) return;
  if (plan.type === "travel" && plan.fromPortId && plan.destinationPortId && !plan.survivalRecovery) {
    ...
    if (routeCause.trafficMultiplier <= 0.01) { interruptNpcPlan(state,npc,"WORLD_ROUTE_RESTRICTION"); return; }
  }
  schedulePlanCheckpoint(state, npc, plan);
  if (!isShipOperational(ship)) { plan.status="invalid"; plan.interruptReason="VESSEL_TERMINAL_STATE"; return; }
  ...
  if (plan.type !== "travel" || !plan.path?.length || !plan.expectedCompletionHour) return;
  plan.routeDistanceNm ??= routeDistanceNm(plan.path);
  ...
  if (scheduled) scheduled.scheduledAtHour = plan.expectedCompletionHour;
  ...
  if (plan.progress >= 1 && plan.destinationPortId) {
    ...
    if (port) ship.position = { ...port.approachPoint };
    ship.dockedAtPortId = plan.destinationPortId; npc.locationPortId = plan.destinationPortId; plan.status = "completed";
    ...
    if (npc.brain.currentPlan.nextDecisionAtHour !== undefined) npc.brain.nextDecisionAtHour = npc.brain.currentPlan.nextDecisionAtHour;
...
export function evaluateImmediateUtility(state: GameState, npc: NpcCharacter, action: "attack"|"flee"|"trade"|"resupply"): { total:number; contributions:Array<{source:string;value:number}> } {
  const c:Array<{source:string;value:number}> = [];
  const push=(source:string,value:number)=>{c.push({source,value});return value;};
  const p=npc.personality; const n=npc.brain.needs;
  let total=0;
  if(action==="attack") total+=push("aggression",(p.aggression??40)*.25)+push("risk_tolerance",(p.riskTolerance??p.courage??40)*.18)+push("desperation",Math.max(0,6-n.foodDays)*4);
  if(action==="flee") total+=push("fear",npc.brain.fastState.fear*.35)+push("survival",Math.max(0,55-n.hullSafety)*.8)-push("courage",(p.courage??40)*.12);
  if(action==="trade") total+=push("commerce_role",npc.skills.commerce*.25)+push("money_need",Math.max(0,150-n.moneyReserve)*.08);
  if(action==="resupply") total+=push("food_need",Math.max(0,8-n.foodDays)*8)+push("water_need",Math.max(0,8-n.waterDays)*8)+push("wage_pressure",n.wageArrearsDays*.8);
...
export function updateNpcSimulationLod(state: GameState, npc: NpcCharacter): void {
  const playerShip = state.ships[state.player.shipId];
  const npcShip = npc.shipId ? state.ships[npc.shipId] : undefined;
  if (!playerShip || !npcShip) { npc.brain.simulationLod = npc.locationPortId === state.player.currentPortId ? "moderate" : "coarse"; return; }
  ...
  if (npc.brain.simulationLod === "detailed") npc.brain.lastHighResolutionAtHour = state.absoluteHour;
}

export function mutinyPressure(npc: NpcCharacter): number {
  const n=npc.brain.needs; const leadership=npc.skills.command; const loyalty=Math.max(0,Math.min(100,50+(npc.personality.loyalty??0)*.25));
