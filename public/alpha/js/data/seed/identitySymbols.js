const runtime = (name) => `/art/ui/symbols/runtime/${name}.png`;
...
const CULTURE_ANCESTRY_FALLBACK = {
...
export function ancestryMarkForContext(context) {
    return ANCESTRY_MARK[context.ancestry] ?? (context.ancestry === "mixed" ? CULTURE_ANCESTRY_FALLBACK[context.culture] : undefined);
}
export function religionMarkForId(religion) {
    return RELIGION_MARK[religion];
}
export function homelandMarkForRegion(region) {
    return HOMELAND_MARK[region];
}
export function affiliationMarkForId(affiliationId) {
 ...
 */
export function affiliationMarkForCharacter(input) {
    const explicit = affiliationMarkForId(input.affiliationId);
    if (explicit)
        return explicit;
    const role = (input.role ?? "").toLowerCase();
    if (/\bhouse vaering\b/.test(role))
        return IDENTITY_MARKS.affiliationHouseVaering;
    if (/\bskeldran royal navy\b|\broyal navy\b/.test(role))
        return IDENTITY_MARKS.affiliationSkeldranRoyalNavy;
    if (/\bmerchant guild\b/.test(role))
        return IDENTITY_MARKS.affiliationMerchantGuild;
    if (/\bmarine order\b/.test(role))
        return IDENTITY_MARKS.affiliationMarineOrder;
    if (/\bharbou?r authority\b/.test(role))
        return IDENTITY_MARKS.affiliationHarborAuthority;
    if (/\bfree captains?\b/.test(role))
        return IDENTITY_MARKS.affiliationFreeCaptains;
    return undefined;
}
export function resolveIdentityPresentation(context) {
...
export function governmentIdentityMarksForPort(portId, region) {
    if (portId === "port.veyrholm")
        return [IDENTITY_MARKS.affiliationHouseVaering];
...
export function religionIdentityMarksForPort(portId) {
    if (portId === "port.veyrholm" || portId === "port.ironhaven")
        return [IDENTITY_MARKS.religionOldGods, IDENTITY_MARKS.religionCovenant];
    if (portId === "port.stormvik" || portId === "port.thorenfjord")
        return [IDENTITY_MARKS.religionOldGods];
    return [];
}
/**
...
//# sourceMappingURL=identitySymbols.js.map