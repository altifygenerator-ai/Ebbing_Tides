...
const playable = (row) => ({ ...row, playableInCurrentAlpha: true, gameplayEntityId: row.id });
...
export function canonLocationsForRegion(region) { return CANON_WORLD_LOCATIONS.filter(row => row.canonicalRegion === region); }
export function canonicalLocationName(id) { return CANON_WORLD_LOCATION_BY_ID[id]?.name ?? id; }
