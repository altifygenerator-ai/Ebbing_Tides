...
function categoryFor(subcategory, legalStatus) {
    if (legalStatus === "contraband" || legalStatus === "stolen")
        return "illicit";
    switch (subcategory) {
...
function generatedPrice(category, id) {
    const ranges = {
...
function generatedCargoUnits(category) {
    if (category === "raw" || category === "manufactured")
        return 2;
    return 1;
}
const legacyIds = new Set(LEGACY_COMMODITIES.map((good) => good.id));
const GENERATED_CANON_COMMODITIES = CONTENT_DEFINITIONS
...
export const COMMODITIES = [...LEGACY_COMMODITIES, ...GENERATED_CANON_COMMODITIES];
