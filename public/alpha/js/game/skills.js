...
export function buildStartingSkills(choices) {
    const skills = Object.fromEntries(ALL_SKILLS.map((skill) => [skill, 0]));
    for (const skill of choices.coreSkills)
        skills[skill] += 3;
    switch (choices.socialOrigin) {
        case "dockside_poor":
            ...
            skills.streetwise += 2;
            break;
        case "artisan_household":
            skills.appraisal += 1;
            skills.engineering += 2;
            break;
        case "merchant_family":
            skills.trading += 2;
            skills.appraisal += 2;
            break;
        case "naval_family":
            skills.sailing += 1;
            skills.gunnery += 1;
            skills.ship_command += 1;
            break;
    }
    switch (choices.background) {
        case "former_naval_midshipman":
            skills.sailing += 2;
            skills.gunnery += 2;
            skills.ship_command += 1;
            break;
        case "foundry_child":
            ...
            skills.appraisal += 1;
            skills.ship_repair += 1;
            break;
        case "raised_among_smugglers":
            skills.streetwise += 2;
            skills.smuggling += 3;
            break;
    ...
    switch (choices.recentProfession) {
        ...
        case "dockworker":
            ...
            skills.streetwise += 1;
            break;
        case "apprentice_engineer":
            skills.engineering += 2;
            skills.ship_repair += 1;
            break;
    }
    if (choices.trait === "sea_legs")
        skills.sailing += 1;
    if (choices.trait === "silver_tongue")
        skills.persuasion += 1;
