export const COMMON_RECKONING_CALENDAR = {
...
export const CAMPAIGN_START_DATE = { year: 628, month: 1, day: 1, hour: 0 };
export function daysInYear(calendar = COMMON_RECKONING_CALENDAR) {
    return calendar.monthLengths.reduce((sum, value) => sum + value, 0);
}
export function validateWorldDate(date, calendar = COMMON_RECKONING_CALENDAR) {
    const errors = [];
    if (!Number.isInteger(date.year))
        errors.push("WorldDate year must be an integer.");
    if (!Number.isInteger(date.month) || date.month < 1 || date.month > calendar.monthsPerYear)
        errors.push("WorldDate month is out of range.");
    const monthLength = calendar.monthLengths[date.month - 1];
    if (!Number.isInteger(date.day) || date.day < 1 || monthLength === undefined || date.day > monthLength)
        errors.push("WorldDate day is out of range.");
    if (!Number.isInteger(date.hour) || date.hour < 0 || date.hour > 23)
        errors.push("WorldDate hour is out of range.");
    return errors;
}
export function absoluteHourFromWorldDate(date, epoch = CAMPAIGN_START_DATE, calendar = COMMON_RECKONING_CALENDAR) {
    const dateErrors = validateWorldDate(date, calendar);
    const epochErrors = validateWorldDate(epoch, calendar);
    if (dateErrors.length || epochErrors.length)
        throw new Error([...dateErrors, ...epochErrors].join(" "));
    const yearHours = daysInYear(calendar) * 24;
    const toDayOffset = (value) => {
...
export function worldDateFromAbsoluteHour(absoluteHour, epoch = CAMPAIGN_START_DATE, calendar = COMMON_RECKONING_CALENDAR) {
    if (!Number.isFinite(absoluteHour))
        throw new Error("absoluteHour must be finite.");
...
export function addHours(date, hours, calendar = COMMON_RECKONING_CALENDAR) {
...
export function formatHistoricalDate(date, calendar = COMMON_RECKONING_CALENDAR) {
...
export function validateHistoricalDate(date, calendar = COMMON_RECKONING_CALENDAR) {
    const errors = [];
    const needsYear = date.precision !== "unknown";
    if (needsYear && date.year === undefined)
        errors.push("Historical date precision requires a year.");
    if (date.precision === "month" && date.month === undefined)
        errors.push("Month-precision historical date requires month.");
    if (date.precision === "exact" && (date.month === undefined || date.day === undefined))
        errors.push("Exact historical date requires month and day.");
    if (date.precision === "range" && date.endYear === undefined)
        errors.push("Range historical date requires endYear.");
    if (date.year !== undefined && date.endYear !== undefined && date.endYear < date.year)
        errors.push("Historical date range ends before it begins.");
    if (date.month !== undefined && (date.month < 1 || date.month > calendar.monthsPerYear))
        errors.push("Historical date month is out of range.");
    if (date.day !== undefined && date.month !== undefined) {
        const max = calendar.monthLengths[date.month - 1];
        if (date.day < 1 || max === undefined || date.day > max)
            errors.push("Historical date day is out of range.");
    }
    if (date.hour !== undefined && (date.hour < 0 || date.hour > 23))
        errors.push("Historical date hour is out of range.");
    return errors;
}
/** Sort key for chronology checks. Unknown dates return undefined rather than pretending precision. */
export function historicalDateSortKey(date) {
    ...
    try {
        return absoluteHourFromWorldDate({ year: date.year, month, day, hour });
    }
    catch {
        return undefined;
    }
