 ...
 */
export function navalCombatSpecializationFor(action, rangeYards) {
    if (action === "fire_hull" || action === "fire_rigging")
        return rangeYards > 300 ? "Long-Range" : undefined;
    if (action === "repair")
        return "Naval Machinery";
    if (action === "demand_surrender")
        return "Naval Discipline";
    if (action === "close" || action === "open" || action === "flee" || action === "grapple")
        return "Heavy Weather";
    return undefined;
}
function navalCombatSpecializationRequest(action, rangeYards) {
    const specializationName = navalCombatSpecializationFor(action, rangeYards);
    return specializationName ? { specializationName } : {};
}
function combatSpeedKnots(ship) {
    return Math.max(0.75, baseCruiseSpeedKnots(ship) * shipConditionSpeedFactor(ship));
}
function maneuverRelativeKnots(actor, opponent, mode) {
...
function changeRange(encounter, relativeKnots) {
...
function advanceCombatTime(state, encounter) {
...
function resolveVictory(state) {
    ...
    if (enemy.systems.hull <= 0 || enemy.systems.crew <= 0 || enemy.systems.morale <= 0) {
        ...
        if (!resolution.ok)
            return `${enemy.name} has already been resolved as a ${resolution.status} vessel; no second prize can be claimed.`;
        ...
        for (const member of state.player.crew) {
            if (member.npcId)
                awardLifeExperience(state, member.npcId, `naval-victory:${encounter.id}`, `Served in naval victory against ${enemy.name}`, 50);
        }
...
export function navalCombatActionChance(state, action) {
    const encounter = state.encounter;
    if (!encounter || encounter.phase !== "combat")
        return undefined;
    const enemy = state.ships[encounter.otherShipId];
    ...
    if (action === "repair") {
        const specialist = effectiveSpecialist(state, "engineering");
        const actor = specialist.source === "officer" ? state.npcs[specialist.characterId] : state.player.character;
        const repairTalent = generalPerkModifier(actor, "field_repair");
        const captainRepairTalent = actor?.id === state.player.character.id ? { value: 0, sources: [] } : generalPerkModifier(state.player.character, "field_repair");
        const repairPerk = repairTalent.value >= captainRepairTalent.value ? repairTalent : captainRepairTalent;
    ...
    if (action === "fire_hull" || action === "fire_rigging") {
        const specialist = effectiveSpecialist(state, "gunnery");
        const actor = specialist.source === "officer" ? state.npcs[specialist.characterId] : state.player.character;
        const prepared = preparedAbilityBonus(state, "tech.precision_weapons.precision_bore_sighting", "naval_fire", playerShip.id);
        const gunTalent = generalPerkModifier(actor, "naval_gunnery");
        const captainGunTalent = actor?.id === state.player.character.id ? { value: 0, sources: [] } : generalPerkModifier(state.player.character, "naval_gunnery");
        const gunneryPerk = gunTalent.value >= captainGunTalent.value ? gunTalent : captainGunTalent;
...
export function combatAction(state, action) {
    ...
    if (action === "grapple" && (encounter.rangeYards > 50 || encounter.shipsSecured))
        return { ok: false, message: "You must close inside 50 yards before attempting to secure the ships." };
    if ((action === "fire_hull" || action === "fire_rigging") && (encounter.rangeYards > 1500 || encounter.shipsSecured))
        return { ok: false, message: encounter.shipsSecured ? "The ships are secured together; the main battery cannot fire safely." : "The current battery is outside effective firing range." };
    if ((action === "close" || action === "open" || action === "flee") && encounter.shipsSecured)
        return { ok: false, message: action === "flee" ? "You cannot flee while the ships are secured together." : "The ships are secured together; break grapples before maneuvering." };
    ...
    if (action === "grapple") {
        ...
        if (!["failure", "severe_failure"].includes(check.outcome)) {
            ...
            encounter.range = "grapple";
            encounter.shipsSecured = true;
            playerText = `${specialist.name} brings Tideworn alongside and the grapples bite (${check.roll} vs ${check.chance}).`;
        }
        else {
            encounter.rangeYards = Math.min(100, encounter.rangeYards + 20);
            syncRange(encounter);
            playerText = `The enemy sheers away from the grapples (${check.outcome}); separation opens to ${Math.round(encounter.rangeYards)} yd.`;
        }
    }
        ...
        if (!["failure", "severe_failure"].includes(check.outcome)) {
            ...
            playerText = `The maneuver fails to gain the range you wanted (${check.outcome}).`;
    }
    else if (action === "fire_hull" || action === "fire_rigging") {
        if (encounter.rangeYards > 1500 || encounter.shipsSecured || encounter.range === "boarding")
            return { ok: false, message: "The current battery cannot resolve that fire order at this exact range/state." };
        const specialist = effectiveSpecialist(state, "gunnery");
        const actor = specialist.source === "officer" ? state.npcs[specialist.characterId] : state.player.character;
        const prepared = preparedAbilityBonus(state, "tech.precision_weapons.precision_bore_sighting", "naval_fire", playerShip.id);
        if (prepared > 0)
            consumePreparedAbilityEffect(state, "tech.precision_weapons.precision_bore_sighting", "naval_fire", playerShip.id);
        const gunTalent = generalPerkModifier(actor, "naval_gunnery");
        const captainGunTalent = actor?.id === state.player.character.id ? { value: 0, sources: [] } : generalPerkModifier(state.player.character, "naval_gunnery");
        const gunneryPerk = gunTalent.value >= captainGunTalent.value ? gunTalent : captainGunTalent;
        ...
        if (!["failure", "severe_failure"].includes(check.outcome)) {
            const damage = Math.max(2, playerShip.firepower + Math.max(0, Math.floor(check.margin / 12)));
            damageShip(enemy, damage, action === "fire_rigging" ? "sails" : "hull");
            enemy.systems.morale = clamp(enemy.systems.morale - 3, 0, 100);
            playerText = `${specialist.name}'s battery lands effective ${action === "fire_rigging" ? "chain" : "round"} shot at ${Math.round(encounter.rangeYards)} yd for ${damage} damage.`;
        }
        else
            playerText = `The broadside at ${Math.round(encounter.rangeYards)} yd fails to produce an effective hit (${check.outcome}).`;
    }
        ...
        const repaired = ["failure", "severe_failure"].includes(check.outcome) ? 0 : Math.max(1, 2 + Math.floor(Math.max(0, check.margin) / 15));
        playerShip.systems.hull = clamp(playerShip.systems.hull + repaired, 0, playerShip.systems.hullMax);
        if (check.margin >= 10)
            playerShip.systems.flooding = Math.max(0, playerShip.systems.flooding - 1);
        playerText = repaired ? `${specialist.name}'s damage-control party restores ${repaired} hull condition.` : `Damage control cannot stabilize the damage this round.`;
    }
    else if (action === "flee") {
        const specialist = effectiveSpecialist(state, "seamanship");
        const actor = specialist.source === "officer" ? state.npcs[specialist.characterId] : state.player.character;
        ...
        if (!["failure", "severe_failure"].includes(check.outcome)) {
            ...
            playerText = `You fail to shake the pursuit (${check.outcome}).`;
    }
    encounter.log.push(playerText);
