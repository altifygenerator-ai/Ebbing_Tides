...
export function generateContracts(state) {
    ...
    for (const destination of Object.values(PORT_BY_ID)) {
        ...
        for (const [commodityId, row] of Object.entries(destinationMarket.goods)) {
            ...
            if (["prohibited", "embargoed"].includes(sourceLaw.status) || ["prohibited", "embargoed"].includes(destinationLaw.status))
                continue;
            ...
            existingKey.add(key);
            if (state.contracts.filter((c) => c.status === "available").length >= 5)
                return;
        }
    }
}
export function acceptContract(state, contractId) {
    ...
    if (state.player.currentPortId !== contract.sourcePortId)
        return { ok: false, message: "You must be at the issuing port to accept this contract." };
...
export function fulfillContract(state, contractId) {
    const contract = state.contracts.find((item) => item.id === contractId);
    if (!contract || contract.status !== "accepted")
        return { ok: false, message: "Contract is not active." };
    if (state.player.currentPortId !== contract.destinationPortId)
        return { ok: false, message: "You are not at the delivery port." };
    if (state.absoluteHour > contract.deadlineHour)
        return { ok: false, message: "The deadline has passed." };
    const destinationLaw = evaluateCommodityTradeLaw(state, contract.destinationPortId, contract.commodityId);
    if (destinationLaw.status !== "open")
        return { ok: false, message: `The delivery cannot be cleared under current trade law: ${destinationLaw.reason}` };
    const ship = state.ships[state.player.shipId];
    const stack = ship?.cargo.find((item) => item.commodityId === contract.commodityId);
    if (!ship || !stack || stack.quantity < contract.quantity)
        return { ok: false, message: `You need ${contract.quantity} units of ${COMMODITY_BY_ID[contract.commodityId]?.name ?? "cargo"}.` };
    ...
    if (destinationPort)
        adjustFactionStanding(state, politicalPowerForRegion(destinationPort.region).factionId, 2);
    state.worldEvents.push({
    ...
    for (const contract of state.contracts) {
        if ((contract.status === "available" || contract.status === "accepted") && state.absoluteHour > contract.deadlineHour) {
            ...
            if (wasAccepted) {
                ...
                const destinationPort = PORT_BY_ID[contract.destinationPortId];
                if (destinationPort)
                    adjustFactionStanding(state, politicalPowerForRegion(destinationPort.region).factionId, -1);
                state.worldEvents.push({
