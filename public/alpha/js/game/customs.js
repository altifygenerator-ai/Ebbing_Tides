...
function cargoViolationIds(state, portId) {
    ...
    for (const stack of ship?.cargo ?? []) {
        ...
        if (law.status === "permit_required")
            controlled.push(stack.commodityId);
        else if (law.status === "prohibited" || law.status === "embargoed")
            prohibited.push(stack.commodityId);
    }
    return { controlled: [...new Set(controlled)], prohibited: [...new Set(prohibited)] };
}
export function customsInspectionRisk(state, portId) {
    ...
    if (causes.some(c => c.policyTags.includes("customs.search.intense")))
        risk += 22;
    if (causes.some(c => c.policyTags.includes("customs.search.relaxed")))
        risk -= 18;
    return Math.max(5, Math.min(95, Math.round(risk)));
}
export function prepareCustomsInspectionOnArrival(state, portId) {
    if (!state.arrival || state.arrival.destination.type !== "port" || state.arrival.destination.id !== portId)
        return undefined;
    const violations = cargoViolationIds(state, portId);
...
function removeCargo(state, ids) {
    const ship = state.ships[state.player.shipId];
    if (!ship)
        return 0;
    let units = 0;
    for (const stack of ship.cargo)
        if (ids.has(stack.commodityId))
            units += stack.quantity;
    ship.cargo = ship.cargo.filter(stack => !ids.has(stack.commodityId));
    return units;
}
export function presentCustomsPapers(state) {
    const inspection = state.arrival?.customsInspection;
    if (!inspection || inspection.status !== "pending")
        return { ok: false, message: "No customs inspection is waiting for your response." };
...
export function concealFromCustoms(state) {
    const inspection = state.arrival?.customsInspection;
    if (!inspection || inspection.status !== "pending")
        return { ok: false, message: "No customs inspection is waiting for your response." };
    const ids = new Set([...inspection.controlledCommodityIds, ...inspection.prohibitedCommodityIds]);
    if (!ids.size)
        return { ok: false, message: "There is no controlled cargo on the manifest worth concealing. Present your papers instead." };
    ...
    const penalty = Math.min(c.crowns, Math.max(50, confiscated * 20));
    c.crowns -= penalty;
...
export function obtainCustomsPermit(state) {
    const portId = state.player.currentPortId;
    if (!portId)
        return { ok: false, message: "Customs papers must be issued in port." };
    normalizeTradeLawState(state);
    const port = PORT_BY_ID[portId];
    const power = politicalPowerForRegion(port.region);
    const policy = tradePolicyForPort(portId);
    if (activeTradeCredential(state, "customs_permit", power.jurisdictionId))
        return { ok: false, message: "Your customs permit is already current." };
    const legal = legalStateForFaction(state, power.factionId);
    if (legal.status === "wanted" || legal.status === "outlawed")
        return { ok: false, message: "The customs office will not issue routine trade papers while an active warrant stands." };
    if (state.player.character.crowns < policy.customsPermitCost)
        return { ok: false, message: `The permit costs ${policy.customsPermitCost} crowns.` };
...
export function obtainLetterOfMarque(state) {
    const portId = state.player.currentPortId;
    if (!portId)
        return { ok: false, message: "A commission must be issued by a competent government office." };
    normalizeTradeLawState(state);
    const port = PORT_BY_ID[portId];
    const power = politicalPowerForRegion(port.region);
    const policy = tradePolicyForPort(portId);
    const enemies = activePolicyEnemyFactions(state, power.factionId);
    if (!privateeringOpenForFaction(state, power.factionId) || !enemies.length)
        return { ok: false, message: "No current state policy authorizes privateering from this jurisdiction." };
    if (activeLetterOfMarque(state, power.factionId))
        return { ok: false, message: "You already hold an active commission from this authority." };
    const legal = legalStateForFaction(state, power.factionId);
    if (legal.status !== "clear" && legal.status !== "watched")
        return { ok: false, message: "The issuing authority will not commission a captain with an active warrant." };
    if (factionStanding(state, power.factionId) < policy.commissionMinimumStanding)
        return { ok: false, message: `The crown requires at least ${policy.commissionMinimumStanding} political standing before entrusting a privateering commission.` };
    if (state.player.character.crowns < policy.commissionCost)
        return { ok: false, message: `The commission bond and papers cost ${policy.commissionCost} crowns.` };
    ...
    const causeEnd = relevantCauses.map(c => c.scheduledEndAtHour).filter((v) => v !== undefined).sort((a, b) => a - b)[0];
    for (const row of state.player.tradeCredentials) {
        if (row.kind === "letter_of_marque" && row.jurisdictionId === power.jurisdictionId && row.status === "active")
            row.status = "expired";
    }
...
export function attemptSmuggledTransaction(state, commodityId, quantity, direction) {
    const portId = state.player.currentPortId;
    if (!portId)
        return { ok: false, message: "You need a port to reach a local broker." };
    const law = evaluateCommodityTradeLaw(state, portId, commodityId);
    if (law.status === "open")
        return { ok: false, message: "That cargo can be traded openly; there is no reason to use the back channels." };
    const access = blackMarketAccess(state, portId);
    if (!access.available)
        return { ok: false, message: access.reason };
    const c = state.player.character;
    const skillId = (c.skills.streetwise ?? 0) >= (c.skills.deception ?? 0) ? "streetwise" : "deception";
    const profile = SETTLEMENT_ECONOMIC_PROFILE_BY_ID[portId];
    const customs = customsCapacityForPort(portId);
