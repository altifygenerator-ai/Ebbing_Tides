...
import { JURISDICTION_TRADE_POLICY_BY_ID } from "../data/seed/tradePolicies.js";
...
export function tradePolicyForPort(portId) {
...
export function normalizeTradeLawState(state) {
    state.player.tradeCredentials ??= [];
    for (const row of state.player.tradeCredentials) {
        row.status ??= "active";
        if (row.status === "active" && row.expiresAtHour !== undefined && state.absoluteHour >= row.expiresAtHour)
            row.status = "expired";
        row.authorizedEnemyFactionIds ??= [];
    }
}
export function activeTradeCredential(state, kind, jurisdictionId) {
...
function matchesCommodityTag(tag, commodityId, category, origin) {
    const [kind, ...rest] = tag.split(":");
    const value = rest.join(":");
    if (kind === "commodity")
        return value === commodityId;
    if (kind === "category")
        return value === category;
    if (kind === "origin")
        return Boolean(origin) && value === origin;
    return false;
}
function policyCausesForPort(state, portId) {
    return activeWorldCauses(state).filter(cause => worldCauseAppliesToPort(cause, portId));
}
export function evaluateCommodityTradeLaw(state, portId, commodityId) {
    normalizeTradeLawState(state);
    const port = PORT_BY_ID[portId];
    const good = COMMODITY_BY_ID[commodityId];
    if (!port || !good)
        return { commodityId, portId, jurisdictionId: "jurisdiction.unknown", factionId: "faction.unknown", status: "prohibited", label: "Unavailable", reason: "No competent customs rule is available for this cargo.", causeIds: [], credentialValid: false, violationCrimeType: "smuggling" };
    const power = politicalPowerForRegion(port.region);
    const policy = tradePolicyForPort(portId);
    const baseline = (policy.byLegalStatus[good.legalStatus ?? "ordinary"] ?? "open");
    const permit = activeTradeCredential(state, "customs_permit", power.jurisdictionId);
    let status = baseline === "prohibited" ? "prohibited" : baseline === "permit" && !permit ? "permit_required" : "open";
    ...
    let violationCrimeType = "smuggling";
    const causeIds = [];
    for (const cause of policyCausesForPort(state, portId)) {
        for (const raw of cause.policyTags ?? []) {
            if (raw.startsWith("trade.embargo.")) {
                ...
                if (matchesCommodityTag(selector, good.id, good.category, String(good.originRegion ?? ""))) {
                    status = "embargoed";
                    reason = `${cause.title}: ${cause.reason}`;
                    violationCrimeType = "aiding_enemy";
                ...
                if (matchesCommodityTag(selector, good.id, good.category, String(good.originRegion ?? ""))) {
                    status = permit ? "open" : "permit_required";
                    reason = `${cause.title}: current policy requires customs papers for this cargo.`;
                    causeIds.push(cause.id);
                }
            }
        }
...
export function customsCapacityForPort(portId) {
...
export function blackMarketAccess(state, portId) {
    const profile = SETTLEMENT_ECONOMIC_PROFILE_BY_ID[portId];
...
function worldCauseAppliesToFaction(cause, factionId) {
    ...
    if (!hasScope)
        return true;
    if (scope.factionIds?.includes(factionId))
        return true;
    const power = POLITICAL_POWER_BY_FACTION[factionId];
    if (!power)
        return false;
    if (scope.jurisdictionIds?.includes(power.jurisdictionId) || scope.regionIds?.includes(power.region))
        return true;
    if (scope.portIds?.some(portId => PORT_BY_ID[portId]?.region === power.region))
        return true;
    return false;
}
export function activePolicyEnemyFactions(state, factionId) {
    const out = new Set();
    for (const cause of activeWorldCauses(state)) {
        if (!worldCauseAppliesToFaction(cause, factionId))
            continue;
        for (const tag of cause.policyTags ?? []) {
            if (tag.startsWith("war.enemy:"))
                out.add(tag.slice("war.enemy:".length));
            if (tag.startsWith("privateering.enemy:"))
                out.add(tag.slice("privateering.enemy:".length));
        }
    }
    return [...out];
}
export function privateeringOpenForFaction(state, factionId) {
...
export function activeLetterOfMarque(state, issuingFactionId) {
    normalizeTradeLawState(state);
    return state.player.tradeCredentials.find(row => {
        if (row.kind !== "letter_of_marque" || row.status !== "active" || (issuingFactionId && row.factionId !== issuingFactionId))
            return false;
        if (!privateeringOpenForFaction(state, row.factionId))
            return false;
...
export function isAuthorizedPrizeTarget(state, target) {
    const targetFaction = politicalPowerForShip(target).factionId;
    normalizeTradeLawState(state);
    for (const commission of state.player.tradeCredentials) {
        if (commission.kind !== "letter_of_marque" || commission.status !== "active")
            continue;
        if (!privateeringOpenForFaction(state, commission.factionId))
            continue;
        const enemies = activePolicyEnemyFactions(state, commission.factionId);
        if (commission.authorizedEnemyFactionIds.includes(targetFaction) && enemies.includes(targetFaction))
            return { authorized: true, commission, reason: `Prize authorized under ${commission.reason}` };
    }
    return { authorized: false, reason: "No active commission authorizes this target." };
}
export function playerPoliticalFactionId(state) {
    return politicalPowerForRegion(state.player.character.homelandRegion).factionId;
}
export function wartimePostureAgainstPlayer(state, ship) {
...
//# sourceMappingURL=tradeLaw.js.map