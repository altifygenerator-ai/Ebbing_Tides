...
function currentLoads(state) {
    ...
    return [ship?.attunementLoad, location?.attunementLoad].filter((row) => Boolean(row));
}
function known(state, abilityId) {
    return state.player.character.abilities.some((row) => row.abilityId === abilityId && row.status === "known");
}
function hasRequiredItemTags(state, required) {
...
export function usePlayerAbility(state, abilityId) {
    const def = ABILITY_BY_ID[abilityId];
    const pc = state.player.character;
    const ship = state.ships[state.player.shipId];
    if (!def || !known(state, abilityId))
        return { ok: false, message: "That practice or technique is not part of your character's learned history." };
    if (def.requiredSkillId && pc.skills[def.requiredSkillId] < (def.requiredSkillRating ?? 0))
        return { ok: false, message: `${def.name} requires more ${def.requiredSkillId} training.` };
    if (def.requiredSystemTags?.length && (!ship || !def.requiredSystemTags.every(tag => ship.systemTags?.includes(tag))))
        return { ok: false, message: `${def.name} requires a compatible real ship system or tool.` };
    if (def.requiredItemTags?.length && !hasRequiredItemTags(state, def.requiredItemTags))
        return { ok: false, message: `${def.name} requires a compatible carried instrument.` };
    ...
    if (isArcane) {
        const strain = Math.max(0, (def.strainCost ?? 1) - (ritualTalent.value > 0 ? 1 : 0));
        applyArcaneStrain(pc.attunement, strain);
        pc.attunement.arcaneExposure += Math.max(1, def.strainCost ?? 1);
        pc.condition.arcaneStrain = pc.attunement.arcaneStrain;
    }
    else if (def.type === "technical_technique")
        pc.attunement.industrialExposure += 2;
    ...
    if (check.outcome === "clean_success" || check.outcome === "exceptional_success" || check.outcome === "costly_success") {
        if (abilityId === "arcane.wind_weather.read_wind") {
            prepareAbilityEffect(state, abilityId);
            prepared = true;
            effect = "Wind/current reading recorded for the next navigation check.";
        }
        else if (abilityId === "arcane.sight_divination.sense_resonance")
            effect = `Resonance impression: ${interference.severity} local Arcane/Industrial interference; ${interference.symptoms.join("; ")}.`;
        else if (abilityId === "arcane.warding.personal_ward") {
            prepareAbilityEffect(state, abilityId);
            prepared = true;
            effect = "A brief personal ward is active for the next dangerous exchange.";
        }
        else if (abilityId === "tech.naval_engineering.emergency_hull_shoring" && ship) {
            if (ship.systems.hull >= ship.systems.hullMax)
                return { ok: false, message: "The hull does not currently need emergency shoring." };
            ship.systems.hull = Math.min(ship.systems.hullMax, ship.systems.hull + 6);
            ship.supplies = Math.max(0, ship.supplies - 1);
            advanceWorld(state, 2);
            effect = "Damage-control materials and two hours of crew labor stabilized part of the hull.";
        }
        else if (abilityId === "tech.instruments.calibrated_sextant_method") {
            prepareAbilityEffect(state, abilityId);
            prepared = true;
            effect = "A calibrated position fix is recorded for the next route-planning check.";
        }
        else if (abilityId === "tech.precision_weapons.precision_bore_sighting") {
            prepareAbilityEffect(state, abilityId);
            prepared = true;
            effect = "The battery is bore-sighted; the next naval firing check gains preparation.";
        }
    }
    else
        effect = def.failureProfile;
...
export function recentAbilityEffect(state, abilityId, withinHours) {
...
//# sourceMappingURL=abilitiesRuntime.js.map