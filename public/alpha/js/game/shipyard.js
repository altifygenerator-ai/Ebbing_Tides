...
import { consumeServiceResources, quoteGenericRefitService, quoteShipRepair } from "./portServices.js";
...
export function quoteRefitAtPort(state, refitId) {
...
export function repairShip(state) {
    const portId = state.player.currentPortId;
    if (!portId)
        return { ok: false, message: "You need a port and a yard crew for repairs." };
    ...
    if (state.player.character.crowns < quote.cost)
        return { ok: false, message: `This yard repair costs ${quote.cost} crowns.` };
...
export function buySupplies(state, amount = 6) {
    const portId = state.player.currentPortId;
    if (!portId)
        return { ok: false, message: "Supplies can only be purchased in port." };
    const ship = state.ships[state.player.shipId];
    if (!ship)
        return { ok: false, message: "Player ship not found." };
    const quote = quoteShipSupplies(state, portId, amount);
    if (!quote.ok)
        return { ok: false, message: quote.message ?? "The harbor cannot fill that stores order." };
    if (state.player.character.crowns < quote.cost)
        return { ok: false, message: `You need ${quote.cost} crowns.` };
    state.player.character.crowns -= quote.cost;
...
export function installRefit(state, refitId) {
    const portId = state.player.currentPortId;
    if (!portId)
        return { ok: false, message: "Refits require a shipyard." };
    const ship = state.ships[state.player.shipId];
    if (!ship)
        return { ok: false, message: "Player ship not found." };
    const refit = SHIP_REFITS[refitId];
    if (!refit)
        return { ok: false, message: "Unknown refit." };
    if (ship.refits.includes(refitId))
        return { ok: false, message: `${refit.name} is already installed.` };
    const quote = quoteRefitAtPort(state, refitId);
    if (!quote.ok)
        return { ok: false, message: quote.message };
    if (state.player.character.crowns < quote.cost)
        return { ok: false, message: `${refit.name} costs ${quote.cost} crowns at this yard.` };
    state.player.character.crowns -= quote.cost;
    consumeServiceResources(state, portId, quote.resources);
