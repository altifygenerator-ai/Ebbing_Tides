...
import { MAX_TACTICAL_RANGE_YARDS, nmToYards, pointAlongPathDistance, rangeBandFromYards, rangeChangeYards, straightLineDistanceNm, yardsToNm } from "./physicalDistance.js";
...
export function navigationHazardDamageModifier(outcome) {
    if (outcome === "exceptional_success")
        return -2;
...
export function visibilityEnvelopeFor(state, target) {
...
function maybeCreateEncounter(state) {
    ...
    if (posture.willPursue)
        encounter.log.push(`${candidate.name} changes course toward you. ${posture.summary}`);
...
function awardVoyageExperience(state, target, fromPortId) {
    ...
    for (const member of state.player.crew) {
        if (member.npcId)
            awardLifeExperience(state, member.npcId, key, `Served aboard Tideworn on voyage to ${target.name}`, 20);
    }
    recordCompletedCrewVoyage(state);
}
export function beginNavigation(state, target) {
    if (state.voyage)
        return { ok: false, message: "A voyage is already active." };
    const ship = state.ships[state.player.shipId];
    if (!ship)
        return { ok: false, message: "Player ship not found." };
    if (target.type === "port" && !state.player.knownPortIds.includes(target.id))
        return { ok: false, message: "Your character does not know a usable route to that port." };
    if (target.type === "poi" && !state.player.knownPoiIds.includes(target.id))
        return { ok: false, message: "Your character does not know enough to plot a course to that location." };
    resetSupplyHardshipIfProvisioned(state);
    const preview = plotCourse(state, target);
    if (!preview.path.length)
        return { ok: false, message: "No navigable sea route can be plotted to that destination." };
    if (preview.path.length === 1)
        return { ok: false, message: "Tideworn is already at that grid square." };
    const fromPortId = state.player.currentPortId;
    if (fromPortId && ship.supplies <= 0)
        recordUnderprovisionedDeparture(state);
    const specialist = effectiveSpecialist(state, "navigation");
    const specialistCharacter = specialist.source === "officer" ? state.npcs[specialist.characterId] : state.player.character;
    ...
    if (readWindPreparation > 0)
        consumePreparedAbilityEffect(state, "arcane.wind_weather.read_wind", "navigation_departure", ship.id);
    if (sextantPreparation > 0)
        consumePreparedAbilityEffect(state, "tech.instruments.calibrated_sextant_method", "navigation_departure", ship.id);
...
export function beginVoyage(state, destinationPortId) {
    ...
    if (!target)
        return { ok: false, message: "Unknown port." };
    return beginNavigation(state, target);
}
export function beginPoiVoyage(state, poiId) {
    const target = navigationTargetForPoi(poiId);
    if (!target || !POI_BY_ID[poiId])
        return { ok: false, message: "Unknown point of interest." };
    return beginNavigation(state, target);
}
export function beginSeaVoyage(state, x, y) {
    const target = navigationTargetForSea({ x, y });
    if (!target)
        return { ok: false, message: "That grid square is land or otherwise unnavigable." };
    return beginNavigation(state, target);
}
function ensureVoyageTelemetry(voyage, ship) {
    if (!voyage.telemetry) {
        voyage.telemetry = {
            startingSupplies: ship.supplies,
            startingCrewMorale: ship.systems.morale,
...
function voyageReport(voyage, ship) {
 ...
 */
export function estimateVoyageSupplyUnits(estimatedHours, stepHours = PLAYER_VOYAGE_SUPPLY_STEP_HOURS) {
    if (!Number.isFinite(estimatedHours) || estimatedHours <= 0)
        return 0;
    return Math.max(1, Math.ceil(estimatedHours / Math.max(1, stepHours)));
}
export function currentVoyageEtaHours(state) {
...
export function advanceVoyage(state, hours = 4) {
    ...
    if (suppliesBefore <= 0)
        applyZeroSupplyHardship(state, step);
    let weather;
    ...
    if (encounter)
        return { ok: true, message: `Sails sighted at ${yardsToNm(encounter.rangeYards).toFixed(1)} nm. Voyage interrupted.`, encounter, ...(weather ? { weather } : {}) };
...
function strategicClosingKnots(aggressor, target) {
...
function advanceEncounterMinutes(state, encounter, minutes) {
    ...
    if (afterHours > beforeHours)
        advanceWorld(state, afterHours - beforeHours);
}
function closeSightingRange(state, aggressor, target, minutes = 30) {
...
export function avoidEncounter(state) {
    ...
    if (!["failure", "severe_failure"].includes(check.outcome) || !posture.willPursue) {
        ...
        if (posture.kind === "detain")
            reportEncounterCrimes(state, encounter, other, "the player refused detention and escaped");
...
export function hailEncounter(state) {
    const encounter = state.encounter;
    if (!encounter || encounter.phase !== "sighting")
        return { ok: false, message: "No ship is in sighting/signaling range." };
    const other = state.ships[encounter.otherShipId];
    const ship = state.ships[state.player.shipId];
    if (!other || !ship)
        return { ok: false, message: "Encounter ship missing." };
...
export function submitToAuthorityEncounter(state) {
    const encounter = state.encounter;
    if (!encounter || encounter.phase !== "sighting")
        return { ok: false, message: "No lawful demand is active." };
    const other = state.ships[encounter.otherShipId];
    if (!other || other.disposition !== "navy")
        return { ok: false, message: "This vessel has no R1 authority action to answer." };
    const posture = vesselPostureTowardPlayer(state, other);
    if (posture.kind !== "detain")
        return { ok: false, message: "No active warrant requires you to heave to." };
    encounter.identified = true;
    encounter.playerIdentityKnown = true;
    encounter.authorityDemanded = true;
    ...
    if (!result.ok) {
        encounter.log.push(`${other.name} repeats the order to heave to. ${result.message}`);
        return result;
    }
    encounter.phase = "resolved";
...
export function observeEncounter(state) {
    const encounter = state.encounter;
    if (!encounter || encounter.phase !== "sighting")
        return { ok: false, message: "No vessel is under observation." };
    const other = state.ships[encounter.otherShipId];
    if (!other)
        return { ok: false, message: "Encounter ship missing." };
    const specialist = effectiveSpecialist(state, "navigation");
...
function markPlayerNavalAggression(state, encounter, other) {
    encounter.playerIdentityKnown = true;
    if (encounter.authorityDemanded)
        recordResistingAuthorityCrime(state, other, encounter);
...
export function attackEncounter(state) {
    ...
    if (!ship || !other)
        return { ok: false, message: "Encounter ship missing." };
    if (!isShipOperational(other)) {
        encounter.phase = "resolved";
        return { ok: false, message: `${other.name} is no longer an active vessel and cannot be attacked again.` };
    }
    if (encounter.rangeYards <= MAX_TACTICAL_RANGE_YARDS) {
        encounter.phase = "combat";
    ...
    let text = "";
    let guard = 24;
    while (encounter.phase === "sighting" && guard-- > 0)
        text = closeSightingRange(state, ship, other, 30);
...
export function sailUntilInterrupted(state, stepHours = PLAYER_VOYAGE_SUPPLY_STEP_HOURS, maxSteps = 240) {
    if (!state.voyage)
        return { ok: false, message: "No voyage is active.", stopReason: "no_voyage", advancedHours: 0, weatherEvents: [] };
    ...
    while (state.voyage && guard-- > 0) {
        ...
        if (result.weather)
            weatherEvents.push(result.weather);
        if (result.encounter) {
...
export function cancelVoyage(state) {
    const voyage = state.voyage;
    const ship = state.ships[state.player.shipId];
    if (!voyage || !ship)
        return { ok: false, message: "No voyage is active." };
 ...
 */
export function searchWaters(state, hours = 3) {
    const ship = state.ships[state.player.shipId];
    if (!ship)
        return { ok: false, message: "Player ship not found.", kind: "nothing", hours: 0, rangeNm: 0 };
    if (state.player.currentPortId || ship.dockedAtPortId)
        return { ok: false, message: "Search Waters is available after leaving harbor.", kind: "nothing", hours: 0, rangeNm: 0 };
    if (state.voyage)
        return { ok: false, message: "Stop the active voyage before conducting a deliberate search.", kind: "nothing", hours: 0, rangeNm: 0 };
    if (state.encounter && state.encounter.phase !== "resolved")
        return { ok: false, message: "Resolve the current contact first.", kind: "nothing", hours: 0, rangeNm: 0 };
    if (state.encounter?.phase === "resolved")
        delete state.encounter;
    ...
    if (suppliesBefore > 0 && ship.supplies === 0)
        markSupplyExhausted(state);
    if (suppliesBefore <= 0)
        applyZeroSupplyHardship(state, duration);
    const specialist = effectiveSpecialist(state, "navigation");
    const actor = specialist.source === "officer" ? state.npcs[specialist.characterId] : state.player.character;
    ...
    const kind = roll < .14 ? "wreckage" : roll < .27 ? "smoke" : roll < .44 ? "traffic" : "nothing";
