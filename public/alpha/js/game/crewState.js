...
export function ensureCrewCommunity(ship) {
    ...
    c.experience = clamp(c.experience);
    c.discipline = clamp(c.discipline);
    c.loyalty = clamp(c.loyalty);
    c.seamanship = clamp(c.seamanship);
    c.gunnery = clamp(c.gunnery);
    c.boarding = clamp(c.boarding);
    c.paySatisfaction = clamp(c.paySatisfaction);
    c.foodSatisfaction = clamp(c.foodSatisfaction);
...
export function crewMoraleLabel(value) {
    if (value >= 82)
        return "High";
    if (value >= 65)
        return "Good";
...
export function crewLoyaltyLabel(value) {
    if (value >= 82)
        return "Devoted";
    if (value >= 66)
        return "Loyal";
...
export function crewHealthLabel(value) {
    if (value >= 90)
        return "Fit";
    if (value >= 75)
        return "Worn";
...
export function crewDisciplineLabel(value) {
    if (value >= 75)
        return "Excellent";
    if (value >= 58)
        return "Disciplined";
...
export function crewUnrestProfile(state) {
    const ship = state.ships[state.player.shipId];
    if (!ship)
        return { score: 0, level: "quiet", label: "Quiet", summary: "The company has no serious grievance pressing on it.", reasons: [] };
    ...
    if (score < 12)
        return { score, level: "quiet", label: "Quiet", summary: "The company has no serious grievance pressing on it.", reasons: active };
    if (score < 25)
        return { score, level: "grumbling", label: "Grumbling", summary: "There is some grumbling, but discipline is holding.", reasons: active };
    if (score < 40)
        return { score, level: "discontented", label: "Discontented", summary: "The company is openly unhappy and may lose sailors in port if conditions do not improve.", reasons: active };
...
export function crewActionModifier(state, domain) {
    const ship = state.ships[state.player.shipId];
    if (!ship)
        return 0;
    const c = ensureCrewCommunity(ship);
...
export function effectivePlayerVoyageSeamanship(state, specialistRating) {
...
export function crewSummary(state) {
    const ship = state.ships[state.player.shipId];
    if (!ship)
        return { morale: "—", loyalty: "—", experience: "—", health: "—", discipline: "—", ordinary: 0, unrest: "—", unrestLevel: "quiet" };
    const c = ensureCrewCommunity(ship);
...
//# sourceMappingURL=crewState.js.map