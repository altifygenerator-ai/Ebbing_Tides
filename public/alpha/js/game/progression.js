...
const SIGNIFICANCE_XP = { minor: 20, meaningful: 50, major: 100, historic: 200 };
/**
...
function pruneCadence(entries, currentHour, windowHours) {
...
function cadenceUse(entries, key, currentHour, windowHours, maxRememberedUses) {
    ...
    if (row.useHours.length > maxRememberedUses)
        row.useHours.splice(0, row.useHours.length - maxRememberedUses);
    return { entries: next, repeats };
}
export function normalizeProgressionCadence(character, currentHour) {
...
export function experienceThresholdForLevel(level) {
    const n = Math.max(0, Math.floor(level) - 1);
    return Math.round(250 * n + 75 * n * n);
}
export function startingAdvancement(level = 3) {
...
export function startingLevelFromAge(age) {
    if (age <= 22)
        return 2;
    if (age <= 31)
        return 3;
...
function characterFor(state, characterId) {
    return characterId === state.player.character.id ? state.player.character : state.npcs[characterId];
}
function pushAdvancementHistory(character, entry) {
    character.advancement.history.push({ ...entry, id: `adv.${entry.kind}.${entry.atHour}.${character.advancement.history.length}` });
    if (character.advancement.history.length > 120)
        character.advancement.history.splice(0, character.advancement.history.length - 120);
}
function processLevelUps(state, characterId) {
    ...
    while (character.advancement.lifeExperience >= experienceThresholdForLevel(character.advancement.level + 1)) {
        ...
        if (character.advancement.level % 3 === 0)
            character.advancement.generalPerkPoints += 1;
...
export function awardLifeExperience(state, characterId, key, source, significance) {
    const character = characterFor(state, characterId);
    if (!character)
...
function practiceThreshold(rating) { return 8 + Math.floor(rating / 10) * 3; }
export function skillProgressPercent(character, skillId) {
...
export function recordMeaningfulPractice(state, characterId, skillId, practiceKey, difficulty) {
    const character = characterFor(state, characterId);
    if (!character)
        return { gained: 0, newRating: 0 };
...
export function canFocusSkill(character, skillId, currentHour) {
    if (character.advancement.developmentPoints <= 0)
        return { ok: false, reason: "No Development Points available." };
    if (character.skills[skillId] >= 100)
        return { ok: false, reason: "This skill cannot improve further." };
    ...
    if (!recent && !trained && !formative)
        return { ok: false, reason: "Focus requires recent meaningful use, relevant training, or a formative core-skill foundation." };
    return { ok: true, reason: "Eligible for focused development." };
}
export function spendDevelopmentPointOnSkill(state, skillId) {
    const character = state.player.character;
    const eligible = canFocusSkill(character, skillId, state.absoluteHour);
    if (!eligible.ok)
        return { ok: false, message: eligible.reason };
    character.advancement.developmentPoints -= 1;
    const practice = character.practice[skillId] ?? { skillId, progress: 0, recentPracticeKeys: [] };
    character.practice[skillId] = practice;
    const before = character.skills[skillId];
    const threshold = practiceThreshold(before);
    practice.progress += Math.max(3, Math.round(threshold * .55 * 10) / 10);
    let improved = false;
    if (practice.progress >= threshold && before < 100) {
        practice.progress -= threshold;
        character.skills[skillId] = before + 1;
        improved = true;
        pushAdvancementHistory(character, { kind: "skill_rank", atHour: state.absoluteHour, source: "Focused development", detail: `${skillId} improved to ${before + 1} after focused development grounded in prior experience.`, skillId });
    }
...
export function availableGeneralPerks(character) {
...
export function hasGeneralPerk(character, perkId) {
    return Boolean(character?.advancement.generalPerks.includes(perkId));
}
export function generalPerkModifier(character, context) {
    if (!character)
...
export function experienceLevelTitle(level) {
    if (level <= 2)
        return "Green Captain";
    if (level <= 4)
        return "Seasoned Captain";
    if (level <= 6)
        return "Experienced Captain";
    if (level <= 8)
        return "Veteran Captain";
    if (level <= 10)
        return "Renowned Captain";
    return "Historic Captain";
}
export function takeGeneralPerk(state, perkId) {
    const character = state.player.character;
    if (character.advancement.generalPerkPoints <= 0)
        return { ok: false, message: "No General Perk opportunity is available." };
    const perk = availableGeneralPerks(character).find(row => row.id === perkId);
    if (!perk)
        return { ok: false, message: "That perk is not currently available from this character's lived experience." };
    character.advancement.generalPerkPoints -= 1;
    character.advancement.generalPerks.push(perk.id);
...
export function recordTraining(state, characterId, entry) {
    const character = characterFor(state, characterId);
    if (!character)
        return;
    character.trainingHistory.push({ ...entry, id: `training.${characterId}.${entry.subjectId}.${entry.completedAtHour}` });
}
export function learnAbilityFromSource(state, characterId, abilityId, source, acquisition = {}) {
    const character = characterFor(state, characterId);
    const definition = ABILITY_BY_ID[abilityId];
    if (!character || !definition)
        return { ok: false, message: "Unknown character or ability." };
    if (character.abilities.some(row => row.abilityId === abilityId))
        return { ok: false, message: `${definition.name} is already known.` };
    if (definition.requiredSkillId && character.skills[definition.requiredSkillId] < (definition.requiredSkillRating ?? 0))
        return { ok: false, message: `Training requires ${definition.requiredSkillRating ?? 0} ${definition.requiredSkillId}.` };
...
export function learnSpecializationFromSource(state, characterId, skillId, name, source, rating = 3, acquisition = {}) {
    const character = characterFor(state, characterId);
    if (!character)
        return { ok: false, message: "Unknown character." };
    if (character.specializations.some(row => row.skillId === skillId && row.name === name))
        return { ok: false, message: "Specialization already known." };
    character.specializations.push({ id: `specialization.${characterId}.${skillId}.${name.toLowerCase().replaceAll(" ", "_")}`, skillId, name, rating, source, learnedAtHour: state.absoluteHour });
    refreshDerivedAttunement(character);
    recordTraining(state, characterId, { subjectType: "specialization", subjectId: `${skillId}:${name}`, teacherOrSource: source, startedAtHour: acquisition.startedAtHour ?? state.absoluteHour, completedAtHour: state.absoluteHour, costCrowns: acquisition.costCrowns ?? 0, result: "learned" });
    awardLifeExperience(state, characterId, `specialization:${skillId}:${name}`, `Learned ${name} specialization from ${source}`, "meaningful");
    state.worldEvents.push({ id: `event.learn.${characterId}.${skillId}.${name.toLowerCase().replaceAll(" ", "_")}.${state.absoluteHour}`, type: "character_learning", atHour: state.absoluteHour, participants: [characterId], summary: `${"name" in character ? character.name : characterId} learned ${name} (${skillId}) from ${source}.`, canonicalData: { characterId, skillId, specialization: name, source, ...(acquisition.sourceId ? { sourceId: acquisition.sourceId } : {}), trainingStartedAtHour: acquisition.startedAtHour ?? state.absoluteHour, costCrowns: acquisition.costCrowns ?? 0 }, importance: 1 });
    return { ok: true, message: `Learned ${name} specialization.` };
}
