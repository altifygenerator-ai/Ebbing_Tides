...
function rememberNamedCrew(state, eventId) {
    for (const member of state.player.crew) {
        if (!member.npcId)
            continue;
        const npc = state.npcs[member.npcId];
        if (npc && !npc.brain.memories.includes(eventId))
            npc.brain.memories.push(eventId);
    }
}
function pushCrewEvent(state, type, summary, canonicalData, importance = 1) {
    ...
    if (c && !c.historyTags.includes(id))
        c.historyTags.push(id);
...
function offerBand(unit, portId, index) {
...
export function crewRecruitOffers(state) {
...
export function recruitCrewOffer(state, offerId) {
    const portId = state.player.currentPortId;
    if (!portId)
        return { ok: false, message: "Recruiting requires a port." };
    const ship = state.ships[state.player.shipId];
    if (!ship)
        return { ok: false, message: "Player ship not found." };
    if (ship.systems.crew >= ship.systems.crewMax)
        return { ok: false, message: `${ship.name} has no open crew berth.` };
    const offer = crewRecruitOffers(state).find(row => row.id === offerId);
    if (!offer)
        return { ok: false, message: "That sailor is no longer available." };
    if (state.player.character.crowns < offer.signing)
        return { ok: false, message: `The ${offer.label.toLowerCase()} wants ${offer.signing} crowns to sign on.` };
    const c = ensureCrewCommunity(ship);
    const ordinaryBefore = ordinaryCrewCount(state);
    const weight = Math.max(1, ordinaryBefore);
    const mix = (current, incoming) => ordinaryBefore <= 0 ? incoming : (current * weight + incoming) / (weight + 1);
...
export function shoreLeaveCost(state) {
    const ship = state.ships[state.player.shipId];
    return ship ? Math.max(10, Math.ceil(ship.systems.crew * 1.5)) : 12;
}
export function takeCrewShoreLeave(state) {
    const portId = state.player.currentPortId;
    if (!portId)
        return { ok: false, message: "You need to be in port to stand the crew down." };
    const ship = state.ships[state.player.shipId];
    if (!ship)
        return { ok: false, message: "Player ship not found." };
    const cost = shoreLeaveCost(state);
    if (state.player.character.crowns < cost)
        return { ok: false, message: `Food, bunks, and shore leave cost ${cost} crowns for the company.` };
    ...
    for (const member of state.player.crew) {
        member.morale = clamp(member.morale + Math.max(3, moraleGain - 2));
        member.loyalty = clamp(member.loyalty + 1);
        member.health = clamp((member.health ?? 100) + 1);
    }
...
export function recordPrizeForCrew(state, prizeValue) {
    const ship = state.ships[state.player.shipId];
    if (!ship || prizeValue <= 0)
        return;
    const c = ensureCrewCommunity(ship);
...
export function sharePrizeWithCrew(state) {
    const ship = state.ships[state.player.shipId];
    if (!ship)
        return { ok: false, message: "Player ship not found." };
    const c = ensureCrewCommunity(ship);
    ...
    if (due <= 0)
        return { ok: false, message: "There is no unsettled prize share to divide." };
    if (state.player.character.crowns < due)
        return { ok: false, message: `The company is due ${due} crowns, but you do not have enough on hand.` };
    ...
    for (const member of state.player.crew) {
        member.morale = clamp(member.morale + 4);
...
export function recordCrewCasualties(state, count, source) {
    if (count <= 0)
        return;
    const ship = state.ships[state.player.shipId];
    if (!ship)
...
export function recordDangerousCrewOrder(state, reason, severity = 1) {
    const ship = state.ships[state.player.shipId];
    if (!ship)
        return;
    const c = ensureCrewCommunity(ship);
...
export function recordUnderprovisionedDeparture(state) {
    const ship = state.ships[state.player.shipId];
...
export function recordCompletedCrewVoyage(state) {
    const ship = state.ships[state.player.shipId];
    if (!ship)
        return;
    const c = ensureCrewCommunity(ship);
    ...
    if (c.outstandingPrizeShare > 0) {
        c.paySatisfaction = clamp(c.paySatisfaction - 2);
        if (c.paySatisfaction < 35)
            c.loyalty = clamp(c.loyalty - 1);
    }
}
export function checkTavernDesertion(state) {
    const portId = state.player.currentPortId;
    const ship = state.ships[state.player.shipId];
    if (!portId || !ship)
        return { ok: true, message: "", left: 0 };
    const c = ensureCrewCommunity(ship);
    const checkKey = `${portId}:${Math.floor(state.absoluteHour / 24)}`;
    if (c.lastDesertionCheckKey === checkKey)
        return { ok: true, message: "", left: 0 };
...
//# sourceMappingURL=crewMechanics.js.map