...
export function buildCharacterMindContext(state, npcId) {
    const npc = state.npcs[npcId];
    if (!npc)
        throw new Error(`Unknown NPC ${npcId}`);
    const currentWorldFacts = [];
    if (npc.locationPortId === "port.ironhaven") {
        const market = state.markets["port.ironhaven"];
        const grain = market?.goods["good.grain"];
        if (grain)
            currentWorldFacts.push(`Ironhaven grain stock is ${grain.stock} against a target of ${grain.targetStock}.`);
    }
    ...
    if (crewAssignment) {
        ...
        if (ship) {
            ...
            currentWorldFacts.push(`Aboard Tideworn: hull ${ship.systems.hull}/${ship.systems.hullMax}, sails ${ship.systems.sails}/${ship.systems.sailsMax}, crew morale ${crewMoraleLabel(ship.systems.morale)}, company loyalty ${crewLoyaltyLabel(company.loyalty)}, supplies ${ship.supplies}, unsettled prize share ${company.outstandingPrizeShare} crowns.`);
        }
        ...
        if (state.voyage)
            currentWorldFacts.push(`Tideworn is currently sailing toward ${state.voyage.destination.name}; ${Math.round(state.voyage.progress * 100)} percent of the planned voyage is complete.`);
    }
    const remembered = npc.brain.memories.slice(-3).map((eventId) => state.worldEvents.find((event) => event.id === eventId)?.summary).filter((summary) => Boolean(summary));
    for (const summary of remembered)
        currentWorldFacts.push(`Remembered event: ${summary}`);
...
function ironhavenFoodPressure(state) {
    const grain = state.markets["port.ironhaven"]?.goods["good.grain"];
    if (!grain || grain.targetStock <= 0)
        return "steady";
...
function korrFoodPressureText(state) {
...
function classifyIntent(text) {
    ...
    if (/work|job|help|need|contract/.test(t))
        return "ask_for_work";
    if (/grain|food|hungry|shortage|market/.test(t))
        return "ask_about_food_pressure";
    if (/omen|storm[- ]born|born.*storm|born.*tide|high tide|first snow|birth sign/.test(t))
        return "ask_about_omen";
    if (/covenant|ilyon|faith|religion|old gods|thoren|veyr/.test(t))
        return "ask_about_religion";
    if (/braeg|factory|foundry|industry|machine/.test(t))
        return "ask_about_industry";
    if (/morale|crew|loyal|happy|complain|problem/.test(t))
        return "ask_about_crew";
    if (/ship|hull|sail|rig|damage|repair|tideworn/.test(t))
        return "ask_about_ship";
    if (/voyage|course|route|where|weather|sea|heading/.test(t))
        return "ask_about_voyage";
    if (/what do you think|advice|opinion|recommend/.test(t))
        return "ask_advice";
    if (/who are you|yourself|name/.test(t))
        return "ask_identity";
    return "general_conversation";
}
export function deterministicCharacterMindReply(state, npcId, playerText) {
    const npc = state.npcs[npcId];
    if (!npc)
        throw new Error(`Unknown NPC ${npcId}`);
    ...
    if (npc.id === "character.pastor_elias_korr") {
        switch (intent) {
            case "ask_for_work": {
                ...
                else if (pc.recentProfession === "healer" || pc.skills.medicine >= 42)
                    text = "If your medical training is real, the hospital can use hands and medicine more than speeches. I can introduce you to the quartermaster and let you see what is actually running short.";
                else
                    text = pressure === "short" || pressure === "tight"
            ...
            case "ask_about_religion":
                if (state.player.character.religion === "covenant")
                    text = "Then you already know the Covenant is not one voice. I serve it here by keeping people fed and treated. If your faith means anything, let it make you useful before it makes you loud.";
                else if (state.player.character.religion === "old_gods")
                    text = "You keep the old rites; I serve the Covenant. That need not make us enemies. Ironhaven has enough sparks without priests or captains carrying tinder.";
                else
        ...
        if (crew) {
            if (intent === "ask_identity")
                text = `"${npc.name}. ${npc.role}. You know the title already; the rest you learn by sharing a deck with me."`;
                ...
                if (!ship || !company)
                    text = '"I cannot tell you what is not in front of me."';
                else if (company.outstandingPrizeShare > 0)
                    text = `"They know there's ${company.outstandingPrizeShare} crowns of prize share still unsettled. You can ask men to wait, but don't ask them to forget."`;
                else if (ship.systems.morale < 40 || company.loyalty < 35)
                    text = `"The company is wearing thin. Food, rest, fair shares, and whether they trust the next order all matter now."`;
                else if (ship.systems.morale < 65 || company.loyalty < 50)
                    text = `"They're holding together. Give them food, a little shore time, and reasons to believe the captain notices what they endure."`;
                else
                    text = `"The company is in good heart. ${crewMoraleLabel(ship.systems.morale)} morale, ${crewLoyaltyLabel(company.loyalty).toLowerCase()} loyalty. Keep earning it."`;
            }
            else if (intent === "ask_about_ship") {
                if (!ship)
                    text = '"I cannot tell you what is not in front of me."';
                else if (npc.id === "character.elsa_tarn")
                    text = `"Hull is ${ship.systems.hull} of ${ship.systems.hullMax}; sails ${ship.systems.sails} of ${ship.systems.sailsMax}. She is serviceable, but numbers do not replace listening to timber and rigging."`;
                else if (npc.id === "character.ulf_brenn")
                    text = `"Battery is what it is: ${ship.firepower} effective firepower, crew morale ${ship.systems.morale}. Give me dry powder, time to drill, and no fool standing over the magazine with a lamp."`;
                else
                    text = `"Tideworn is holding. Hull ${ship.systems.hull} of ${ship.systems.hullMax}, supplies ${ship.supplies}. If you want the fine details, ask the person whose hands are on that part of her."`;
            }
            else if (intent === "ask_about_voyage") {
                if (npc.id === "character.nils_orr")
                    text = state.voyage ? `"Course is laid for ${state.voyage.destination.name}. We're about ${Math.round(state.voyage.progress * 100)} percent through it. A chart tells us where we meant to go; the sea still gets a vote."` : '"No active course. Give me a destination and what we actually know of it, and I can tell you what I trust in the chart."';
                else
                    text = state.voyage ? `"We're committed to ${state.voyage.destination.name} for now. If weather, supplies, or trouble changes the facts, then we change the plan."` : '"We are not under way. That is the best time to decide what kind of trouble is worth sailing toward."';
            }
            else if (intent === "ask_advice") {
                if (npc.id === "character.mira_holst")
                    text = '"Keep enough margin that one bad hour does not decide the whole voyage. Food, crew, hull, money. Captains usually lose ships after they convince themselves one of those can wait."';
                else if (npc.id === "character.nils_orr")
                    text = '"Do not confuse a confident line on a chart with knowledge. Check the source, the age, the weather, and what the person drawing it could actually have seen."';
                else if (npc.id === "character.ulf_brenn")
                    text = '"Do not spend powder proving you own guns. Fire when it changes the fight."';
                else if (npc.id === "character.elsa_tarn")
                    text = '"Repair the small failure before the sea turns it into the large one."';
                else
                    text = `${npc.name} considers it. "Ask me about the part of the world I actually know, and my advice will be worth more."`;
            }
            else {
                ...
                if (shortageMemory)
                    text = `"I'm listening. And yes, I remember the stores running dry. We got through it, but the crew will remember whether that becomes a habit. Ask me about the ship, the crew, or the next voyage."`;
                else
                    text = `"I'm listening. Ask me about the ship, the crew, the voyage, or something I might actually know."`;
            }
        }
        else {
    ...
    let respectGain = 0;
    if (npc.id === "character.pastor_elias_korr") {
        if (pc.religion === "covenant")
            trustGain += 1;
        if (pc.recentProfession === "healer" || pc.skills.medicine >= 42 || pc.skills.persuasion >= 45)
            respectGain += 2;
        if (pc.recentProfession === "merchant_clerk" || pc.skills.commerce >= 45)
            respectGain += 1;
    }
    const crew = state.player.crew.find(member => member.npcId === npc.id);
    if (crew && (pc.skills.command >= 45 || pc.attributes.presence >= 7))
        respectGain += 1;
    ...
    if (trustGain > 1 || respectGain > 0)
        npc.relationshipToPlayer.suspicion = Math.max(0, npc.relationshipToPlayer.suspicion - 1);
    return {
        text,
