...
export function gridSeparationCells(a, b) {
    return Math.hypot(b.x - a.x, b.y - a.y);
}
export function straightLineDistanceNm(a, b) {
...
export function nmToYards(nm) {
    return nm * YARDS_PER_NM;
}
export function yardsToNm(yards) {
    return yards / YARDS_PER_NM;
}
/** Derived UX label. Exact rangeYards remains authoritative. */
export function rangeBandFromYards(rangeYards, boarding = false) {
    if (boarding)
        return "boarding";
...
export function legacyRangeSeedYards(range) {
    switch (range) {
...
export function rangeChangeYards(relativeClosingKnots, roundMinutes = TACTICAL_ROUND_MINUTES) {
 ...
 */
export function pointAlongPathDistance(path, distanceTravelledNm) {
    if (!path.length)
        return { x: 0, y: 0 };
    if (path.length === 1)
        return { ...path[0] };
...
export function distanceAlongPathForPoint(path, point) {
    if (path.length < 2)
        return 0;
...
//# sourceMappingURL=physicalDistance.js.map