import { armorDefense, firearmAttack, meleeApCost, meleeAttack } from "./inventory.js";
...
const BODY_PARTS = ["torso", "left_arm", "right_arm", "left_leg", "right_leg", "head"];
const INJURY_TYPES = ["cut", "puncture", "bruise", "fracture", "concussion"];
...
function stanceAttack(stance) { return stance === "aggressive" ? 2 : stance === "defensive" ? -2 : 0; }
function stanceDefense(stance) { return stance === "defensive" ? 3 : stance === "aggressive" ? -2 : 0; }
function injuryPenalty(state) { return state.player.injuries.filter((i) => !i.treated).reduce((sum, i) => sum + i.severity, 0); }
function addInjury(state, source, severity, key) {
...
function playerHealthMax(state) {
    return 28 + state.player.character.attributes.might * 2 + state.player.character.attributes.will;
}
export function beginDeckDrill(state) {
    if (!state.player.currentPortId)
        return { ok: false, message: "Deck drills require the ship to be safely in port in this alpha." };
    if (state.personalCombat && !state.personalCombat.resolved)
        return { ok: false, message: "A personal combat is already active." };
...
export function beginBoardingCombat(state) {
    const encounter = state.encounter;
    if (!encounter || encounter.phase !== "combat" || !encounter.shipsSecured)
        return { ok: false, message: "You must have the enemy grappled and secured before boarding." };
    const ship = state.ships[encounter.otherShipId];
    if (!ship)
        return { ok: false, message: "Enemy ship not found." };
    if (!isShipOperational(ship))
        return { ok: false, message: `${ship.name} has already been resolved as a ${ship.lifecycle?.status ?? "terminal"} vessel.` };
...
function resolvePlayerHit(state, combat, kind) {
    const actionIndex = combat.log.length;
    const pc = state.player.character;
    const skillId = kind === "pistol" ? "firearms" : "blades";
    const attributeId = kind === "pistol" ? "perception" : "agility";
    ...
    if (["failure", "severe_failure"].includes(check.outcome))
        return `${kind === "pistol" ? "The pistol shot" : "Your attack"} fails to land cleanly (${check.roll} vs ${check.chance}).`;
    const weapon = kind === "pistol" ? firearmAttack(state) : meleeAttack(state);
    const damage = Math.max(2, Math.floor(weapon + Math.max(0, check.margin) / 12));
    combat.opponentHealth = clamp(combat.opponentHealth - damage, 0, combat.opponentHealthMax);
    return `${kind === "pistol" ? "Your pistol cracks at arm's length" : "Your blade lands"} for ${damage} damage (${check.outcome}).`;
}
function resolveEnemyTurn(state, combat) {
    const opponent = state.npcs[combat.opponentId];
    const skill = opponent?.skills.blades ?? combat.opponentSkill;
    const agility = opponent?.attributes.agility ?? 5;
    ...
    if (wardBonus > 0)
        consumePreparedAbilityEffect(state, "arcane.warding.personal_ward", "personal_defense", state.player.character.id);
    ...
    if (["failure", "severe_failure"].includes(check.outcome))
        return `${combat.opponentName} attacks, but you turn or evade the blow (${check.outcome}).`;
    const damage = Math.max(2, 4 + Math.floor(Math.max(0, check.margin) / 15));
    combat.playerHealth = clamp(combat.playerHealth - damage, 0, combat.playerHealthMax);
    let text = `${combat.opponentName} lands a hit for ${damage} damage.`;
    if (combat.source === "boarding" && (damage >= 6 || check.margin >= 18)) {
        const severity = damage >= 9 ? 3 : damage >= 7 ? 2 : 1;
        const injury = addInjury(state, `Boarding ${state.ships[state.encounter?.otherShipId ?? ""]?.name ?? "combat"}`, severity, `${combat.id}:${combat.round}:injury`);
        text += ` You suffer a ${injury.type} to the ${injury.bodyPart.replaceAll("_", " ")}.`;
    }
    return text;
}
function recentRespectfulDeckDrill(state, firstMateId) {
    return state.worldEvents.some(event => event.type === "deck_drill_completed" &&
...
function resolveDeckDrill(state, combat, playerWon) {
    combat.outcome = "drill_complete";
    if (!playerWon)
        combat.playerHealth = Math.max(1, combat.playerHealth);
...
function resolveOutcome(state, combat) {
    if (combat.opponentHealth <= 0) {
        combat.resolved = true;
        if (combat.source === "deck_drill")
            return resolveDeckDrill(state, combat, true);
        ...
        if (encounter && enemy) {
            ...
            if (!resolution.ok)
                return `${enemy.name} was already resolved; no second prize can be claimed.`;
    ...
    if (combat.playerHealth <= 0) {
        combat.resolved = true;
        if (combat.source === "deck_drill")
            return resolveDeckDrill(state, combat, false);
        combat.outcome = "defeat";
        const encounter = state.encounter;
        const enemy = encounter ? state.ships[encounter.otherShipId] : undefined;
        if (encounter && enemy)
            reportEncounterCrimes(state, encounter, enemy, "the defending vessel survived the failed boarding attempt");
        if (encounter)
...
export function personalCombatAction(state, action) {
    const combat = state.personalCombat;
    if (!combat || combat.resolved)
        return { ok: false, message: "No active personal combat." };
    let text = "";
    if (action.startsWith("stance_")) {
        const stance = action.replace("stance_", "");
        if (combat.playerAP < 1)
            return { ok: false, message: "Not enough AP." };
        combat.playerAP -= 1;
        ...
        if (combat.playerAP < cost)
            return { ok: false, message: `You need ${cost} AP for your equipped melee weapon.` };
        combat.playerAP -= cost;
        text = resolvePlayerHit(state, combat, "melee");
    }
    else if (action === "pistol") {
        if (!firearmAttack(state))
            return { ok: false, message: "No firearm is equipped in either hand." };
        if (!combat.pistolLoaded)
            return { ok: false, message: "Your pistol is not loaded." };
        if (combat.playerAP < 4)
            return { ok: false, message: "You need 4 AP to fire." };
        combat.playerAP -= 4;
        combat.pistolLoaded = false;
        text = resolvePlayerHit(state, combat, "pistol");
    }
    else if (action === "reload") {
        if (combat.playerAP < 3)
            return { ok: false, message: "You need 3 AP to reload under pressure." };
        if (combat.pistolLoaded)
            return { ok: false, message: "The pistol is already loaded." };
        combat.playerAP -= 3;
        ...
        if (combat.playerAP < 2)
            return { ok: false, message: "You need 2 AP to settle into a guarded defense." };
        combat.playerAP -= 2;
...
export function treatInjuries(state) {
    const portId = state.player.currentPortId;
    if (!portId)
        return { ok: false, message: "You need a port physician or healer in this alpha." };
    const quote = quoteMedicalTreatment(state, portId);
    if (!quote.ok)
        return { ok: false, message: quote.message };
    if (state.player.character.crowns < quote.cost)
        return { ok: false, message: `Treatment costs ${quote.cost} crowns here.` };
...
//# sourceMappingURL=personalCombat.js.map