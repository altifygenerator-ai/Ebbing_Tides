...
function renderCreation() {
  ...
  `).join("");
    ...
    <main class="creation">
      <section class="creation-box">
        ...
        <form id="creation-form" class="creation-body">
          ...
          <div class="attr-grid">
            ${["strength", "dexterity", "intelligence", "willpower", "charisma", "perception"].map((attr) => `
    ...
    const updateCounters = () => {
        const attrs = ["strength", "dexterity", "intelligence", "willpower", "charisma", "perception"]
            .map((attr) => Number(form.elements.namedItem(`attr_${attr}`)?.value ?? 0));
        const total = attrs.reduce((sum, value) => sum + value, 0);
        const attrEl = document.querySelector("#attr-total");
        if (attrEl) {
            attrEl.textContent = `${total} / 36`;
            attrEl.className = `small ${total > 36 ? "shortage" : "muted"}`;
        }
        const count = form.querySelectorAll('input[name="coreSkill"]:checked').length;
        const skillEl = document.querySelector("#skill-count");
        if (skillEl) {
            skillEl.textContent = `${count} / 5`;
            skillEl.className = `small ${count !== 5 ? "shortage" : "muted"}`;
        }
    };
    form.addEventListener("input", updateCounters);
    updateCounters();
}
function creationFromForm(form) {
    ...
    const coreSkills = fd.getAll("coreSkill").map(String);
    return {
        name: String(fd.get("name") ?? "").trim(),
        age: Number(fd.get("age")),
        homePortId: String(fd.get("homePortId")),
        socialOrigin: String(fd.get("socialOrigin")),
        background: String(fd.get("background")),
        religion: String(fd.get("religion")),
        devotion: String(fd.get("devotion")),
        attributes: attrs,
        coreSkills,
        trait: String(fd.get("trait")),
        birthOmen: String(fd.get("birthOmen")),
        recentProfession: String(fd.get("recentProfession")),
        shipOrigin: String(fd.get("shipOrigin")),
...
function renderNav() {
    const items = [["harbor", "Harbor / Ship"], ["market", "Market"], ["people", "People"], ["journal", "Journal"], ["chart", "Navigation Map"]];
    return `<nav class="navrail">${items.map(([id, label]) => `<button data-tab="${id}" class="${tab === id ? "active" : ""}">${label}</button>`).join("")}</nav>`;
}
function portArtStyle(portId) {
...
function pctX(x) { return (x / 12) * 100; }
function pctY(y) { return (y / 8) * 100; }
...
function renderGame() {
    ...
    let content;
    if (state.voyage || tab === "chart" || !state.player.currentPortId)
...
function handleClick(target) {
    ...
    if (combatEl && state) {
        const result = combatAction(state, combatEl.dataset.combat);
    ...
    switch (action) {
        ...
        case "accept-contract": {
            const result = acceptContract(s, String(actionEl.dataset.id));
            toast(result.message);
            renderGame();
            return;
        }
        case "fulfill-contract": {
            const result = fulfillContract(s, String(actionEl.dataset.id));
            toast(result.message);
            renderGame();
            return;
        }
        case "select-destination":
            selectedDestination = String(actionEl.dataset.id);
            renderGame();
            return;
        case "begin-voyage": {
            const result = beginVoyage(s, selectedDestination);
            toast(result.message);
            if (result.ok)
                tab = "chart";
            renderGame();
        ...
        case "advance-voyage": {
            const result = advanceVoyage(s, 4);
            toast(result.weather ? `${result.message} ${result.weather}` : result.message);
            if (result.arrived)
                tab = "harbor";
            renderGame();
            return;
        ...
        case "hail": {
            const result = hailEncounter(s);
            toast(result.message);
            renderGame();
            return;
        }
        case "avoid": {
            const result = avoidEncounter(s);
            toast(result.message);
            renderGame();
            return;
        }
        case "attack": {
            const result = attackEncounter(s);
            toast(result.message);
            renderGame();
            return;
        }
        case "resume-voyage":
            ...
            return;
        case "open-dialogue":
            document.querySelector("#dialogue-panel")?.scrollIntoView({ behavior: "smooth" });
            return;
        default:
            return;
    }
