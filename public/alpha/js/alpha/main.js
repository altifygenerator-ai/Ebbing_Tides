...
import { attackEncounter, avoidEncounter, beginNavigation, cancelVoyage, currentVoyageEtaHours, estimateVoyageSupplyUnits, hailEncounter, observeEncounter, PLAYER_VOYAGE_SUPPLY_STEP_HOURS, sailUntilInterrupted, searchWaters, submitToAuthorityEncounter } from "../game/travel.js";
...
function cueAfter(name, delayMs) {
    window.setTimeout(() => cue(name), Math.max(0, delayMs));
}
function navalResolutionCueFromNewEvents(s, beforeEventCount, delayMs) {
    ...
    if (result === "player_victory") {
        cueAfter("naval_victory", delayMs);
        return true;
    }
    if (result === "player_defeat") {
        cueAfter("naval_defeat", delayMs);
        return true;
    }
    return false;
}
function playNavalCombatAudio(s, action, before, result) {
    if (!result.ok) {
        cue("ui");
        return;
    }
    ...
    if (action === "fire_hull") {
        cue("cannon_round");
        tailDelay = 520;
        if (enemy.systems.hull < before.enemyHull)
            cueAfter("hull_impact", 180);
    }
    else if (action === "fire_rigging") {
        cue("cannon_chain");
        tailDelay = 540;
        if (enemy.systems.sails < before.enemySails || enemy.systems.rigging < before.enemyRigging)
            cueAfter("rigging_impact", 190);
    }
        ...
        if (!before.shipsSecured && e.shipsSecured)
            cue("grapple");
        else
            cue("naval_maneuver");
        tailDelay = 390;
        ...
        if (before.enemyMorale > 0 && enemy.systems.morale <= 0)
            cue("surrender");
        else
            cue("ui");
        tailDelay = 420;
    ...
    if (enemyFired) {
        ...
        if (playerTookHullDamage)
            cueAfter("hull_impact", fireDelay + 180);
...
function renderChart(s) {
    ...
    const lod = cameraLod(box.width);
    const playerVisualSet = shipCombatVisualSet(ship.classId, ship.name);
    const playerDirection = shipDirectionForRoute(trail, ship.position);
    const preferredPlayerToken = shipTokenAssetId(playerVisualSet, playerDirection);
    const playerTokenFallback = ship.tokenAssetId;
...
function renderEncounter(s) {
    ...
    const playerVisualSet = shipCombatVisualSet(player.classId, player.name);
    const otherVisualSet = shipCombatVisualSet(other.classId, other.name);
    const playerVisualState = shipHullVisualState(player.systems.hull, player.systems.hullMax);
    const otherVisualState = shipHullVisualState(other.systems.hull, other.systems.hullMax);
    const playerCombatArtId = shipCombatAssetId(playerVisualSet, playerVisualState);
    const otherCombatArtId = shipCombatAssetId(otherVisualSet, otherVisualState);
    ...
    const playerTokenId = shipTokenAssetId(playerVisualSet, "east");
    const otherTokenId = shipTokenAssetId(otherVisualSet, "west");
    const playerToken = playerTokenId ? ASSET_BY_ID[playerTokenId]?.path : undefined;
    const otherToken = otherTokenId ? ASSET_BY_ID[otherTokenId]?.path : undefined;
...
function encounterResolutionEvent(s, encounterId) { return [...s.worldEvents].reverse().find(event => event.id.endsWith(encounterId) || event.canonicalData.encounterId === encounterId); }
function renderNavalResolution(s) {
    const e = s.encounter, player = getPlayerShip(s), other = s.ships[e.otherShipId];
    const lifecycle = other.lifecycle;
    ...
    const outcome = victory ? "victory" : defeat ? "defeat" : e.playerEscaped ? "disengaged" : "resolved";
    const title = victory ? "Victory at Sea" : defeat ? "The Action Is Lost" : e.playerEscaped ? "Contact Broken" : "Contact Resolved";
    const event = encounterResolutionEvent(s, e.id), summary = event?.summary ?? e.log.at(-1) ?? "The encounter has ended.";
    const focus = defeat ? player : other;
    const focusSet = shipCombatVisualSet(focus.classId, focus.name), focusState = victory && lifecycle?.status === "disabled" ? "wrecked" : shipHullVisualState(focus.systems.hull, focus.systems.hullMax);
    const focusArtId = shipCombatAssetId(focusSet, focusState), focusArt = (focusArtId ? ASSET_BY_ID[focusArtId]?.path : undefined) ?? ASSET_BY_ID[focus.artAssetId ?? ""]?.path;
...
function renderGame() {
    ...
    if (state.personalCombat)
        ...
        app.innerHTML = `<div class="shell">${renderTopBar(state)}${state.encounter.phase === "resolved" ? renderNavalResolution(state) : renderEncounter(state)}</div>`;
...
function playerMapTokenPath(s, ship, path) {
...
function handleClick(target) {
    ...
    if (personalEl && state) {
        ...
        const eventCount = state.worldEvents.length;
        const result = personalCombatAction(state, action);
        handleResult(result, sound);
        if (result.ok)
            navalResolutionCueFromNewEvents(state, eventCount, 520);
        return;
    }
    if (combatEl && state) {
        const action = d(combatEl, "combat");
        const audioBefore = navalAudioSnapshot(state);
        ...
        playNavalCombatAudio(state, action, audioBefore, result);
        resolveEncounterAndResume(state, result);
        if (result.ok)
    ...
    switch (action) {
        ...
        case "avoid":
            ...
            return;
        case "attack":
            handleResult(attackEncounter(s), "naval_maneuver");
            return;
        case "board":
            handleResult(beginBoardingCombat(s), "boarding");
            return;
        case "leave-personal":
            ...
            renderGame();
            return;
        case "leave-naval-resolution":
            delete s.encounter;
            if (s.voyage) {
                runVoyageUntilAttention(s, "Encounter resolved.");
                return;
            }
            tab = "chart";
            cue("sail");
