...
class MainMenuMusic {
    audio = new Audio();
    fadeFrame;
    targetVolume = DEFAULT_SETTINGS.musicVolume;
    enabled = true;
    unlocked = false;
    constructor() {
...
async function closeMenuAfterSuccessfulLaunch(mode) {
    if (closing)
        return;
    closing = true;
    ...
    root.remove();
    if (mode === "new") {
        document.querySelector('#creation-form input[name="name"]')?.focus({ preventScroll: true });
    }
}
window.addEventListener(LAUNCHER_READY_EVENT, (event) => {
    ...
    launchStatus = "";
    if (root.isConnected && panel === "main")
        render();
});
window.addEventListener(LAUNCHER_RESULT_EVENT, (event) => {
    const detail = event.detail;
    if (!detail || detail.requestId !== launchRequestId)
        return;
...
root.addEventListener("click", (event) => {
    ...
    if (!(target instanceof Element))
        return;
    const actionNode = target.closest("[data-menu-action]");
    if (!actionNode)
        return;
    const action = actionNode.dataset.menuAction;
    void music.unlock();
    if (action === "continue") {
        void enterGame("continue");
        return;
    }
    if (action === "new-game") {
        void enterGame("new");
...
root.addEventListener("input", (event) => {
    ...
    if (kind === "volume")
        settings.musicVolume = clamp(Number(target.value) / 100, 0, 1);
    if (kind === "music")
        settings.musicEnabled = target.checked;
    if (kind === "motion")
        settings.reducedMotion = target.checked;
    saveSettings(settings);
    music.configure(settings);
    if (kind === "music" && settings.musicEnabled)
        void music.unlock();
    if (kind === "motion")
        root.classList.toggle("reduced-motion", settings.reducedMotion);
    if (kind === "volume") {
...
window.addEventListener("keydown", (event) => {
    if (!root.isConnected || closing)
        return;
    if (event.key === "Escape" && panel !== "main") {
        panel = "main";
        render();
        return;
    }
