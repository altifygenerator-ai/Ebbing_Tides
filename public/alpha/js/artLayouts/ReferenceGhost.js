...
export function referenceGhostEnabled() {
    if (typeof window === "undefined")
        return false;
    const params = new URLSearchParams(window.location.search);
    if (params.get("referenceGhost") === "1")
        return true;
    if (params.get("referenceGhost") === "0")
        return false;
    return window.sessionStorage.getItem(GHOST_SESSION_KEY) === "1";
}
export function setReferenceGhostEnabled(enabled) {
    if (typeof window === "undefined")
        return;
    window.sessionStorage.setItem(GHOST_SESSION_KEY, enabled ? "1" : "0");
}
export function installReferenceGhostShortcut(onToggle) {
    if (typeof window === "undefined")
        return () => undefined;
    const handler = (event) => {
...
export function renderReferenceGhost(referencePath, label = "Approved reference") {
    if (!referencePath)
        return "";
...
let cleanupFns = [];
export function initializeReferenceGhosts(root = document) {
    ...
    for (const ghost of Array.from(root.querySelectorAll("[data-reference-ghost]"))) {
        const surface = ghost.closest(".reference-ghost-surface, .art-screen-letterbox, .art-directed-canvas") ?? ghost.parentElement;
        const slider = ghost.querySelector("[data-reference-opacity]");
        const image = ghost.querySelector(".reference-ghost-image");
        const controls = ghost.querySelector(".reference-ghost-controls");
        let mode = "both";
        let blinkTimer;
        let blinkOn = true;
        const apply = () => {
            ...
            if (image)
                image.style.opacity = mode === "live" ? "0" : mode === "reference" ? "1" : String(opacity);
            if (surface)
                surface.classList.toggle("reference-only", mode === "reference");
        ...
        for (const button of Array.from(ghost.querySelectorAll("[data-reference-mode]"))) {
            const click = () => {
                mode = (button.dataset.referenceMode ?? "both");
                apply();
            };
            button.addEventListener("click", click);
            cleanupFns.push(() => button.removeEventListener("click", click));
        }
        const blink = ghost.querySelector("[data-reference-blink]");
        if (blink) {
            const click = () => {
                ...
                blinkTimer = window.setInterval(() => {
                    blinkOn = !blinkOn;
                    if (image)
                        image.style.opacity = blinkOn ? String(Math.max(0, Math.min(100, Number(slider?.value ?? storedOpacity))) / 100) : "0";
                }, 650);
            };
...
//# sourceMappingURL=ReferenceGhost.js.map