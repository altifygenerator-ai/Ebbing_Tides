...
export function renderArtScreenHost(options) {
...
function updateCalibrationHud(host, canvas, width, height, left, top) {
    const viewportNode = canvas.querySelector("[data-art-viewport]");
    const mainNode = canvas.querySelector("[data-art-main-host]");
    const hostNode = canvas.querySelector("[data-art-host]");
    const canvasNode = canvas.querySelector("[data-art-canvas]");
    const letterboxNode = canvas.querySelector("[data-art-letterbox]");
    const main = host.closest(".main") ?? host.parentElement;
    const mainRect = main?.getBoundingClientRect();
    const hostRect = host.getBoundingClientRect();
    if (viewportNode)
        viewportNode.textContent = `${window.innerWidth} × ${window.innerHeight}`;
    if (mainNode)
        mainNode.textContent = mainRect ? `${numberText(mainRect.width)} × ${numberText(mainRect.height)}` : "—";
    if (hostNode)
        hostNode.textContent = `${numberText(hostRect.width)} × ${numberText(hostRect.height)}`;
    if (canvasNode)
        canvasNode.textContent = `${numberText(width)} × ${numberText(height)}`;
    if (letterboxNode)
        letterboxNode.textContent = `${numberText(left)} / ${numberText(top)}`;
}
function fitHost(host) {
    const canvas = host.querySelector(".art-directed-canvas");
    const letterbox = host.querySelector(".art-screen-letterbox");
    if (!canvas || !letterbox)
        return;
    ...
    if (!(nativeWidth > 0 && nativeHeight > 0 && logicalWidth > 0 && logicalHeight > 0))
        return;
...
let hostCleanups = [];
export function initializeArtScreenHosts(root = document) {
    hostCleanups.forEach((cleanup) => cleanup());
    hostCleanups = [];
    for (const host of Array.from(root.querySelectorAll("[data-art-screen-host]"))) {
...
//# sourceMappingURL=ArtScreenHost.js.map