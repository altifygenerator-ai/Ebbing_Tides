...
export const WORLD_DEVELOPED_BOUNDS = { x: 0, y: 0, width: WORLD_TERRAIN_MASK_WIDTH, height: WORLD_TERRAIN_MASK_HEIGHT };
...
function inAtlas(point) { return point.x >= 0 && point.y >= 0 && point.x < GLOBAL_ATLAS.width && point.y < GLOBAL_ATLAS.height; }
...
export function isNavigableCell(point) { return getWorldCell(point).navigable; }
export function isMappedWater(point) { return inAtlas(point) && isWater(point); }
...
export function viewportAround(point, zoom = "navigation") {
    const view = navigationViewForZoom(zoom);
    return clampNavigationViewport({ x: Math.floor(point.x - view.width / 2), y: Math.floor(point.y - view.height / 2) }, zoom);
}
export function cellKey(point) { return `${point.x},${point.y}`; }
export function parseCellKey(key) { const [x, y] = key.split(",").map(Number); return { x: x ?? 0, y: y ?? 0 }; }
