...
function finishVoyageAutomation(result:ReturnType<typeof sailUntilInterrupted>,prefix:string|undefined,weatherEvents:string[]):void {
...
async function runVoyageUntilAttention(s:GameState,prefix?:string):Promise<void> {
  ...
  // Render the chart immediately before the first simulation burst. This lets
  // the browser paint the departure state instead of appearing frozen.
  mapCameraState=undefined;
  lastSearchWatersResult=undefined;
  tab="chart";
  renderGame();
  ...
  try {
    ...
    while(s===state && s.voyage && runId===voyageAutomationRunId && completedSteps<VOYAGE_AUTOMATION_MAX_STEPS){
      ...
      if(!chunkExhausted){
        finishVoyageAutomation(result,prefix,weatherEvents);
        return;
      }

      completedSteps+=chunkSteps;

      // Repaint periodically so the chart/camera visibly follows long voyages,
      // but not every 2-hour simulation step.
...
function finishVoyageAutomation(result, prefix, weatherEvents) {
    mapCameraState = undefined;
    lastSearchWatersResult = undefined;
...
# ---------------------------------------------------------------------------
# 5) Write atomically enough for a local hotfix: both edits are prepared before
#    either target is changed, and backups already exist.
# ---------------------------------------------------------------------------
...
Write-Host "  - Chart repaints every 12 steps and follows the ship"
