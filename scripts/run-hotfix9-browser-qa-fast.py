...
class C:
 def __init__(self):
  page=next(p for p in requests.get(DEBUG+'/json',timeout=5).json() if p['type']=='page'); self.ws=websocket.create_connection(page['webSocketDebuggerUrl'],timeout=15); self.i=0; self.call('Page.enable');self.call('Runtime.enable')
 ...
 def nav(self,u): self.call('Page.navigate',{'url':u});time.sleep(.75)
 ...
 def shot(self,p):
  x=self.call('Page.captureScreenshot',{'format':'jpeg','quality':62,'fromSurface':True,'captureBeyondViewport':False});Path(p).write_bytes(base64.b64decode(x['data']))
c=C(); c.nav(BASE)
SC={
'equipment_male':(MALE,'[data-tab="inventory"]','[data-action="set-character-subtab"][data-owner="captain"][data-view="gear"]'),
'equipment_female':(FEMALE,'[data-tab="inventory"]','[data-action="set-character-subtab"][data-owner="captain"][data-view="gear"]'),
'ship_management':(MALE,'[data-tab="ship"]',None),'market':(MALE,'[data-tab="market"]',None),'crew_roster':(MALE,'[data-tab="crew"]',None),'journal':(MALE,'[data-tab="journal"]',None),'naval_encounter':(ENCOUNTER,None,None),'character_creator':(None,None,None)}
def load(name):
 st,sel,sub=SC[name]
 if st is None:
  c.ev("localStorage.removeItem('ebbing-tides.alpha.save');sessionStorage.clear()");c.nav(BASE);return
 c.ev("localStorage.setItem('ebbing-tides.alpha.save',"+json.dumps(st)+");sessionStorage.clear()");c.nav(BASE);c.ev("document.querySelector('[data-action=\"continue-save\"]')?.click()");time.sleep(.18)
 if sel:c.ev('document.querySelector('+json.dumps(sel)+')?.click()');time.sleep(.15)
 if sub:c.ev('document.querySelector('+json.dumps(sub)+')?.click()');time.sleep(.15)
def toggle(): c.ev("window.dispatchEvent(new KeyboardEvent('keydown',{altKey:true,shiftKey:true,code:'KeyC',bubbles:true}))");time.sleep(.18)
def populate(name):
 if name=='character_creator':c.ev("document.querySelector('[data-action=\"creator-step\"][data-step=\"4\"]')?.click()")
 elif name=='journal':c.ev("document.querySelector('[data-action=\"journal-tab\"][data-tab=\"history\"]')?.click()")
...
def geom():
 ...
 checks['overflowPolicySatisfied']=all('encounter-log-region' in x['className'] for x in g['overflowingRegions']) if g['layoutId']=='ui.combat.naval_encounter' else len(g['overflowingRegions'])==0
 g['checks']=checks;g['pass']=all(checks.values());return g
report={'generatedAt':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'browser':c.ev('navigator.userAgent'),'screens':{}}
for name in SC:
 load(name); report['screens'][name]={'viewports':{}}
 for phase in ['A_live','B_calibration','C_populated']:
  if phase=='B_calibration':toggle()
  if phase=='C_populated':toggle();populate(name)
  for w,h in VPS:
   c.vp(w,h);g=geom();key=f'{w}x{h}';p=OUT/f'{name}_{key}_{phase}.jpg';c.shot(p);entry=report['screens'][name]['viewports'].setdefault(key,{});entry[phase]=g;entry.setdefault('screenshots',{})[phase]=str(p)
for s in report['screens'].values():
 for v in s['viewports'].values():v['pass']=all(v[p]['pass'] for p in ['A_live','B_calibration','C_populated'])
report['pass']=all(v['pass'] for s in report['screens'].values() for v in s['viewports'].values())
(OUT/'browser-geometry.json').write_text(json.dumps(report,indent=2)+'\n')
print('PASS' if report['pass'] else 'FAIL')
for n,s in report['screens'].items():print(n,[k for k,v in s['viewports'].items() if not v['pass']])
